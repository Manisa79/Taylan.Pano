...
Set-Location $repo
if (-not $SkipBuild) {
    powershell -ExecutionPolicy Bypass -File .\build\Build-Gridly.ps1 -Configuration $Configuration -Pack
}
New-Item -ItemType Directory -Force .\artifacts\release | Out-Null
$zip = ".\artifacts\release\Gridly_v$($Version.Replace('.','_'))_Release.zip"
