#!/usr/bin/env python3
"""Lightweight static guard for Gridly partial-class growth.

It is not a compiler. It catches the most common regression seen during rapid
phase additions: duplicate GridlyView properties/events and sample code that
...
# Known nested classes in GridlyView*.cs may have properties named RowObject, Column, etc.
# These are intentionally ignored because they are not GridlyView members.
IGNORE_NAMES = {"RowObject", "RowIndex", "Column", "Model", "Name", "Position", "Rows", "TargetViewIndex", "GridlyView"}


def scan() -> int:
    props: dict[str, list[str]] = defaultdict(list)
    events: dict[str, list[str]] = defaultdict(list)
    for file in CORE.glob("GridlyView*.cs"):
        text = file.read_text(encoding="utf-8-sig", errors="ignore")
        marker = "partial class GridlyView"
    ...
    for title, data in (("property", props), ("event", events)):
        for name, files in sorted(data.items()):
            # GridlyPlatformFeatureOptions is a separate options class that may share names.
    ...
    print("Gridly API guard completed. Review CHECK/WARNING lines before release.")
