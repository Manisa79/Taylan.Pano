...
public sealed class MasterDataScenarioSampleForm : Form
{
    ...
    {
        Text = "Gridly MasterData Görünüm Senaryoları";
        Width = 1280;
        Height = 780;
        ...
        ApplyScenario(GridlyViewScenario.DataTable);
    }

    private void ConfigureColumns()
    {
        _grid.Columns.Add(new GridlyColumn("Tür", nameof(MasterDataRow.Type), 110) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Kod", nameof(MasterDataRow.Code), 150));
        _grid.Columns.Add(new GridlyColumn("Açıklama", nameof(MasterDataRow.Name), 280) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new GridlyColumn("Makine", nameof(MasterDataRow.Machine), 120));
        _grid.Columns.Add(new GridlyColumn("Katman", nameof(MasterDataRow.Layer), 80));
        _grid.Columns.Add(new GridlyColumn("RefDes", nameof(MasterDataRow.RefDes), 120));
        _grid.Columns.Add(new GridlyColumn("Miktar", nameof(MasterDataRow.Quantity), 70));
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(MasterDataRow.Status), 100) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Detay", nameof(MasterDataRow.Detail), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 5 });
    }

    private void ConfigureToolbar()
    {
        foreach (GridlyViewScenario scenario in Enum.GetValues<GridlyViewScenario>())
        {
            GridlyViewScenario captured = scenario;
    ...
    private void ApplyScenario(GridlyViewScenario scenario)
    {
        _grid.ClearGrouping();
        LoadRows(scenario);
        _grid.ApplyScenario(scenario);

        if (scenario == GridlyViewScenario.ProductTree)
            _grid.SetGroupBy(nameof(MasterDataRow.Type));
        else if (scenario == GridlyViewScenario.TicketBoard)
            _grid.SetGroupBy(nameof(MasterDataRow.Status));
        else if (scenario == GridlyViewScenario.ProgramFiles)
            _grid.SetGroupBy(nameof(MasterDataRow.Machine));

        _info.Text = GetScenarioDescription(scenario);
    }

    private void LoadRows(GridlyViewScenario scenario)
    {
        _rows.Clear();
        ...
        switch (scenario)
        {
            ...
            case GridlyViewScenario.Timeline:
                ...
                break;
            default:
                AddBomRows(scenario == GridlyViewScenario.DenseData ? 260 : 80);
                break;
        }
    ...
    private static string GetScenarioDescription(GridlyViewScenario scenario)
        => scenario switch
        {
            GridlyViewScenario.DataTable => "Standart Tablo: MasterData içinde SAP sonuçları, ürün ağaçları, SQL kayıtları ve kolon filtreleri için güvenli varsayılan görünüm.",
            GridlyViewScenario.DenseData => "Yoğun Veri Tablosu: çok fazla BOM/pozisyon satırında maksimum kayıt görmek için kısa satır yüksekliği.",
            GridlyViewScenario.ProductTree => "Ürün Ağacı: yarı mamul, alt ürün, komponent ve operasyon kırılımlarını gruplu liste olarak gösterir.",
            GridlyViewScenario.BomPositions => "BOM / Pozisyon Listesi: RefDes, malzeme kodu, katman ve miktar gibi teknik satırlar için Excel benzeri görünüm.",
            GridlyViewScenario.ProgramFiles => "Program Dosyaları: makine bazlı çıktı dosyaları, dizgi programları ve kontrol adımları için kart + grup kullanımı.",
            GridlyViewScenario.MachineOrLinePicker => "Makine / Hat Seçimi: Flex, QX, AX, OIB, KnS gibi hat/makine seçimlerinde ikon grid mantığı.",
            GridlyViewScenario.TicketBoard => "Ticket Dashboard: AOI Support Desk tarzı açık/bekliyor/çözüldü durumlarını kartlarla izlemek için.",
            GridlyViewScenario.Timeline => "İşlem Geçmişi: program oluşturma, SAP okuma, kullanıcı kararı ve hata loglarını kronolojik kartlarla göstermek için.",
            GridlyViewScenario.MasterDetail => "MasterData Detay: üstte ana kayıt, altta/yan tarafta pozisyon-detay gridleri olan bakım formları için.",
