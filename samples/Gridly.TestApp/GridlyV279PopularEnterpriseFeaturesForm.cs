...
public sealed class GridlyV279PopularEnterpriseFeaturesForm : Form
{
    ...
    private readonly GridlyView _grid = new()
    {
        ...
        PopularFeaturePreset = GridlyPopularFeaturePreset.MasterData,
    ...
    public GridlyV279PopularEnterpriseFeaturesForm()
    {
        Text = "Gridly v27.9 - Popular Enterprise Features";
        Width = 1380;
        Height = 820;
    ...
    {
        _tool.Items.Add(new ToolStripButton("Balanced", null, (_, __) => ApplyPreset(GridlyPopularFeaturePreset.Balanced)));
        _tool.Items.Add(new ToolStripButton("MasterData", null, (_, __) => ApplyPreset(GridlyPopularFeaturePreset.MasterData)));
        _tool.Items.Add(new ToolStripButton("SupportDesk", null, (_, __) => ApplyPreset(GridlyPopularFeaturePreset.SupportDesk)));
        _tool.Items.Add(new ToolStripButton("Large Data", null, (_, __) => ApplyPreset(GridlyPopularFeaturePreset.LargeDataReview)));
        _tool.Items.Add(new ToolStripButton("Data Entry", null, (_, __) => ApplyPreset(GridlyPopularFeaturePreset.DataEntry)));
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new GridlyColumn("ID", nameof(EnterpriseRow.Id), 58));
        _grid.Columns.Add(new GridlyColumn("Tip", nameof(EnterpriseRow.Type), 105) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Kod", nameof(EnterpriseRow.Code), 130));
        _grid.Columns.Add(new GridlyColumn("Başlık / Malzeme", nameof(EnterpriseRow.Title), 280) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new GridlyColumn("Makine", nameof(EnterpriseRow.Machine), 130) { AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(EnterpriseRow.Status), 126) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Miktar", nameof(EnterpriseRow.Quantity), 90));
        _grid.Columns.Add(new GridlyColumn("İlerleme", nameof(EnterpriseRow.Progress), 96) { Kind = GridlyColumnKind.ProgressBar });
        _grid.Columns.Add(new GridlyColumn("Etiketler", nameof(EnterpriseRow.Tags), 230) { Kind = GridlyColumnKind.Tags });
        _grid.Columns.Add(new GridlyColumn("Açıklama / Yol", nameof(EnterpriseRow.Description), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }
    ...
    private void ApplyDemoEnhancements()
    {
        ...
        _grid.AddNumericSummary(nameof(EnterpriseRow.Quantity), GridlySummaryType.Sum, "Toplam {0}");
        _grid.AddNumericSummary(nameof(EnterpriseRow.Progress), GridlySummaryType.Average, "Ort. %{0:0}");
    ...
    private void ApplyPreset(GridlyPopularFeaturePreset preset)
    {
    ...
    {
        _dark = dark;
        var theme = dark ? GridlyTheme.DarkTheme() : GridlyTheme.LightTheme();
