...
using GridlyToastKind = global::Gridly.Notifications.GridlyToastKind;
using System.Data;
using Gridly.Tree;
...
{
    public static GridlyTheme AppTheme { get; set; } = WindowsThemeService.CurrentTheme();
    ...
    {
        string settingsFile = Path.Combine(Application.StartupPath, "settings", "gridly-testapp-language.json");
        ...
        Gridly.Localization.GridlyLocalization.Use(languageForm.SelectedLanguage);
    ...
    private static void InstallGlobalExceptionGuards()
    {
        ...
        {
            Debug.WriteLine("Gridly UI exception: " + e.Exception);
        };

        AppDomain.CurrentDomain.UnhandledException += (_, e) =>
        {
            Debug.WriteLine("Gridly domain exception: " + e.ExceptionObject);
        };

        TaskScheduler.UnobservedTaskException += (_, e) =>
        {
            Debug.WriteLine("Gridly task exception: " + e.Exception);
            e.SetObserved();
        };
    }
}
...
{
    private readonly Gridly.Core.GridlyView _gridly = new()
    {
        Dock = DockStyle.Fill,
    ...
    private GridlyTheme _currentTheme = Program.AppTheme;
    private string _themeMode = "Windows";
    private bool _suppressSearchFilter;

    public MainForm()
    {
        Text = $"{GridlyVersionInfo.SuggestedProjectName} Test App - {GetAppVersionText()}";
        Width = 1240;
        Height = 800;
        MinimumSize = new Size(980, 620);
        GridlyWindowChrome.ApplyOnHandleCreated(this, () => _currentTheme, true);

        BuildToolbar();
        BuildThemePreview();

        Controls.Add(_gridly);
        ...
        _search.TextChanged += (_,__) =>
        {
            ...
            _gridly.SetGlobalFilter(_search.Text);
            if (!string.IsNullOrWhiteSpace(_search.Text)) _gridly.JumpToFirstMatch(_search.Text);
        };
        _gridly.ButtonClick += (_, e) => MessageBox.Show(this, $"Buton: {((DemoRow)e.RowObject).Name}", "GridlyView");
        _gridly.CellValueChanged += (_, e) => _status.Text = $"Güncellendi: {e.Column.Header} = {e.NewValue}";
        _gridly.ShowSummaryFooter = true;
        _gridly.EnableRowDetails = true;
        _gridly.FilterMenuMode = GridlyFilterMenuMode.Both;
        ConfigureMillionRowFiltering(_gridly);
        _gridly.SetRowDetailsProvider(CreateDetailsProvider());
    ...
    internal static void ConfigureMillionRowFiltering(Gridly.Core.GridlyView grid)
    {
    ...
    private void ShowVersionInfo()
    {
        MessageBox.Show(this,
            $"Proje adı önerisi: {GridlyVersionInfo.SuggestedProjectName}\n" +
            $"DLL: {GridlyVersionInfo.DisplayText}\n" +
    ...
    private void BuildToolbar()
    {
        ...
        _tool.Items.Add(new ToolStripButton("Header Sağ Tık / Filtre Popup", null, (_,__) => _gridly.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        _tool.Items.Add(new ToolStripButton("Ad filtre penceresi", null, (_,__) => _gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        _tool.Items.Add(new ToolStripButton("Kolon Seç", null, (_,__) => _gridly.ShowColumnChooser()));

        var samplesMenu = new ToolStripDropDownButton("Gridly");
        BuildProductGridlyMenu(samplesMenu);
        _tool.Items.Add(samplesMenu);

        var viewMenu = new ToolStripDropDownButton("Görünüm");
        viewMenu.DropDownItems.Add("Kompakt", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.ExtraLargeIcons); _status.Text = "Görünüm: Kompakt"; });
        viewMenu.DropDownItems.Add("Standart", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.LargeIcons); _status.Text = "Görünüm: Standart"; });
        viewMenu.DropDownItems.Add("Geniş", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.MediumIcons); _status.Text = "Görünüm: Geniş"; });
        viewMenu.DropDownItems.Add("Liste", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.List); _status.Text = "Görünüm: Liste"; });
        viewMenu.DropDownItems.Add("Kart", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.Tile); _status.Text = "Görünüm: Kart"; });
        viewMenu.DropDownItems.Add("Geniş Kart", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.LargeCard); _status.Text = "Görünüm: Geniş Kart"; });
        viewMenu.DropDownItems.Add("Dashboard Kart", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.DashboardCard); _status.Text = "Görünüm: Dashboard Kart"; });
        viewMenu.DropDownItems.Add("Satır Kart", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.RowCard); _status.Text = "Görünüm: Satır Kart"; });
        viewMenu.DropDownItems.Add("DetailCard", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.DetailCard); _status.Text = "Görünüm: DetailCard"; });
        viewMenu.DropDownItems.Add("Poster", null, (_,__) => { _gridly.TilePosterMode = true; _gridly.SetViewMode(GridlyViewMode.Poster); _status.Text = "Görünüm: Poster"; });
        viewMenu.DropDownItems.Add("MediaTile", null, (_,__) => { _gridly.TilePosterMode = true; _gridly.SetViewMode(GridlyViewMode.MediaTile); _status.Text = "Görünüm: MediaTile"; });
        viewMenu.DropDownItems.Add("Gallery", null, (_,__) => { _gridly.TilePosterMode = true; _gridly.SetViewMode(GridlyViewMode.Gallery); _status.Text = "Görünüm: Gallery"; });
        viewMenu.DropDownItems.Add("FilmStrip", null, (_,__) => { _gridly.TilePosterMode = true; _gridly.SetViewMode(GridlyViewMode.FilmStrip); _status.Text = "Görünüm: FilmStrip"; });
        viewMenu.DropDownItems.Add("GroupCard", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.GroupCard); _status.Text = "Görünüm: GroupCard"; });
        viewMenu.DropDownItems.Add("PropertyCard", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.PropertyCard); _status.Text = "Görünüm: PropertyCard"; });
        viewMenu.DropDownItems.Add("KPI Dashboard", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.KpiDashboard); _status.Text = "Görünüm: KPI Dashboard"; });
        viewMenu.DropDownItems.Add("HeatMap", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.HeatMap); _status.Text = "Görünüm: HeatMap"; });
        viewMenu.DropDownItems.Add("MiniChart", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.MiniChart); _status.Text = "Görünüm: MiniChart"; });
        viewMenu.DropDownItems.Add("RowPreview", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.RowPreview); _status.Text = "Görünüm: RowPreview"; });
        viewMenu.DropDownItems.Add("MasterDetail", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.MasterDetail); _status.Text = "Görünüm: MasterDetail"; });
        viewMenu.DropDownItems.Add("Kanban", null, (_,__) => { _gridly.TilePosterMode = false; _gridly.SetViewMode(GridlyViewMode.Kanban); _status.Text = "Görünüm: Kanban"; });
        viewMenu.DropDownItems.Add("Zaman Akışı", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.Timeline); _status.Text = "Görünüm: Zaman Akışı"; });
        viewMenu.DropDownItems.Add("Detay", null, (_,__) => { _gridly.SetViewMode(GridlyViewMode.Details); _status.Text = "Görünüm: Detay"; });
        _tool.Items.Add(viewMenu);

        var groupMenu = new ToolStripDropDownButton("Gruplama");
        groupMenu.DropDownItems.Add("Duruma göre grupla", null, (_,__) => { _gridly.SetGroupBy(nameof(DemoRow.State)); _status.Text = "Gruplama: Durum"; });
        groupMenu.DropDownItems.Add("Mesleğe göre grupla", null, (_,__) => { _gridly.SetGroupBy(nameof(DemoRow.Occupation)); _status.Text = "Gruplama: Meslek"; });
        groupMenu.DropDownItems.Add("Tüm grupları daralt", null, (_,__) => { _gridly.CollapseAllGroups(); _status.Text = "Tüm gruplar daraltıldı"; });
        groupMenu.DropDownItems.Add("Tüm grupları aç", null, (_,__) => { _gridly.ExpandAllGroups(); _status.Text = "Tüm gruplar açıldı"; });
        groupMenu.DropDownItems.Add("Gruplamayı kaldır", null, (_,__) => { _gridly.ClearGrouping(); _status.Text = "Gruplama kapalı"; });
        _tool.Items.Add(groupMenu);

        var filterStyleMenu = new ToolStripDropDownButton("Filtre Stili");
        filterStyleMenu.DropDownItems.Add("Popup menü", null, (_,__) => { _gridly.FilterMenuMode = GridlyFilterMenuMode.PopupMenu; _status.Text = "Filtre stili: Popup menü"; });
        filterStyleMenu.DropDownItems.Add("Ayrı pencere", null, (_,__) => { _gridly.FilterMenuMode = GridlyFilterMenuMode.ModalWindow; _status.Text = "Filtre stili: Ayrı pencere"; });
        filterStyleMenu.DropDownItems.Add("İkisi birlikte", null, (_,__) => { _gridly.FilterMenuMode = GridlyFilterMenuMode.Both; _status.Text = "Filtre stili: İkisi birlikte"; });
        _tool.Items.Add(filterStyleMenu);

        var contextMenuMode = new ToolStripDropDownButton("Başlık Sağ Tık");
        contextMenuMode.DropDownItems.Add("Menü yok", null, (_,__) => { _gridly.HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.None; _status.Text = "Başlık sağ tık: menü yok"; });
        contextMenuMode.DropDownItems.Add("Sadece filtre popup", null, (_,__) => { _gridly.HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.FilterOnly; _status.Text = "Başlık sağ tık: sadece filtre"; });
        contextMenuMode.DropDownItems.Add("Tam menü", null, (_,__) => { _gridly.HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.Full; _status.Text = "Başlık sağ tık: tam menü"; });
        contextMenuMode.DropDownItems.Add(new ToolStripSeparator());
        contextMenuMode.DropDownItems.Add("Sıralama öğelerini aç/kapat", null, (_,__) => { _gridly.ShowHeaderMenuSortItems = !_gridly.ShowHeaderMenuSortItems; _status.Text = $"Sıralama menüsü: {_gridly.ShowHeaderMenuSortItems}"; });
        contextMenuMode.DropDownItems.Add("Layout öğelerini aç/kapat", null, (_,__) => { _gridly.ShowHeaderMenuLayoutItems = !_gridly.ShowHeaderMenuLayoutItems; _status.Text = $"Layout menüsü: {_gridly.ShowHeaderMenuLayoutItems}"; });
        contextMenuMode.DropDownItems.Add("Gruplama öğelerini aç/kapat", null, (_,__) => { _gridly.ShowHeaderMenuGroupingItems = !_gridly.ShowHeaderMenuGroupingItems; _status.Text = $"Gruplama menüsü: {_gridly.ShowHeaderMenuGroupingItems}"; });
        contextMenuMode.DropDownItems.Add(new ToolStripSeparator());
        contextMenuMode.DropDownItems.Add("Profil: Full", null, (_,__) => { _gridly.ApplyMenuProfile(GridlyMenuProfile.Full); _status.Text = "Menü profili: Full"; });
        contextMenuMode.DropDownItems.Add("Profil: Standard", null, (_,__) => { _gridly.ApplyMenuProfile(GridlyMenuProfile.Standard); _status.Text = "Menü profili: Standard"; });
        contextMenuMode.DropDownItems.Add("Profil: Minimal", null, (_,__) => { _gridly.ApplyMenuProfile(GridlyMenuProfile.Minimal); _status.Text = "Menü profili: Minimal"; });
        contextMenuMode.DropDownItems.Add("Profil: ReadOnly", null, (_,__) => { _gridly.ApplyMenuProfile(GridlyMenuProfile.ReadOnly); _status.Text = "Menü profili: ReadOnly"; });
        contextMenuMode.DropDownItems.Add("Profil: None", null, (_,__) => { _gridly.ApplyMenuProfile(GridlyMenuProfile.None); _status.Text = "Menü profili: None / Gridly menüleri kapalı"; });
        contextMenuMode.DropDownItems.Add("Boş liste bilgisini aç/kapat", null, (_,__) => { _gridly.ShowEmptyListMessage = !_gridly.ShowEmptyListMessage; _gridly.Invalidate(); _status.Text = $"Boş liste bilgisi: {_gridly.ShowEmptyListMessage}"; });
        _tool.Items.Add(contextMenuMode);

        var colorMenu = new ToolStripDropDownButton("Satır Renkleri");
        void AddRowPreset(string text, GridlyRowColorPreset preset, double strength = 0.18)
        {
            colorMenu.DropDownItems.Add(text, null, (_,__) =>
            {
                _gridly.RowBackColorGetter = null;
                _gridly.RowForeColorGetter = null;
                _gridly.RowColorAspectName = nameof(DemoRow.State);
                _gridly.RowColorStrength = strength;
                _gridly.RowColorPreset = preset;
                _gridly.CustomGroupBackColor = Color.Empty;
                _gridly.Invalidate();
                _status.Text = "Satır renkleri: " + text;
            });
        }
        AddRowPreset("Tema varsayılanı", GridlyRowColorPreset.ThemeDefault);
        AddRowPreset("Yumuşak zebra", GridlyRowColorPreset.SubtleZebra, 0.12);
        AddRowPreset("Tema vurgu satırları", GridlyRowColorPreset.SoftAccent, 0.13);
        AddRowPreset("AOI risk / durum uyumlu", GridlyRowColorPreset.AOIRisk, 0.22);
        AddRowPreset("Durum bazlı pastel", GridlyRowColorPreset.StatusPills, 0.18);
        AddRowPreset("Severity bantları", GridlyRowColorPreset.SeverityBands, 0.24);
        AddRowPreset("Pastel kart görünümü", GridlyRowColorPreset.PastelCards, 0.16);
        AddRowPreset("Focus glow", GridlyRowColorPreset.FocusGlow, 0.18);
        colorMenu.DropDownItems.Add(new ToolStripSeparator());
        colorMenu.DropDownItems.Add("Özel: Fail yazısı vurgulu", null, (_,__) =>
        {
            _gridly.RowColorPreset = GridlyRowColorPreset.AOIRisk;
            _gridly.RowColorAspectName = nameof(DemoRow.State);
            _gridly.RowColorStrength = 0.24;
            _gridly.RowForeColorGetter = row => ((DemoRow)row).State == "Fail" ? (_currentTheme.IsDark ? Color.FromArgb(255, 190, 190) : Color.FromArgb(140, 0, 0)) : null;
            _gridly.Invalidate();
        ...
        {
            if (!_gridly.FindNext(_search.Text))
        ...
        {
            if (!_gridly.FindPrevious(_search.Text))
                _status.Text = "Eşleşme bulunamadı";
            else
        ...
        {
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.csv");
            try
            {
                var actualPath = _gridly.ExportVisibleCsv(path);
        ...
        {
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.xlsx");
            try
            {
                var actualPath = _gridly.ExportVisibleExcel(path, "GridlyView");
        ...
        _tool.Items.Add(new ToolStripButton("Yazdırma Önizleme", null, (_,__) => _gridly.ShowPrintPreview("GridlyView Ana Örnek")));
        _tool.Items.Add(new ToolStripButton("Yazdır", null, (_,__) => _gridly.Print("GridlyView Ana Örnek")));
        _tool.Items.Add(new ToolStripButton("Layout Kaydet", null, (_,__) => _gridly.SaveLayout(Path.Combine(Application.StartupPath, "gridly-layout.json"))));
        _tool.Items.Add(new ToolStripButton("Layout Yükle", null, (_,__) => _gridly.LoadLayout(Path.Combine(Application.StartupPath, "gridly-layout.json"))));
        ...
        themeMenu.DropDownItems.Add("Açık tema", null, (_,__) => { _themeMode = "Light"; ApplyAppTheme(GridlyTheme.LightTheme(), "Açık tema"); });
        themeMenu.DropDownItems.Add("Koyu tema", null, (_,__) => { _themeMode = "Dark"; ApplyAppTheme(GridlyTheme.DarkTheme(), "Koyu tema"); });
        themeMenu.DropDownItems.Add("Fluent açık + acrylic", null, (_,__) => { _themeMode = "FluentLight"; ApplyAppTheme(GridlyTheme.FluentLightTheme(), "Fluent açık + acrylic"); });
        themeMenu.DropDownItems.Add("Fluent koyu + acrylic", null, (_,__) => { _themeMode = "FluentDark"; ApplyAppTheme(GridlyTheme.FluentDarkTheme(), "Fluent koyu + acrylic"); });
        themeMenu.DropDownItems.Add(new ToolStripSeparator());
        themeMenu.DropDownItems.Add("Midnight Purple", null, (_,__) => { _themeMode = "MidnightPurple"; _gridly.ThemePreset = GridlyThemePreset.MidnightPurple; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.MidnightPurple), "Midnight Purple"); });
        themeMenu.DropDownItems.Add("Slate Blue", null, (_,__) => { _themeMode = "SlateBlue"; _gridly.ThemePreset = GridlyThemePreset.SlateBlue; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.SlateBlue), "Slate Blue"); });
        themeMenu.DropDownItems.Add("Graphite", null, (_,__) => { _themeMode = "Graphite"; _gridly.ThemePreset = GridlyThemePreset.Graphite; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Graphite), "Graphite"); });
        themeMenu.DropDownItems.Add("Emerald", null, (_,__) => { _themeMode = "Emerald"; _gridly.ThemePreset = GridlyThemePreset.Emerald; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Emerald), "Emerald"); });
        themeMenu.DropDownItems.Add("Warm Sand", null, (_,__) => { _themeMode = "WarmSand"; _gridly.ThemePreset = GridlyThemePreset.WarmSand; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.WarmSand), "Warm Sand"); });
        themeMenu.DropDownItems.Add("Ocean", null, (_,__) => { _themeMode = "Ocean"; _gridly.ThemePreset = GridlyThemePreset.Ocean; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Ocean), "Ocean"); });
        themeMenu.DropDownItems.Add("Rose Quartz", null, (_,__) => { _themeMode = "RoseQuartz"; _gridly.ThemePreset = GridlyThemePreset.RoseQuartz; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.RoseQuartz), "Rose Quartz"); });
        themeMenu.DropDownItems.Add("Cyber Neon", null, (_,__) => { _themeMode = "CyberNeon"; _gridly.ThemePreset = GridlyThemePreset.CyberNeon; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.CyberNeon), "Cyber Neon"); });
        themeMenu.DropDownItems.Add("Nord", null, (_,__) => { _themeMode = "Nord"; _gridly.ThemePreset = GridlyThemePreset.Nord; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Nord), "Nord"); });
        themeMenu.DropDownItems.Add("Olive", null, (_,__) => { _themeMode = "Olive"; _gridly.ThemePreset = GridlyThemePreset.Olive; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Olive), "Olive"); });
        themeMenu.DropDownItems.Add("Steel", null, (_,__) => { _themeMode = "Steel"; _gridly.ThemePreset = GridlyThemePreset.Steel; ApplyAppTheme(GridlyTheme.FromPreset(GridlyThemePreset.Steel), "Steel"); });
        themeMenu.DropDownItems.Add(new ToolStripSeparator());
        ...
        _tool.Items.Add(new ToolStripSeparator());
        _versionLabel.Text = $"DLL: {GridlyVersionInfo.InformationalVersion} | App: {GetAppVersionText()}";
    ...
    private void BuildProductGridlyMenu(ToolStripDropDownButton root)
    {
        ...
        AddMenuItem(root.DropDownItems, "Audix Medya Pilot", () => new GridlyV51RealUsagePilotSampleForm().Show());
        AddMenuItem(root.DropDownItems, "Theme Audit / Okunurluk", () => new GridlyV51RealUsagePilotSampleForm().Show());
        AddMenuItem(root.DropDownItems, "Media Playback State", () => new GridlyV41MediaPlaybackSampleForm().Show());
        ...
        BuildDeveloperGridlyMenu(developer);
    }

    private void BuildDeveloperGridlyMenu(ToolStripMenuItem developer)
    {
        ...
        AddMenuItem(latest.DropDownItems, "Gridly 5.1 Audix Pilot / Theme Audit", () => new GridlyV51RealUsagePilotSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "Gridly v50.2 Build & Runtime Hardening", () => new GridlyV502HardeningSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "Gridly 5.0 Foundation / Stability", () => new GridlyV50FoundationSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "V41 Media Playback State", () => new GridlyV41MediaPlaybackSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "V37-V40 Pro Experience", () => new GridlyV37ToV40ProExperienceSampleForm().Show());
        ...
        AddMenuItem(designer.DropDownItems, "Kolon Designer Editor Smoke Test", () => new GridlyColumnDesignerEditorSmokeForm().Show());
        ...
        AddMenuItem(designer.DropDownItems, "Gridly Designer Smart Tag", () => new GridlyDesignerSmartTagSampleForm().Show());
        AddMenuItem(designer.DropDownItems, "Gridly/GLV uyumluluk / SetObjects", () => new GridlyCompatSampleForm().Show());
        AddMenuItem(designer.DropDownItems, "Gridly kolon özellikleri uyumluluk", () => new GridlyColumnCompatibilitySampleForm().Show());
    ...
    private GridlyRowDetailsProvider CreateDetailsProvider() => new()
    {
        PreferredHeight = 82,
    ...
    {
        _gridly.Columns.Add(new GridlyColumn("✓", nameof(DemoRow.Checked), 42)
        {
        ...
        _gridly.Columns.Add(new GridlyColumn("İşaret", nameof(DemoRow.NeedsReview), 58)
        {
            Kind = GridlyColumnKind.CheckBox,
            AspectPutter = (row, value) => ((DemoRow)row).SetNeedsReview(Convert.ToBoolean(value))
        });
        _gridly.Columns.Add(new GridlyColumn("Id", nameof(DemoRow.Id), 70));
        _gridly.Columns.Add(new GridlyColumn("İkon", nameof(DemoRow.State), 70)
        {
            Kind = GridlyColumnKind.Icon,
            ImageGetter = row => CreateStateIcon(((DemoRow)row).State),
            AspectGetter = row => ((DemoRow)row).State
        });
        _gridly.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 220) { Editable = true, FillFreeSpace = true });
        _gridly.Columns.Add(new GridlyColumn("Meslek", nameof(DemoRow.Occupation), 145));
        _gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 120) { Kind = GridlyColumnKind.Badge });
        _gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DemoRow.Progress), 150) { Kind = GridlyColumnKind.ProgressBar });
        _gridly.Columns.Add(new GridlyColumn("Puan", nameof(DemoRow.Rating), 120) { Kind = GridlyColumnKind.Rating, MaxRating = 5 });
        _gridly.Columns.Add(new GridlyColumn("İşlem", nameof(DemoRow.ActionText), 100) { Kind = GridlyColumnKind.Button });
        _gridly.Summaries.Add(new GridlySummaryItem { Column = _gridly.Columns[nameof(DemoRow.Id)]!, Type = GridlySummaryType.Count, Format = "Adet: {0}" });
        _gridly.Summaries.Add(new GridlySummaryItem { Column = _gridly.Columns[nameof(DemoRow.Progress)]!, Type = GridlySummaryType.Average, Format = "Ort: {0:0.0}" });
        _gridly.ConditionalFormats.Add(new GridlyConditionalFormat { Column = _gridly.Columns[nameof(DemoRow.State)], BackColor = Color.FromArgb(60, 220, 60, 60), ForeColor = Color.DarkRed, Predicate = (_,_,v) => string.Equals(Convert.ToString(v), "Fail", StringComparison.OrdinalIgnoreCase) });
    }
    ...
    private void ApplyAppTheme(GridlyTheme theme, string caption)
    {
        _currentTheme = EnsureReadable(theme);
        Program.AppTheme = _currentTheme;
        _gridly.ApplyTheme(_currentTheme);
        _gridly.SetRowDetailsProvider(CreateDetailsProvider());
        ...
        _themeInfo.Text = $"Tema: {caption}  |  DLL={GridlyVersionInfo.InformationalVersion}  |  Accent={ColorTranslator.ToHtml(_currentTheme.AccentColor)}  |  Fluent={_currentTheme.UseFluentBackdrop}  |  Acrylic={_currentTheme.UseAcrylicEffect}  |  Animasyonlu seçim aktif";
        ...
        GridlyWindowChrome.Apply(this, _currentTheme, true);
        _themePreview.Invalidate(true);
        Invalidate(true);
    }

    private static GridlyTheme CreateAoiTheme()
    {
        var t = GridlyTheme.FluentDarkTheme();
    ...
    private static GridlyTheme CreateHighContrastTheme() => new()
    {
    ...
    private static GridlyTheme EnsureReadable(GridlyTheme t)
    {
    ...
    private async Task LoadRowsAsync(int rowCount, string caption)
    {
        ...
        try
        {
            var rows = Enumerable.Range(1, rowCount).Select(i => new DemoRow
                {
                    ...
                    NeedsReview = i % 18 == 0,
                    ActionText = "Aç",
                    Tags = i % 3 == 0 ? "AOI,Fail,Review" : i % 2 == 0 ? "Gridly,Filter,Tile" : "Fast,Virtual,Theme",
            ...
            _gridly.ClearFilters();
            _search.Clear();
            if (rowCount <= 10_000)
                _gridly.SetObjects(rows);
            else
                await _gridly.SetObjectsAsync(_ => Task.FromResult<IEnumerable<DemoRow>>(rows));
            sw.Stop();
    ...
    private void LoadVirtualRows(int rowCount = 150_000_000)
    {
        ...
        _gridly.ClearFilters();
        _gridly.SetVirtualProvider(new VirtualDemoRowProvider(rowCount));
...
public sealed class DemoRow
{
    ...
    public string Description { get; set; } = "Gridly örnek açıklama metni";
    public string Notes { get; set; } = "Gridly örnek not metni";
    public DateTime Date { get; set; } = DateTime.Now;
    public int Progress { get; set; }
    public int Rating { get; set; }
    public string ActionText { get; set; } = "Aç";
    public string Tags { get; set; } = "AOI,Gridly";
...
public sealed class VirtualDemoRowProvider : IQueryRowProvider, IAsyncPreloadRowProvider, IProviderChangeNotifier, IBulkCheckStateProvider
{
    ...
    private GridlyFilterSet _filters = new();
    private GridlyColumn[] _columns = Array.Empty<GridlyColumn>();
    ...
    public void ApplyView(GridlyFilterSet filters, GridlyColumn[] columns, GridlyColumn? sortColumn, bool sortDescending)
    {
        _filters = filters ?? new GridlyFilterSet();
        _columns = columns ?? Array.Empty<GridlyColumn>();
    ...
    public bool TryGetDistinctValues(GridlyColumn column, GridlyFilterSet filters, GridlyColumn[] columns, int maxValues, string? searchText, out IReadOnlyList<string> values)
    {
    ...
    private IReadOnlyList<string> BuildVirtualNameDistinctValues(string? searchText, int maxValues)
    {
        ...
        // GridlyView to scan hundreds of millions of rows on the UI thread.
    ...
    private int MapViewIndexToRealIndex(int viewIndex)
    {
        ...
        // Sıralama da provider tarafında: GridlyView dev index listesi oluşturmaz.
    ...
    private static bool TryResolveExactIdentityFilter(GridlyColumnFilter f, out int realIndex)
    {
        realIndex = -1;
        ...
        if (f.Mode == GridlyFilterMode.ValueList && f.SelectedValues is { Count: 1 })
            return TryParseVirtualName(f.SelectedValues.First(), out realIndex);

        if ((f.Mode == GridlyFilterMode.Equals || f.Mode == GridlyFilterMode.Contains || f.Mode == GridlyFilterMode.StartsWith) &&
    ...
    private DemoRow CreateRowByRealIndex(int realIndex)
    {
        return new DemoRow
        {
            ...
            ActionText = "Git",
            Tags = realIndex % 3 == 0 ? "AOI,Fail,Review" : realIndex % 2 == 0 ? "Gridly,Filter,Tile" : "Fast,Virtual,Theme",
    ...
    public bool TrySetAllCheckStates(GridlyColumn column, CheckState state)
    {
    ...
    public bool TryGetCheckStateSummary(GridlyColumn column, out int checkedCount, out int uncheckedCount)
    {
        checkedCount = 0;
        uncheckedCount = 0;
        if (column == null) return false;
    ...
    private static bool TryGetNameExactIndex(GridlyFilterSet filters, out int index)
    {
        index = -1;
...
{
    private readonly GridlyTheme _theme;
    ...
    public SampleHubForm()
    {
        ...
        Text = "Gridly Example Center Pro - Tek Merkez";
        Width = 1280;
        Height = 760;
        MinimumSize = new Size(980, 620);
        StartPosition = FormStartPosition.CenterScreen;

        GridlyWindowChrome.ApplyOnHandleCreated(this, () => _theme, true);
        ApplyThemeToShell();
    ...
    {
        bool tr = GridlyLocalization.EffectiveLanguage == GridlyLanguage.Turkish;
        Text = tr ? "Gridly Örnek Merkezi Pro - Tek Merkez" : "Gridly Example Center Pro - Single Hub";
        _header.Text = tr ? "Gridly Örnek Merkezi Pro" : "Gridly Example Center Pro";
        _info.Text = tr
    ...
    private void BuildNavigation()
    {
        ...
        AddNav("Dil Değiştir", "TestApp açıkken Gridly yerleşik menüleri, dialogları ve yeni açılan örnek ekranları için dili değiştirir.", () =>
        {
            if (Program.ShowLanguageSelectionDialog(this))
            {
                ...
                bool tr = GridlyLocalization.EffectiveLanguage == GridlyLanguage.Turkish;
                MessageBox.Show(this,
                    tr ? "Dil değiştirildi. Yeni açılan Gridly örnekleri seçilen dili kullanacak." : "Language changed. Newly opened Gridly samples will use the selected language.",
                    tr ? "Dil" : "Language",
        ...
        AddNav("Gridly 5.1 Audix Pilot", "Audix gerçek kullanım pilotu: albüm kapağı, play/pause state, video preview, tema audit ve Example Center cleanup akışı.", () => new GridlyV51RealUsagePilotSampleForm());
        AddNav("Gridly v50.2 Hardening", "Build/API guard, runtime check, tema okunurluğu, Audix medya güvenli ayarları ve tak-çalıştır stabilizasyon ekranı.", () => new GridlyV502HardeningSampleForm());
        AddNav("Gridly 5.0 Foundation", "Stability, Theme Accessibility, Media Playback, modül profilleri ve hangi uygulamada hangi paket açılmalı sorusunu tek ekranda gösterir.", () => new GridlyV50FoundationSampleForm());
        AddNav("V41 Media Playback State", "Audix ve video arşivleri için play/pause, şimdi çalıyor rozeti, equalizer ve video preview davranışını dene.", () => new GridlyV41MediaPlaybackSampleForm());
        AddNav("V37-40 Pro Experience", "Enterprise Layout, Performance Pro, Interaction Pro ve Visual Analytics fazlarını tek ekranda dene.", () => new GridlyV37ToV40ProExperienceSampleForm());
        ...
        AddNav("V31 Faz Merkezi", "Faz 31-37: Media Experience, Smart Views, Grouping, MasterDetail, Kanban Pro, Designer ve Export tek ekranda.", () => new GridlyV31AllPhasesSampleForm());
        AddNav("Audix Albüm Kapakları", "Albüm kapağı, placeholder, FLAC/MP3 rozeti, hover play overlay ve Poster/Gallery/FilmStrip geçişleri.", () => new GridlyV31AllPhasesSampleForm());
        ...
        AddNav("Tüm Görünüm Modları", "GridlyViewMode listesindeki tüm yeni modları tek ekrandan dene.", () => new ViewModeShowcaseSampleForm());
        ...
        AddNav("GridlyView Mode", "Object / DataTable / Virtual / Tree / Tile modlarını tek ekranda test et", () => new GridlyModeVerificationForm());
        ...
        AddNav("V31 Faz Merkezi", "Faz 31-37 tamamı: Audix medya, kullanıcı preset, gelişmiş gruplama, master detail, kanban, designer ve export akışı.", () => new GridlyV31AllPhasesSampleForm());
        ...
        AddSection("Klasik Gridly Örnekleri");
        AddNav("Filtreleme", "Popup filtre, metin/değer listesi, temizleme", () => new FilteringSampleForm());
        AddNav("Gruplama detay", "Gridly/GLV tarzı grup davranışları", () => new GroupingSampleForm());
        ...
        AddNav("MasterData görünüm senaryoları", "SAP, BOM, ürün ağacı, program dosyası, makine seçimi ve log ekranları için hazır Gridly senaryoları", () => new MasterDataScenarioSampleForm());
        AddNav("Gridly v27 Product Core", "State Engine, renderer profilleri, range virtual provider ve MasterData hazır kullanım örneği", () => new GridlyV27ProductSampleForm());
        AddNav("Example Center Pro / Kategori Bulucu", "Tüm fazları kategori + arama ile bulabileceğin yeni Gridly 5.0 örnek merkezi", () => new ExampleCenterProForm());
        AddNav("v27.3 Renderer Showcase", "Badge, progress, icon+text, tag, button, hyperlink ve status renklerini tek tabloda gösterir", () => new GridlyV273RendererShowcaseForm());
        AddNav("v27.3.1 Filter Popup UX", "Uzun SAP malzeme adı ve program yolu filtrelerinde resize, tooltip, auto-width ve boyut hatırlama davranışını gösterir", () => new GridlyV2731FilterPopupUxForm());
        AddNav("v27.6 Design-Time Theme Sync", "Visual Studio designer açık tema önizlemesi, runtime temadan bağımsız design-time sync ve menü tema uyumu", () => new GridlyV276DesignTimeThemeSampleForm());
        ...
        AddNav("v27.9 Popular Enterprise Features", "Arama paneli, özet/footer, conditional format, column chooser, frozen column, advanced filter ve presetleri tek ekranda toplar", () => new GridlyV279PopularEnterpriseFeaturesForm());
        AddNav("v28 UX Polish", "Kart/poster/dashboard görünümlerinde global filtre butonunu üst bara taşır; best-practice UX presetlerini gösterir", () => new GridlyV28UxPolishForm());
        AddNav("v28.1 Poster Mode", "GridlyViewMode.Poster, otomatik poster ölçüleri, image scaling ve üst filtre UX davranışını gösterir", () => new GridlyV28UxPolishForm());
        AddNav("GridlyView Mode", "Object / DataTable / Virtual / Tree / Tile ana çalışma modları", () => new GridlyModeVerificationForm());
        ...
        AddNav("Kolon Designer Editor Smoke Test", "Son kolon editor düzeltmesini test eder: gerçek GridlyColumnCollectionEditor, Tamam/İptal, ekle/sil ve Audix property korunması", () => new GridlyColumnDesignerEditorSmokeForm());
        AddNav("Designer önizleme", "Design-time davranış vitrini", () => new DesignerPreviewSampleForm());
        AddNav("Designer + Menü Merge", "User ContextMenuStrip içinde Gridly liste özellikleri alt menüsü", () => new DesignerMenuMergeSampleForm());
        ...
        AddNav("Gridly/GLV uyumluluk / SetObjects", "SetObjects, AddObject, RemoveObject, seçim ve checked nesne uyumluluğu", () => new GridlyCompatSampleForm());
        AddNav("Gridly kolon özellikleri uyumluluk", "Gridly kolon propertyleri, header checkbox, button sizing ve hyperlink davranışı", () => new GridlyColumnCompatibilitySampleForm());
        ...
        AddNav("Gridly Designer Smart Tag", "Visual Studio Designer hızlı görev menüsü ve Gridly görevleri", () => new GridlyDesignerSmartTagSampleForm());
        AddNav("Klavye erişimi / Shortcut testi", "Tüm Gridly işlemlerine klavye ile erişim ve düzenleme kısayolları", () => new KeyboardAccessibilitySampleForm());
        AddNav("Menu Visibility Manager", "Örnek menüleri ve Gridly yerleşik menülerini göster/gizle", () => new MenuVisibilityManagerSampleForm());
        ...
        AddNav("Master-detail", "Üst kayıt seçilince alt Gridly detaylarını yükler", () => new MasterDetailSampleForm());
        ...
        AddNav("TreeView + TreeGrid", "TreeView master/detail ve hiyerarşik Gridly", () => new TreeViewSampleForm());
        ...
        AddNav("Toast Notification Demo", "GridlyToast info/success/warning/error bildirimi", () => new ToastNotificationSampleForm());
...
public abstract class GridlySampleFormBase : Form
{
    protected readonly Gridly.Core.GridlyView Gridly = new()
    {
        Dock = DockStyle.Fill,
        EmptyListMessage = "Kayıt yok",
        CellEditActivationKey = Keys.F2,
    ...
    protected GridlySampleFormBase(string title, string info)
    {
        Text = title;
        Width = 980;
        Height = 620;
        ...
        GridlyWindowChrome.ApplyOnHandleCreated(this, () => sampleTheme, true);
        ...
        Gridly.ApplyTheme(sampleTheme);
        MainForm.ConfigureMillionRowFiltering(Gridly);
        Info.Text = info;
        Controls.Add(Gridly);
        Controls.Add(Info);
        Controls.Add(Tool);
        ...
        Gridly.SetObjects(CreateRows(250));
    }

    protected void ApplySampleChrome(GridlyTheme theme)
    {
        ...
        Tool.Renderer = new Gridly.Theming.SmartMenuRenderer(theme);
    ...
    private static void ApplyToolItemTheme(ToolStripItem item, GridlyTheme theme)
    {
    ...
    private void AddSampleNavigation()
    {
        ...
        jump.DropDownItems.Add("V31 Faz Merkezi", null, (_,__) => new GridlyV31AllPhasesSampleForm().Show());
        ...
        jump.DropDownItems.Add("Gridly kolon özellikleri", null, (_,__) => new GridlyColumnCompatibilitySampleForm().Show());
        ...
        jump.DropDownItems.Add("Gridly Designer Smart Tag", null, (_,__) => new GridlyDesignerSmartTagSampleForm().Show());
        ...
        themeMenu.DropDownItems.Add("Açık tema", null, (_,__) => ApplySampleChrome(GridlyTheme.LightTheme()));
        themeMenu.DropDownItems.Add("Koyu tema", null, (_,__) => ApplySampleChrome(GridlyTheme.DarkTheme()));
        themeMenu.DropDownItems.Add("Fluent açık", null, (_,__) => ApplySampleChrome(GridlyTheme.FluentLightTheme()));
        themeMenu.DropDownItems.Add("Fluent koyu", null, (_,__) => ApplySampleChrome(GridlyTheme.FluentDarkTheme()));
    ...
    {
        Gridly.Columns.Add(new GridlyColumn("✓", nameof(DemoRow.Checked), 42) { Kind = GridlyColumnKind.CheckBox, AspectPutter = (row, value) => ((DemoRow)row).SetChecked(Convert.ToBoolean(value)) });
        Gridly.Columns.Add(new GridlyColumn("Id", nameof(DemoRow.Id), 70));
        Gridly.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 240) { Editable = true, FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Meslek", nameof(DemoRow.Occupation), 140));
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 110) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.Columns.Add(new GridlyColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = GridlyColumnKind.Rating, MaxRating = 5 });
    }
...
public sealed class MultiColumnCheckBoxSampleForm : GridlySampleFormBase
{
    ...
    {
        Gridly.Columns.Clear();
        Gridly.FullRowSelect = true;
        Gridly.MultiSelect = true;
        Gridly.ShowGridLines = true;
        Gridly.KeyboardSpaceTogglesCheckBoxes = true;

        Gridly.Columns.Add(new GridlyColumn("Person", nameof(DemoRow.Name), 190)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Operasyon", nameof(DemoRow.Occupation), 150)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Onay", nameof(DemoRow.State), 120)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Kilit", nameof(DemoRow.Tags), 180)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Action", nameof(DemoRow.ActionText), 110) { Kind = GridlyColumnKind.Button, ButtonText = "Aç" });
        Gridly.Columns.Add(new GridlyColumn("Link", nameof(DemoRow.Link), 120) { Kind = GridlyColumnKind.Hyperlink });
        Gridly.Columns.Add(new GridlyColumn("Notes", nameof(DemoRow.Tags), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });

        Gridly.ButtonClick += (_, e) => Info.Text = $"Action tıklandı: {((DemoRow)e.RowObject).Name}";
        Gridly.CellValueChanged += (_, e) =>
        {
        ...
        Gridly.HyperlinkClick += (_, e) => Info.Text = $"Link tıklandı: {((DemoRow)e.RowObject).Link}";
        ...
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Gridly.ShowColumnChooser()));

        var rows = CreateRows(80);
        for (int i = 0; i < rows.Count; i++)
        {
            rows[i].Name = i < 5 ? new[] { "Wilhelm Frat", "Alana Roderick", "Frank Price", "Eric", "Nicola Scotts" }[i] : "Person " + (i + 1);
            rows[i].Tags = "Gridly kolon checkbox uyumluluğu test satırı " + (i + 1);
            rows[i].Approved = i % 3 == 0;
            rows[i].Locked = i % 7 == 0;
        }
        Gridly.SetObjects(rows);
    ...
    {
        var c = Gridly.Columns[columnName];
        if (c == null) return;
...
public sealed class TileCardCheckBoxSampleForm : GridlySampleFormBase
{
    public TileCardCheckBoxSampleForm() : base("Kart / Geniş Kart Checkbox", "Tile, Kart, Geniş Kart ve Poster görünümünde kart üzerinde checkbox overlay desteği; header checkbox ile senkron, designer propertyleri ve konum ayarı test edilir.")
    {
        ...
        Gridly.FullRowSelect = true;
        Gridly.MultiSelect = true;
        Gridly.TileCheckBoxes = true;
        Gridly.TileCheckBoxAspectName = nameof(DemoRow.Checked);
        Gridly.TileCheckBoxPosition = GridlyTileCheckBoxPosition.TopRight;
        Gridly.TileCheckBoxSize = 20;
        Gridly.TileCheckBoxMargin = 10;
        Gridly.TilePreferredWidth = 280;
        Gridly.TilePreferredHeight = 112;
        Gridly.TileMaxTextLines = 4;
        Gridly.LargeCardPreferredWidth = 560;
        Gridly.LargeCardPreferredHeight = 176;
        Gridly.LargeCardMaxTextLines = 8;
        Gridly.TileShowAllVisibleTextColumns = true;
        Gridly.KeyboardSpaceTogglesCheckBoxes = true;

        Gridly.Columns.Add(new GridlyColumn("Seç", nameof(DemoRow.Checked), 70)
        {
            Kind = GridlyColumnKind.CheckBox,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 220) { Editable = true, FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 110) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Meslek", nameof(DemoRow.Occupation), 150));
        Gridly.Columns.Add(new GridlyColumn("Not", nameof(DemoRow.Tags), 330) { WordWrap = true, MaxTextLines = 3 });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DemoRow.Progress), 120) { Kind = GridlyColumnKind.ProgressBar });

        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => SetMode(GridlyViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Kart", null, (_,__) => SetMode(GridlyViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Geniş Kart", null, (_,__) => SetMode(GridlyViewMode.LargeCard)));
        Tool.Items.Add(new ToolStripButton("Poster", null, (_,__) =>
        {
            Gridly.TilePosterMode = !Gridly.TilePosterMode;
            Gridly.TilePosterImageHeight = 120;
            SetMode(GridlyViewMode.Tile);
            Info.Text = "Poster modu = " + Gridly.TilePosterMode;
        }));
        Tool.Items.Add(new ToolStripButton("Checkbox Aç/Kapat", null, (_,__) =>
        {
            Gridly.TileCheckBoxes = !Gridly.TileCheckBoxes;
            Info.Text = "TileCheckBoxes = " + Gridly.TileCheckBoxes;
        }));
        Tool.Items.Add(new ToolStripButton("Sol Üst", null, (_,__) => SetCheckBoxPosition(GridlyTileCheckBoxPosition.TopLeft)));
        Tool.Items.Add(new ToolStripButton("Sağ Üst", null, (_,__) => SetCheckBoxPosition(GridlyTileCheckBoxPosition.TopRight)));
        Tool.Items.Add(new ToolStripButton("Sol Alt", null, (_,__) => SetCheckBoxPosition(GridlyTileCheckBoxPosition.BottomLeft)));
        Tool.Items.Add(new ToolStripButton("Sağ Alt", null, (_,__) => SetCheckBoxPosition(GridlyTileCheckBoxPosition.BottomRight)));
        ...
        Gridly.SetObjects(rows);
        Gridly.SetViewMode(GridlyViewMode.Tile);
    ...
    private void SetMode(GridlyViewMode mode)
    {
        Gridly.SetViewMode(mode);
        Info.Text = "Görünüm = " + mode + " / TileCheckBoxes = " + Gridly.TileCheckBoxes + " / Konum = " + Gridly.TileCheckBoxPosition;
    }

    private void SetCheckBoxPosition(GridlyTileCheckBoxPosition position)
    {
        Gridly.TileCheckBoxPosition = position;
        Gridly.Invalidate();
        Info.Text = "TileCheckBoxPosition = " + Gridly.TileCheckBoxPosition + " / Margin = " + Gridly.TileCheckBoxMargin;
    }

    private void ChangeMargin(int delta)
    {
        Gridly.TileCheckBoxMargin = Math.Max(0, Math.Min(48, Gridly.TileCheckBoxMargin + delta));
        Gridly.Invalidate();
        Info.Text = "TileCheckBoxPosition = " + Gridly.TileCheckBoxPosition + " / Margin = " + Gridly.TileCheckBoxMargin;
    }
...
public sealed class SampleCenterSearchDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(80));
...
public sealed class ToastNotificationSampleForm : GridlySampleFormBase
{
    public ToastNotificationSampleForm() : base("Toast Notification Demo", "GridlyToast bildirimleri host formun sağ altında tema uyumlu şekilde gösterir.")
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(45));
        Tool.Items.Add(new ToolStripButton("Info", null, (_,__) => Gridly.ShowToast("Bilgilendirme bildirimi gösterildi.", GridlyToastKind.Info)));
        Tool.Items.Add(new ToolStripButton("Success", null, (_,__) => Gridly.ShowToast("İşlem başarıyla tamamlandı.", GridlyToastKind.Success)));
        Tool.Items.Add(new ToolStripButton("Warning", null, (_,__) => Gridly.ShowToast("Kontrol edilmesi gereken kayıtlar var.", GridlyToastKind.Warning)));
        Tool.Items.Add(new ToolStripButton("Error", null, (_,__) => Gridly.ShowToast("Örnek hata bildirimi.", GridlyToastKind.Error)));
    }
}

public sealed class SearchPanelDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.EnableModernSearchPanel = true;
        Gridly.SearchPanelCanFilterResults = false;
        Gridly.SetObjects(CreateRows(500));
        Tool.Items.Add(new ToolStripButton("Modern Arama Paneli", null, (_,__) => Gridly.ShowModernSearchPanel()));
        Tool.Items.Add(new ToolStripButton("Panel filtre modunu aç/kapat", null, (_,__) =>
        {
            Gridly.SearchPanelCanFilterResults = !Gridly.SearchPanelCanFilterResults;
            Info.Text = "SearchPanelCanFilterResults = " + Gridly.SearchPanelCanFilterResults;
        }));
    }
}

public sealed class AsyncFilterDemoForm : GridlySampleFormBase
{
    public AsyncFilterDemoForm() : base("Async Filter Demo", "Standart listelerde popup filtre değerleri daha hızlı açılsın diye async/full scan ayarları tek örnekte gösterilir.")
    {
        ConfigureCommonColumns();
        MainForm.ConfigureMillionRowFiltering(Gridly);
        Gridly.SetObjects(CreateRows(15000));
        Tool.Items.Add(new ToolStripButton("Ad filtresini aç", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Durum filtresini aç", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.State))));
...
public sealed class ThemePreviewDemoForm : GridlySampleFormBase
{
    public ThemePreviewDemoForm() : base("Theme Preview Demo", "Light, Dark ve Fluent temalar Gridly + host kontrollerinde birlikte test edilir.")
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(120));
        Tool.Items.Add(new ToolStripButton("Light", null, (_,__) => ApplySampleChrome(GridlyTheme.LightTheme())));
        Tool.Items.Add(new ToolStripButton("Dark", null, (_,__) => ApplySampleChrome(GridlyTheme.DarkTheme())));
        Tool.Items.Add(new ToolStripButton("Fluent Light", null, (_,__) => ApplySampleChrome(GridlyTheme.FluentLightTheme())));
        Tool.Items.Add(new ToolStripButton("Fluent Dark", null, (_,__) => ApplySampleChrome(GridlyTheme.FluentDarkTheme())));
    }
}

public sealed class DialogShowcaseSampleForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(180));
        Tool.Items.Add(new ToolStripButton("Filtre Penceresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon Seçici", null, (_,__) => Gridly.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Arama Paneli", null, (_,__) => Gridly.ShowModernSearchPanel()));
        Tool.Items.Add(new ToolStripButton("Toast", null, (_,__) => Gridly.ShowToast("Dialog standardı örneği", GridlyToastKind.Info)));
    }
}

public sealed class WindowChromeDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(60));
    ...
    private void ShowChromeDialog(bool sizable)
    {
        ...
        {
            Text = sizable ? "Gridly sizeable themed dialog" : "Gridly fixed themed dialog",
            StartPosition = FormStartPosition.CenterParent,
            Size = new Size(520, 340),
            MinimumSize = new Size(420, 260),
            Font = Font
        };
        GridlyDialogChrome.ConfigureStandardDialog(form, Program.AppTheme, new Size(520, 340), sizable);
        ...
        GridlyDialogThemeApplier.Apply(form, Program.AppTheme);
        form.ShowDialog(this);
    }
}

public sealed class TileAdvancedDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(240));
        Gridly.SetViewMode(GridlyViewMode.Tile);
        Tool.Items.Add(new ToolStripButton("Tile", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Details", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Large Icons", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.LargeIcons)));
    }
}

public sealed class VirtualizationStressDemoForm : GridlySampleFormBase
{
    public VirtualizationStressDemoForm() : base("Virtualization Stress Test", "1M+ satır senaryosunda provider tabanlı liste, hızlı filtre ve scroll davranışı test edilir.")
    {
        ConfigureCommonColumns();
        MainForm.ConfigureMillionRowFiltering(Gridly);
        Gridly.SetVirtualProvider(new VirtualDemoRowProvider(1_500_000));
        Tool.Items.Add(new ToolStripButton("Ad filtresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Durum filtresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("999M provider", null, (_,__) => Gridly.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000))));
    }
}

public sealed class LayoutProfilesDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.SetObjects(CreateRows(100));
        Tool.Items.Add(new ToolStripButton("Layout Kaydet", null, (_,__) => Gridly.SaveLayout(Path.Combine(Application.StartupPath, "gridly-sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout Yükle", null, (_,__) => Gridly.LoadLayout(Path.Combine(Application.StartupPath, "gridly-sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Kolon Seçici", null, (_,__) => Gridly.ShowColumnChooser()));
    }
}

public sealed class KeyboardNavigationDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.CellEditActivationKey = Keys.F2;
        Gridly.AllowEditAllCells = true;
        Gridly.SetObjects(CreateRows(70));
        ...
        Tool.Items.Add(new ToolStripButton("Ctrl+F Panel", null, (_,__) => Gridly.ShowModernSearchPanel()));
    }
}

public sealed class ExcelFilterDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.FilterMenuMode = GridlyFilterMenuMode.Both;
        Gridly.SetObjects(CreateRows(320));
        Tool.Items.Add(new ToolStripButton("Ad filtre", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Meslek filtre", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => Gridly.ClearFilters()));
    }
}

public sealed class DesignerPreviewHubDemoForm : GridlySampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Gridly.DesignTimeSampleData = false;
        Gridly.SetObjects(CreateRows(36));
        Tool.Items.Add(new ToolStripButton("Kolon düzenleyici", null, (_,__) => Gridly.ShowColumnChooser()));
...
public sealed class ProgressBarSampleForm : GridlySampleFormBase
{
    public ProgressBarSampleForm() : base("Gridly Modern ProgressBar Örneği", "ProgressBar animasyonu varsayılan kapalıdır. Görünüm tema, satır rengi ve accent renkle uyumludur; istenirse kod/designer ile açılır.")
    {
        Gridly.ProgressBarAnimated = false;
        Gridly.ProgressBarUseGradient = true;
        Gridly.ProgressBarShowText = true;
        Gridly.ProgressBarUseAccentColor = true;
        Gridly.RowColorPreset = GridlyRowColorPreset.StatusPills;
        Gridly.RowColorAspectName = nameof(DemoRow.State);
        Gridly.RowColorStrength = 0.16;
        Tool.Items.Add(new ToolStripButton("Animasyon Aç/Kapat", null, (_,__) => { Gridly.ProgressBarAnimated = !Gridly.ProgressBarAnimated; Gridly.Invalidate(); Info.Text = "ProgressBarAnimated = " + Gridly.ProgressBarAnimated + "  (büyük listelerde kapalı önerilir)"; }));
        Tool.Items.Add(new ToolStripButton("Accent renk", null, (_,__) => { Gridly.ProgressBarUseAccentColor = true; Gridly.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Düşük/orta/yüksek renk", null, (_,__) => { Gridly.ProgressBarUseAccentColor = false; Gridly.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Gradient Aç/Kapat", null, (_,__) => { Gridly.ProgressBarUseGradient = !Gridly.ProgressBarUseGradient; Gridly.Invalidate(); }));
    }
}

public sealed class PrintPreviewSampleForm : GridlySampleFormBase
{
    public PrintPreviewSampleForm() : base("Gridly Yazdırma / Önizleme Örneği", "Gridly/GLV tarzı önizleme desteği: tüm görünür satırlar, seçili satırlar, PageSetup, PrintPreview ve Print.")
    {
        Gridly.PrintTitle = "GridlyView Yazdırma Önizleme Örneği";
        Gridly.PrintMaxRows = 5000;
        Tool.Items.Add(new ToolStripButton("Önizleme", null, (_,__) => Gridly.ShowPrintPreview("GridlyView Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Yazdır", null, (_,__) => Gridly.Print("GridlyView Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Sayfa Ayarı", null, (_,__) => Gridly.ShowPageSetup("GridlyView Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Sadece Seçili Satırlar", null, (_,__) => { Gridly.PrintSelectedRowsOnly = !Gridly.PrintSelectedRowsOnly; Info.Text = "PrintSelectedRowsOnly = " + Gridly.PrintSelectedRowsOnly + " | Çoklu seçim yapıp önizleme açabilirsin."; }));
        Tool.Items.Add(new ToolStripButton("Tema Rengi Aç/Kapat", null, (_,__) => { Gridly.PrintUseThemeColors = !Gridly.PrintUseThemeColors; Info.Text = "PrintUseThemeColors = " + Gridly.PrintUseThemeColors + " | Profesyonel çıktı için kapalı önerilir."; }));
        Tool.Items.Add(new ToolStripButton("Fit to page width", null, (_,__) => { Gridly.PrintFitToPageWidth = !Gridly.PrintFitToPageWidth; Info.Text = "PrintFitToPageWidth = " + Gridly.PrintFitToPageWidth; }));
        Tool.Items.Add(new ToolStripButton("Grid Aç/Kapat", null, (_,__) => { Gridly.PrintShowGrid = !Gridly.PrintShowGrid; Info.Text = "PrintShowGrid = " + Gridly.PrintShowGrid; }));
        Tool.Items.Add(new ToolStripButton("Zebra Aç/Kapat", null, (_,__) => { Gridly.PrintZebraRows = !Gridly.PrintZebraRows; Info.Text = "PrintZebraRows = " + Gridly.PrintZebraRows; }));
    }
}

public sealed class FilteringSampleForm : GridlySampleFormBase
{
    public FilteringSampleForm() : base("Gridly Filtreleme Detay Örneği", "Popup filtre, ayrı filtre penceresi, hızlı metin filtreleme ve tüm filtreleri temizleme örneği.")
    {
        Gridly.FilterMenuMode = GridlyFilterMenuMode.Both;
        var search = new ToolStripTextBox { Width = 180 };
        search.TextChanged += (_,__) => Gridly.SetGlobalFilter(search.Text ?? "");
        Tool.Items.Add(new ToolStripLabel("Ara:"));
        Tool.Items.Add(search);
        Tool.Items.Add(new ToolStripButton("Ad popup filtre", null, (_,__) => Gridly.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Popup stil", null, (_,__) => Gridly.FilterMenuMode = GridlyFilterMenuMode.PopupMenu));
        Tool.Items.Add(new ToolStripButton("İkisi birlikte", null, (_,__) => Gridly.FilterMenuMode = GridlyFilterMenuMode.Both));
        Tool.Items.Add(new ToolStripButton("Ad filtre penceresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Filtreleri temizle", null, (_,__) => { search.Text = ""; Gridly.ClearFilters(); }));
    }
}

public sealed class GroupingSampleForm : GridlySampleFormBase
{
    public GroupingSampleForm() : base("Gridly Gruplama Detay Örneği", "Gridly/GLV tarzı gruplama: başlığa tıkla aç/kapat, tüm grupları toplu daralt/aç.")
    {
        Tool.Items.Add(new ToolStripButton("Duruma göre grupla", null, (_,__) => Gridly.SetGroupBy(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Mesleğe göre grupla", null, (_,__) => Gridly.SetGroupBy(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Tüm grupları daralt", null, (_,__) => Gridly.CollapseAllGroups()));
        Tool.Items.Add(new ToolStripButton("Tüm grupları aç", null, (_,__) => Gridly.ExpandAllGroups()));
        Tool.Items.Add(new ToolStripButton("Gruplamayı kaldır", null, (_,__) => Gridly.ClearGrouping()));
        Tool.Items.Add(new ToolStripButton("Grup başlığı rengi", null, (_,__) => { Gridly.CustomGroupBackColor = Color.FromArgb(235, 229, 250); Gridly.Invalidate(); }));
        Gridly.SetGroupBy(nameof(DemoRow.State));
    }
}

public sealed class ViewModesSampleForm : GridlySampleFormBase
{
    public ViewModesSampleForm() : base("Gridly Görünüm Modları Detay Örneği", "Kompakt, Standart, Geniş, Liste, Kart, Geniş Kart ve Detay görünümleri aynı veriyle denenebilir.")
    {
        AddMode("Kompakt", GridlyViewMode.ExtraLargeIcons);
        AddMode("Standart", GridlyViewMode.LargeIcons);
        AddMode("Geniş", GridlyViewMode.MediumIcons);
        AddMode("Liste", GridlyViewMode.List);
        AddMode("Kart", GridlyViewMode.Tile);
        AddMode("Geniş Kart", GridlyViewMode.LargeCard);
        AddMode("Detay", GridlyViewMode.Details);
    }
    private void AddMode(string text, GridlyViewMode mode) => Tool.Items.Add(new ToolStripButton(text, null, (_,__) => Gridly.SetViewMode(mode)));
}

public sealed class TileSampleForm : GridlySampleFormBase
{
    public TileSampleForm() : base("Gridly Kart Detay Örneği", "Gridly tile görünümüne benzer kart dizilim örneği. Kartlarda ad, meslek, tarih/durum gibi alanlar kompakt gösterilir.")
    {
        Gridly.TilePreferredHeight = 96;
        Gridly.TileMaxTextLines = 4;
        Gridly.SetViewMode(GridlyViewMode.Tile);
        Tool.Items.Add(new ToolStripButton("Kart görünümü", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Detay görünümü", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Mesleğe göre grupla", null, (_,__) => Gridly.SetGroupBy(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Kart yüksekliği +", null, (_,__) => { Gridly.TilePreferredHeight += 12; Gridly.SetViewMode(GridlyViewMode.Tile); }));
        Tool.Items.Add(new ToolStripButton("Kart yüksekliği -", null, (_,__) => { Gridly.TilePreferredHeight = Math.Max(72, Gridly.TilePreferredHeight - 12); Gridly.SetViewMode(GridlyViewMode.Tile); }));
        Tool.Items.Add(new ToolStripButton("Satır +", null, (_,__) => { Gridly.TileMaxTextLines = Math.Min(12, Gridly.TileMaxTextLines + 1); Gridly.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Satır -", null, (_,__) => { Gridly.TileMaxTextLines = Math.Max(1, Gridly.TileMaxTextLines - 1); Gridly.Invalidate(); }));
    }
}

public sealed class PosterGallerySampleForm : GridlySampleFormBase
{
    ...
    public PosterGallerySampleForm() : base("Gridly Poster / Resimli İçerik Örneği", "Plex/Netflix tarzı katalog: büyük görsel, başlık, yıl, tür, rating ve responsive poster kartları.")
    {
        _posters = CreatePosterImages();

        Gridly.Columns.Add(new GridlyColumn("Poster", nameof(DemoRow.PosterIndex), 92)
        {
            Kind = GridlyColumnKind.Image,
            ImageGetter = row => _posters[((DemoRow)row).PosterIndex % _posters.Length]
        });
        Gridly.Columns.Add(new GridlyColumn("Başlık", nameof(DemoRow.Name), 230) { FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Tür", nameof(DemoRow.Occupation), 115) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Yıl", nameof(DemoRow.Year), 80));
        Gridly.Columns.Add(new GridlyColumn("Rating", nameof(DemoRow.Rating), 100) { Kind = GridlyColumnKind.Rating });
        Gridly.Columns.Add(new GridlyColumn("İzleme", nameof(DemoRow.Progress), 110) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.SetViewMode(GridlyViewMode.Poster);
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 214;
        Gridly.TilePreferredHeight = 292;
        Gridly.TilePosterImageHeight = 168;
        Gridly.EnableModernEmptyState = true;
        Gridly.EmptyListMessage = "Bu filtreye uygun poster bulunamadı";
        Gridly.AutoSizeTileWidthToContent = false;
        Gridly.MediaImageScaleMode = GridlyMediaImageScaleMode.Cover;

        _rows = CreatePosterRows();
        Gridly.SetObjects(_rows);
        ...
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(new ToolStripButton("Poster", null, (_,__) => { Gridly.TilePosterMode = true; Gridly.SetViewMode(GridlyViewMode.Poster); }));
        Tool.Items.Add(new ToolStripButton("MediaTile", null, (_,__) => { Gridly.TilePosterMode = true; Gridly.SetViewMode(GridlyViewMode.MediaTile); }));
        Tool.Items.Add(new ToolStripButton("FilmStrip", null, (_,__) => { Gridly.TilePosterMode = true; Gridly.SetViewMode(GridlyViewMode.FilmStrip); }));
        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.Details); }));
        Tool.Items.Add(new ToolStripButton("Görsel +", null, (_,__) => { Gridly.TilePosterImageHeight += 12; Gridly.TilePreferredHeight += 12; Gridly.RefreshView(); }));
        Tool.Items.Add(new ToolStripButton("Görsel -", null, (_,__) => { Gridly.TilePosterImageHeight = Math.Max(90, Gridly.TilePosterImageHeight - 12); Gridly.TilePreferredHeight = Math.Max(190, Gridly.TilePreferredHeight - 12); Gridly.RefreshView(); }));
        Tool.Items.Add(new ToolStripButton("Yıla göre grupla", null, (_,__) => Gridly.SetGroupBy(nameof(DemoRow.Year))));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_,__) => Gridly.ClearGrouping()));
        Tool.Items.Add(new ToolStripButton("Filtre menüsü", null, (_,__) => Gridly.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripSeparator());
    ...
    private static Bitmap CreatePosterBitmap(Color baseColor, int number)
    {
        ...
        TextRenderer.DrawText(g, "GRIDLY POSTER", small, new Rectangle(0, 248, bmp.Width, 26), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class MediaLibrarySampleForm : GridlySampleFormBase
{
    ...
    public MediaLibrarySampleForm() : base("Gridly Media Library Showcase", "Audix, film arşivi, fotoğraf galerisi, makine kataloğu ve AOI hata görseli gibi senaryolarda aynı Gridly verisini Poster / MediaTile / FilmStrip / DetailCard olarak gösterir.")
    {
        _covers = CreateMediaImages();
        _items = CreateMediaItems();

        Gridly.Columns.Add(new GridlyColumn("Kapak", nameof(MediaItem.ImageIndex), 120)
        {
            Kind = GridlyColumnKind.Image,
            ImageGetter = row => _covers[((MediaItem)row).ImageIndex % _covers.Length]
        });
        Gridly.Columns.Add(new GridlyColumn("Başlık", nameof(MediaItem.Title), 240) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        Gridly.Columns.Add(new GridlyColumn("Alt Bilgi", nameof(MediaItem.Subtitle), 210) { FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Tür", nameof(MediaItem.MediaType), 95) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Albüm / Koleksiyon", nameof(MediaItem.Collection), 170));
        Gridly.Columns.Add(new GridlyColumn("Süre", nameof(MediaItem.Duration), 70));
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(MediaItem.State), 90) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Açıklama", nameof(MediaItem.Description), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });

        Gridly.SetObjects(_items);
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 218;
        Gridly.TilePreferredHeight = 292;
        Gridly.TilePosterImageHeight = 166;
        Gridly.TileMaxTextLines = 5;
        Gridly.PosterPreferredWidth = 220;
        Gridly.PosterPreferredHeight = 306;
        Gridly.PosterImageHeight = 182;
        Gridly.MediaImageScaleMode = GridlyMediaImageScaleMode.Cover;
        Gridly.MediaImageRoundedCorners = true;
        Gridly.EnableModernEmptyState = true;
        Gridly.EmptyListMessage = "Bu filtreye uygun medya kaydı yok";
        Gridly.SetViewMode(GridlyViewMode.Poster);
        ...
        _scale.Items.Add(GridlyMediaImageScaleMode.Cover);
        _scale.Items.Add(GridlyMediaImageScaleMode.Contain);
        _scale.Items.Add(GridlyMediaImageScaleMode.Stretch);
        _scale.SelectedItem = GridlyMediaImageScaleMode.Cover;

        _search.TextChanged += (_, __) => ApplyFilter();
        _type.SelectedIndexChanged += (_, __) => ApplyFilter();
        _scale.SelectedIndexChanged += (_, __) =>
        {
            if (_scale.SelectedItem is GridlyMediaImageScaleMode mode)
            {
                Gridly.MediaImageScaleMode = mode;
                Gridly.RefreshView();
            }
        };
        ...
        Tool.Items.Add(new ToolStripButton("KPI", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.TilePreferredWidth = 240; Gridly.TilePreferredHeight = 140; Gridly.SetViewMode(GridlyViewMode.KpiDashboard); }));
        Tool.Items.Add(new ToolStripButton("HeatMap", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.TilePreferredWidth = 190; Gridly.TilePreferredHeight = 126; Gridly.SetViewMode(GridlyViewMode.HeatMap); }));
        Tool.Items.Add(new ToolStripButton("MiniChart", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.TilePreferredWidth = 320; Gridly.TilePreferredHeight = 118; Gridly.SetViewMode(GridlyViewMode.MiniChart); }));
        Tool.Items.Add(new ToolStripButton("Property", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.PropertyCard); }));
        Tool.Items.Add(new ToolStripButton("DetailCard", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.DetailCard); }));
        Tool.Items.Add(new ToolStripButton("Detay Liste", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.Details); }));
        Tool.Items.Add(new ToolStripButton("Koleksiyona göre grupla", null, (_, __) => Gridly.SetGroupBy(nameof(MediaItem.MediaType))));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_, __) => Gridly.ClearGrouping()));
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(_status);
    ...
    {
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 210;
        Gridly.TilePreferredHeight = 250;
        Gridly.TilePosterImageHeight = 148;
        Gridly.TileMaxTextLines = 5;
    ...
    {
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 190;
        Gridly.TilePreferredHeight = 236;
        Gridly.TilePosterImageHeight = 128;
        Gridly.TileMaxTextLines = 4;
    ...
    {
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 920;
        Gridly.TilePreferredHeight = 164;
        Gridly.TilePosterImageHeight = 116;
        Gridly.TileMaxTextLines = 7;
    ...
    private static Bitmap CreateMediaBitmap(Color baseColor, string label, int seed)
    {
        ...
        TextRenderer.DrawText(g, "GRIDLY MEDIA", small, new Rectangle(0, 392, bmp.Width, 30), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class GridlyV31AllPhasesSampleForm : GridlySampleFormBase
{
    ...
    public GridlyV31AllPhasesSampleForm() : base("Gridly v31 Faz Merkezi", "Faz 31-37 tek ekranda: Audix medya deneyimi, smart view presetleri, gelişmiş grouping, master-detail, kanban, designer ve export akışı.")
    {
        _images = CreatePhaseImages();
        _items = CreateFeatureItems();

        Gridly.Columns.Clear();
        Gridly.Columns.Add(new GridlyColumn("Görsel", nameof(V31FeatureItem.ImageIndex), 96)
        {
            Kind = GridlyColumnKind.Image,
            ImageGetter = row => _images[((V31FeatureItem)row).ImageIndex % _images.Length]
        });
        Gridly.Columns.Add(new GridlyColumn("Özellik", nameof(V31FeatureItem.Title), 220) { Editable = true, FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        Gridly.Columns.Add(new GridlyColumn("Faz", nameof(V31FeatureItem.Phase), 90) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Proje", nameof(V31FeatureItem.Project), 120) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Gridly Modu", nameof(V31FeatureItem.ViewMode), 140));
        Gridly.Columns.Add(new GridlyColumn("Kullanım", nameof(V31FeatureItem.Usage), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
        Gridly.Columns.Add(new GridlyColumn("Rozet", nameof(V31FeatureItem.Badge), 85) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Konum", nameof(V31FeatureItem.WhereToFind), 260) { WordWrap = true, MaxTextLines = 3 });

        Gridly.SetObjects(_items);
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = 245;
        Gridly.TilePreferredHeight = 314;
        Gridly.TilePosterImageHeight = 150;
        Gridly.TileMaxTextLines = 6;
        Gridly.MediaImageScaleMode = GridlyMediaImageScaleMode.Cover;
        Gridly.MediaImageRoundedCorners = true;
        Gridly.MediaPlaceholderImage = _images[0];
        Gridly.ShowMediaOverlayButton = true;
        Gridly.MediaOverlayButtonText = "▶";
        Gridly.MediaQualityBadgeAspectName = nameof(V31FeatureItem.Badge);
        Gridly.MediaQualityBadgeGetter = row => ((V31FeatureItem)row).Badge;
        Gridly.EnableModernEmptyState = true;
        Gridly.EmptyListMessage = "Bu arama/filtre için özellik bulunamadı";
        Gridly.SetViewMode(GridlyViewMode.Poster);
        ...
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(new ToolStripButton("Audix Poster", null, (_, __) => ApplyMedia(GridlyViewMode.Poster, 245, 314, 150)));
        Tool.Items.Add(new ToolStripButton("Gallery", null, (_, __) => ApplyMedia(GridlyViewMode.Gallery, 218, 265, 132)));
        Tool.Items.Add(new ToolStripButton("MediaTile", null, (_, __) => ApplyMedia(GridlyViewMode.MediaTile, 198, 238, 112)));
        Tool.Items.Add(new ToolStripButton("FilmStrip", null, (_, __) => ApplyMedia(GridlyViewMode.FilmStrip, 900, 168, 110)));
        Tool.Items.Add(new ToolStripButton("Feature Finder", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.RowPreview); }));
        Tool.Items.Add(new ToolStripButton("Kanban Pro", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.TilePreferredWidth = 330; Gridly.TilePreferredHeight = 188; Gridly.SetViewMode(GridlyViewMode.Kanban); Gridly.SetGroupBy(nameof(V31FeatureItem.Project)); }));
        Tool.Items.Add(new ToolStripButton("MasterDetail", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.MasterDetail); }));
        Tool.Items.Add(new ToolStripButton("KPI", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.TilePreferredWidth = 240; Gridly.TilePreferredHeight = 140; Gridly.SetViewMode(GridlyViewMode.KpiDashboard); }));
        Tool.Items.Add(new ToolStripButton("Export/Print", null, (_, __) => { Gridly.TilePosterMode = false; Gridly.SetViewMode(GridlyViewMode.DetailCard); }));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_, __) => Gridly.ClearGrouping()));
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(_status);
        UpdateStatus(_items.Count);
    ...
    private void ApplyMedia(GridlyViewMode mode, int width, int height, int imageHeight)
    {
        Gridly.ClearGrouping();
        Gridly.TilePosterMode = true;
        Gridly.TilePreferredWidth = width;
        Gridly.TilePreferredHeight = height;
        Gridly.TilePosterImageHeight = imageHeight;
        Gridly.PosterPreferredWidth = width;
        Gridly.PosterPreferredHeight = height;
        Gridly.PosterImageHeight = imageHeight;
        Gridly.SetViewMode(mode);
    }
    ...
    private static List<V31FeatureItem> CreateFeatureItems()
    {
        ...
        {
            new("31", "Audix", "Albüm kapağı lazy-load + disk cache", "Poster / Gallery / MediaTile", "ImageGetter içinde dosyadan kapak oku, disk cache ile sakla, Gridly MediaPlaceholderImage ile eksik kapakları düzgün göster.", "Example Center > V31 Faz Merkezi / Media Library", "CACHE", 0),
            ...
            new("31", "Audix", "FLAC / MP3 / 320kbps rozeti", "Poster / Gallery", "MediaQualityBadgeAspectName veya MediaQualityBadgeGetter ile kalite rozeti kapak üzerinde gösterilir.", "Gridly - Media Experience property grubu", "FLAC", 2),
            new("31", "Genel", "Eksik görsel placeholder", "Poster / MediaTile", "MediaPlaceholderImage ile albüm kapağı, öğrenci fotoğrafı, makine görseli veya AOI karesi yoksa boş alan yerine kurumsal placeholder gösterilir.", "Gridly - Media Experience > MediaPlaceholderImage", "SAFE", 3),
            ...
            new("32", "Genel", "Layout export/import", "Details", "Gridly layout profilini dışarı aktarma ve başka kullanıcıya/PC’ye taşıma senaryosu.", "Example Center > Layout Profiles Demo", "JSON", 6),
            ...
            new("35", "LineWorkspace", "Duruma göre kart taşıma hazırlığı", "Kanban", "Sürükle-bırak iş akışına hazır görünüm; şimdilik grup + kart mantığıyla pratik takip sağlar.", "GridlyViewMode.Kanban", "FLOW", 12),

            new("36", "Genel", "Designer friendly property grupları", "Tüm modlar", "Medya ayarları Gridly - Media Experience kategorisinde görünür; WinForms Designer’da ayarlanabilir property yaklaşımı korundu.", "PropertyGrid > Gridly - Media Experience", "DESIGN", 13),
    ...
    private static Bitmap CreatePhaseBitmap(Color baseColor, string label, int seed)
    {
        ...
        using var small = new Font("Segoe UI", 11, FontStyle.Bold);
        TextRenderer.DrawText(g, "GRIDLY V31", small, new Rectangle(0, 392, bmp.Width, 28), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class HighlightSearchSampleForm : GridlySampleFormBase
{
    private readonly ToolStripTextBox _search = new() { Width = 180 };
    public HighlightSearchSampleForm() : base("Gridly Highlight Search Detay Örneği", "Arama metni satır içinde sarı highlight ile gösterilir, sonraki/önceki eşleşmeye atlanabilir.")
    {
        _search.Text = "kayıt 25";
        _search.TextChanged += (_,__) => { Gridly.SetGlobalFilter(_search.Text ?? ""); Gridly.JumpToFirstMatch(_search.Text ?? ""); };
        Tool.Items.Add(new ToolStripLabel("Highlight ara:"));
        Tool.Items.Add(_search);
        Tool.Items.Add(new ToolStripButton("Sonraki", null, (_,__) => Gridly.FindNext(_search.Text ?? "")));
        Tool.Items.Add(new ToolStripButton("Önceki", null, (_,__) => Gridly.FindPrevious(_search.Text ?? "")));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => { _search.Text = ""; Gridly.ClearFilters(); }));
        Shown += (_,__) => { Gridly.SetGlobalFilter(_search.Text ?? ""); Gridly.JumpToFirstMatch(_search.Text ?? ""); };
    }
}

public sealed class ColoringSampleForm : GridlySampleFormBase
{
    ...
    public ColoringSampleForm() : base("Gridly Satır Renklendirme Detay Örneği", "Satır renkleri seçilen temaya göre otomatik uyarlanır. Fail/Review/OK gibi durumlar açık ve koyu temada okunabilir kalır.")
    {
        foreach (GridlyThemePreset preset in Enum.GetValues(typeof(GridlyThemePreset)))
            _themeBox.Items.Add(preset);
        foreach (GridlyRowColorPreset preset in Enum.GetValues(typeof(GridlyRowColorPreset)))
            _rowColorBox.Items.Add(preset);

        _themeBox.SelectedItem = GridlyThemePreset.MidnightPurple;
        _rowColorBox.SelectedItem = GridlyRowColorPreset.AOIRisk;
        ...
        {
            Gridly.RowForeColorGetter = row => ((DemoRow)row).State == "Fail" ? (GridlyTheme.FromPreset((GridlyThemePreset)_themeBox.SelectedItem!).IsDark ? Color.FromArgb(255, 190, 190) : Color.FromArgb(140, 0, 0)) : null;
            Gridly.Invalidate();
        }));
        Tool.Items.Add(new ToolStripButton("Koşullu format ekle", null, (_,__) =>
        {
            Gridly.ConditionalFormats.Clear();
            Gridly.ConditionalFormats.Add(new GridlyConditionalFormat { Column = Gridly.Columns[nameof(DemoRow.State)], BackColor = Color.FromArgb(80, 220, 60, 60), ForeColor = Color.DarkRed, Predicate = (_,_,v) => Convert.ToString(v) == "Fail" });
            Gridly.Invalidate();
        }));
        Tool.Items.Add(new ToolStripButton("Sıfırla", null, (_,__) =>
        {
            Gridly.RowBackColorGetter = null;
            Gridly.RowForeColorGetter = null;
            Gridly.ConditionalFormats.Clear();
            _rowColorBox.SelectedItem = GridlyRowColorPreset.ThemeDefault;
            ApplyPreview();
        }));
    ...
    {
        var themePreset = _themeBox.SelectedItem is GridlyThemePreset tp ? tp : GridlyThemePreset.System;
        var rowPreset = _rowColorBox.SelectedItem is GridlyRowColorPreset rp ? rp : GridlyRowColorPreset.ThemeDefault;
        Gridly.ThemePreset = themePreset;
        Gridly.RowColorPreset = rowPreset;
        Gridly.RowColorAspectName = nameof(DemoRow.State);
        Gridly.RowColorStrength = rowPreset == GridlyRowColorPreset.AOIRisk || rowPreset == GridlyRowColorPreset.SeverityBands ? 0.23 : 0.16;
        Gridly.RowBackColorGetter = null;
        Gridly.RowForeColorGetter = null;
        Gridly.Invalidate();
        Info.Text = $"Tema: {themePreset} | Satır renkleri: {rowPreset} | Aspect: {Gridly.RowColorAspectName}";
    }
}

public sealed class ImageComboSampleForm : GridlySampleFormBase
{
    private static readonly string[] StatusValues = { "OK", "Review", "Fail" };

    public ImageComboSampleForm() : base("Gridly Image + Combo Hücre Örneği", "İkon/görsel kolon + ikonlu ComboBox hücre düzenleme. Combo hücreye tek tıklayınca liste direkt açılır; F2 de desteklenir.")
    {
        Gridly.Columns.Clear();
        Gridly.RowHeight = 34;
        Gridly.CellEditActivationKey = Keys.F2;
        Gridly.AllowEditAllCells = true;
        Gridly.EnableCellEditing = true;

        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 76)
        {
            Kind = GridlyColumnKind.Icon,
        ...
        Gridly.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 240) { Editable = true, FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Kategori", nameof(DemoRow.Occupation), 170)
        {
            Kind = GridlyColumnKind.ComboBox,
            Editable = true,
            ComboBoxItems = DemoData.Occupations.ToList(),
            ComboBoxImageGetter = CreateOccupationIcon,
            Editor = new Gridly.Editing.ComboBoxCellEditor(DemoData.Occupations)
        });
        Gridly.Columns.Add(new GridlyColumn("Karar", nameof(DemoRow.State), 130)
        {
            Kind = GridlyColumnKind.ComboBox,
            Editable = true,
            ComboBoxItems = StatusValues.ToList(),
            ComboBoxImageGetter = CreateStatusIcon,
            Editor = new Gridly.Editing.ComboBoxCellEditor(StatusValues)
        });
        Gridly.Columns.Add(new GridlyColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = GridlyColumnKind.Rating, MaxRating = 5 });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = GridlyColumnKind.ProgressBar });

        Tool.Items.Add(new ToolStripButton("Karar filtresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Kategori filtresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Occupation))));
...
public sealed class ExportSampleForm : GridlySampleFormBase
{
    public ExportSampleForm() : base("Gridly Export Detay Örneği", "Görünen satırları CSV veya Excel .xlsx olarak dışarı aktarır.")
    {
    ...
    {
        var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.Sample.csv");
        try
        {
            var actualPath = Gridly.ExportVisibleCsv(path);
    ...
    {
        var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.Sample.xlsx");
        try
        {
            var actualPath = Gridly.ExportVisibleExcel(path, "GridlyViewSample");
...
public sealed class MillionRowsSampleForm : GridlySampleFormBase
{
    public MillionRowsSampleForm() : base("Gridly 1M Sanal Liste Detay Örneği", "999.000.000 satır virtual/query provider ile simüle edilir; büyük veri RAM'e alınmaz, filtre/sort provider tarafında çalışır.")
    {
        Tool.Items.Add(new ToolStripButton("999M Virtual yükle", null, (_,__) => Gridly.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000))));
        Tool.Items.Add(new ToolStripButton("1.500.000 normal/async yükle", null, async (_,__) => await Gridly.SetObjectsAsync<DemoRow>(async ct => { await Task.Delay(25, ct); return CreateRows(1_500_000); })));
        Tool.Items.Add(new ToolStripButton("Ad filtre popup", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Hızlı filtre modu", null, (_,__) => { Gridly.FastFilterMenuForHugeLists = true; MainForm.ConfigureMillionRowFiltering(Gridly); Info.Text = "999M filtre menüsü hızlı açılır; yazılan metin provider tarafında tüm sanal veri üzerinde filtrelenir."; }));
        Tool.Items.Add(new ToolStripButton("Tam tarama modu", null, (_,__) => { Gridly.FastFilterMenuForHugeLists = false; Info.Text = "Dev veride tüm distinct değerleri listelemek yerine arama/provider filtresi kullanılmalıdır."; }));
        Shown += (_,__) => Gridly.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000));
    }
}

public sealed class DesignerApiSampleForm : GridlySampleFormBase
{
    public DesignerApiSampleForm() : base("Gridly Designer / API Detay Örneği", "DLL projeye eklendiğinde designer tarafında değiştirilebilen özelliklerin ve kodla ayarlanabilen API'lerin kısa vitrini.")
    {
        Tool.Items.Add(new ToolStripButton("Designer özelliklerini göster", null, (_,__) => MessageBox.Show(this,
            ...
            "• Columns koleksiyonu: CheckBox, Badge, ProgressBar, Rating, Button", "Gridly Designer/API")));
        Tool.Items.Add(new ToolStripButton("Header filtre menüsü", null, (_,__) => Gridly.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Gridly.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => Gridly.SaveLayout(Path.Combine(Application.StartupPath, "sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => Gridly.LoadLayout(Path.Combine(Application.StartupPath, "sample-layout.json"))));
    }
}



public sealed class ColumnManagerSampleForm : GridlySampleFormBase
{
    private readonly string _layoutPath = Path.Combine(Application.StartupPath, "column-manager-sample-layout.json");

    public ColumnManagerSampleForm() : base("Gridly Column Manager Detay Örneği", "Kolon başlığını tutup sağa/sola sürükleyerek yer değiştir. Genişlik değişiklikleri ve kolon görünürlüğü layout olarak kaydedilebilir.")
    {
        Gridly.Name = "ColumnManagerSampleGridly";
        Gridly.AllowColumnReorder = true;
        Gridly.ShowColumnReorderPreview = true;
        Gridly.AutoSaveColumnLayout = false;
        Gridly.ColumnLayoutStorageKey = "ColumnManagerSample";

        Tool.Items.Add(new ToolStripButton("Sürükle-bırak aktif", null, (_,__) => { Gridly.AllowColumnReorder = true; Info.Text = "Kolon başlığını tutup sağa/sola sürükle."; }));
        Tool.Items.Add(new ToolStripButton("Otomatik kaydet Aç/Kapat", null, (_,__) => { Gridly.AutoSaveColumnLayout = !Gridly.AutoSaveColumnLayout; Info.Text = "AutoSaveColumnLayout = " + Gridly.AutoSaveColumnLayout; }));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => { Gridly.SaveLayout(_layoutPath); Info.Text = "Kaydedildi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => { Gridly.LoadLayout(_layoutPath); Info.Text = "Yüklendi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("Varsayılana dön", null, (_,__) => { Gridly.ResetColumnLayout(); Info.Text = "Kolon düzeni varsayılana döndü."; }));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Gridly.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("İçeriğe sığdır", null, (_,__) => Gridly.AutoResizeColumnsToContent()));
...
public sealed class LocalizationSampleForm : GridlySampleFormBase
{
    private readonly ToolStripComboBox _language = new() { DropDownStyle = ComboBoxStyle.DropDownList, Width = 170 };

    public LocalizationSampleForm() : base("Gridly Dil / Localization Test Formu", "Menü, filtre popup, kolon seçimi ve boş liste metinlerinin Windows dili veya seçilen dile göre değişmesini test eder.")
    {
        foreach (GridlyLanguage lang in Enum.GetValues(typeof(GridlyLanguage)))
            _language.Items.Add(lang);
        _language.SelectedItem = GridlyLanguage.Auto;
        _language.SelectedIndexChanged += (_,__) => ApplySelectedLanguage();

        Tool.Items.Add(new ToolStripLabel("Dil:"));
        Tool.Items.Add(_language);
        Tool.Items.Add(new ToolStripButton("Header filtre", null, (_,__) => Gridly.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Filtre penceresi", null, (_,__) => Gridly.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Gridly.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Boş liste metni", null, (_,__) => Gridly.SetObjects(Array.Empty<DemoRow>())));
        Tool.Items.Add(new ToolStripButton("Veriyi geri yükle", null, (_,__) => Gridly.SetObjects(CreateRows(250))));

        Gridly.Language = GridlyLanguage.Auto;
        Gridly.FilterMenuMode = GridlyFilterMenuMode.Both;
    ...
    {
        if (_language.SelectedItem is not GridlyLanguage lang) return;
        Gridly.Language = lang;
        Gridly.EmptyListMessage = GridlyText.EmptyList;
        Info.Text = $"Aktif dil: {lang} / Effective: {GridlyText.EffectiveLanguage}. Sağ tık menüsünü veya filtre popup'ını açarak metinleri kontrol et.";
        Gridly.Invalidate();
    }
}

public sealed class GLVFeaturePackSampleForm : GridlySampleFormBase
{
    public GLVFeaturePackSampleForm() : base("Gridly GLV Feature Pack", "GLV'de sık kullanılan liste özelliklerinin Gridly karşılıkları: hyperlink, toggle, tags, sparkline, color swatch, seçim/kolon/layout işlemleri.")
    {
        Gridly.Columns.Clear();
        Gridly.Columns.Add(new GridlyColumn("✓", nameof(DemoRow.Checked), 42) { Kind = GridlyColumnKind.CheckBox, AspectPutter = (row, value) => ((DemoRow)row).SetChecked(Convert.ToBoolean(value)) });
        Gridly.Columns.Add(new GridlyColumn("Toggle", nameof(DemoRow.NeedsReview), 76) { Kind = GridlyColumnKind.ToggleSwitch, AspectPutter = (row, value) => ((DemoRow)row).SetNeedsReview(Convert.ToBoolean(value)) });
        Gridly.Columns.Add(new GridlyColumn("Id", nameof(DemoRow.Id), 70));
        Gridly.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 210) { Editable = true, FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Link", nameof(DemoRow.Link), 90) { Kind = GridlyColumnKind.Hyperlink });
        Gridly.Columns.Add(new GridlyColumn("Etiketler", nameof(DemoRow.Tags), 180) { Kind = GridlyColumnKind.Tags });
        Gridly.Columns.Add(new GridlyColumn("Mini grafik", nameof(DemoRow.Spark), 130) { Kind = GridlyColumnKind.Sparkline });
        Gridly.Columns.Add(new GridlyColumn("Renk", nameof(DemoRow.RowColor), 105) { Kind = GridlyColumnKind.ColorSwatch });
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 110) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.Columns.Add(new GridlyColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = GridlyColumnKind.Rating });
        Gridly.SetObjects(CreateRows(500));
        Gridly.HyperlinkClick += (_, e) => MessageBox.Show(this, $"Hyperlink tıklandı: {((DemoRow)e.RowObject).Name}", "Gridly Hyperlink");
        Gridly.ItemActivate += (_, e) => Info.Text = $"Aktif satır: {((DemoRow)e.RowObject).Name}";

        Tool.Items.Add(new ToolStripButton("Tümünü seç", null, (_,__) => Gridly.SelectAllRows()));
        Tool.Items.Add(new ToolStripButton("Seçimi temizle", null, (_,__) => Gridly.ClearSelection()));
        Tool.Items.Add(new ToolStripButton("Seçimi ters çevir", null, (_,__) => Gridly.InvertSelection()));
        Tool.Items.Add(new ToolStripButton("Kolonları içeriğe göre ayarla", null, (_,__) => Gridly.AutoResizeColumnsToContent()));
        Tool.Items.Add(new ToolStripButton("Üste git", null, (_,__) => Gridly.ScrollToTop()));
        Tool.Items.Add(new ToolStripButton("Alta git", null, (_,__) => Gridly.ScrollToBottom()));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => Gridly.SaveLayout(Path.Combine(Application.StartupPath, "olv-feature-pack-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => Gridly.LoadLayout(Path.Combine(Application.StartupPath, "olv-feature-pack-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Kart görünümü", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.Details)));
    }
}
...
public sealed class DatabaseSampleForm : Form
{
    ...
    {
        Text = "Gridly Database Örnekleri";
        Width = 1180;
        Height = 720;
        StartPosition = FormStartPosition.CenterParent;
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new GridlyColumn("Id", "", 70) { AspectGetter = row => ((DataRow)row)["Id"] });
        _grid.Columns.Add(new GridlyColumn("Barkod", "", 160) { AspectGetter = row => ((DataRow)row)["Barcode"] });
        _grid.Columns.Add(new GridlyColumn("Makine", "", 130) { AspectGetter = row => ((DataRow)row)["MachineName"] });
        _grid.Columns.Add(new GridlyColumn("Assembly", "", 220) { AspectGetter = row => ((DataRow)row)["AssemblyName"] });
        _grid.Columns.Add(new GridlyColumn("AOI", "", 90) { AspectGetter = row => ((DataRow)row)["AoiResult"] });
        _grid.Columns.Add(new GridlyColumn("AI", "", 90) { AspectGetter = row => ((DataRow)row)["AIResult"] });
        _grid.Columns.Add(new GridlyColumn("Bekleyen", "", 90) { AspectGetter = row => ((DataRow)row)["WaitingDecision"] });
        _grid.Columns.Add(new GridlyColumn("Tarih", "", 160) { AspectGetter = row => ((DateTime)((DataRow)row)["AoiTestTime"]).ToString("yyyy-MM-dd HH:mm") });
    }
...
// gridlyView1.SetObjects(rows);
// gridlyView1.FastFilterMenuForHugeLists = true;
// gridlyView1.AsyncLoadFullFilterValues = true;
"""";
}
...
public sealed class TreeViewSampleForm : Form
{
    ...
    private readonly GridlyView _detailGrid = new()
    {
        Dock = DockStyle.Fill,
        FullRowSelect = true,
        ShowGridLines = true,
    ...
    private readonly TreeGridlyView _treeGrid = new()
    {
        Dock = DockStyle.Fill,
        FullRowSelect = true,
        ShowGridLines = true,
        ...
        TreeSearchBehavior = TreeGridlySearchBehavior.ExpandAncestorsAndDescendants,
    ...
    {
        Text = "Gridly TreeView + TreeGrid Örnekleri";
        Width = 1260;
        Height = 760;
        MinimumSize = new Size(980, 620);
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
        ForeColor = Program.AppTheme.ForeColor;
        GridlyDialogChrome.ConfigureStandardDialog(this, Program.AppTheme, new Size(980, 620), sizeable: true);
        GridlyDialogThemeApplier.Apply(this, Program.AppTheme);
        ...
        var tabs = new TabControl { Dock = DockStyle.Fill };
        var masterDetail = new TabPage("TreeView + Gridly detay") { BackColor = Program.AppTheme.BackColor };
        var treeGridTab = new TabPage("TreeGridlyView") { BackColor = Program.AppTheme.BackColor };
    ...
    private void ConfigureDetailGrid()
    {
        ...
        _detailGrid.Columns.Add(new GridlyColumn("Seviye", "Level", 70));
        _detailGrid.Columns.Add(new GridlyColumn("Tip", "Kind", 100));
        _detailGrid.Columns.Add(new GridlyColumn("Kod", "Code", 150));
        _detailGrid.Columns.Add(new GridlyColumn("Ad", "Name", 260) { FillFreeSpace = true });
        _detailGrid.Columns.Add(new GridlyColumn("Durum", "State", 100) { Kind = GridlyColumnKind.Badge });
        _detailGrid.Columns.Add(new GridlyColumn("Adet", "Count", 80));
    }

    private void ConfigureTreeGrid()
    {
        _treeGrid.ApplyTheme(Program.AppTheme);
        _treeGrid.Columns.Add(new GridlyColumn("Ağaç", "Name", 360) { FillFreeSpace = true, AspectGetter = row => new string(' ', ((TreeDemoNode)row).Level * 4) + (((TreeDemoNode)row).Children.Count > 0 ? (_treeGrid.IsExpanded(row) ? "▾ " : "▸ ") : "  ") + ((TreeDemoNode)row).Name });
        _treeGrid.Columns.Add(new GridlyColumn("Tip", "Kind", 110));
        _treeGrid.Columns.Add(new GridlyColumn("Kod", "Code", 150));
        _treeGrid.Columns.Add(new GridlyColumn("Durum", "State", 100) { Kind = GridlyColumnKind.Badge });
        _treeGrid.Columns.Add(new GridlyColumn("Adet", "Count", 80));
...
{
    private readonly GridlyTheme _theme = Program.AppTheme;
    ...
    public DialogThemeResizeShowcaseForm()
    {
        ...
        GridlyDialogChrome.ConfigureStandardDialog(this, _theme, new Size(640, 420), sizeable: true);
        ...
        var grid = new GridlyView { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
        grid.ApplyTheme(_theme);
        grid.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 180) { FillFreeSpace = true });
        grid.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 100));
        grid.Columns.Add(new GridlyColumn("Yıl", nameof(DemoRow.Year), 80));
        ...
        Controls.Add(panel);
        Controls.Add(info);
        GridlyDialogThemeApplier.Apply(this, _theme);
    }
}

public sealed class GridlyModeVerificationForm : Form
{
    public GridlyModeVerificationForm()
    {
        Text = "GridlyView Mode Doğrulama - Object / DataTable / Virtual / Tree / Tile";
        Width = 1180;
        Height = 720;
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
    ...
    {
        var grid = CreateDemoGrid(GridlyMode.Object);
        grid.SetObjects(CreateRows(300));
        return Wrap("Mode = Object", "POCO / model listesi ile klasik Gridly/GLV benzeri kullanım.", grid);
    }

    private static TabPage CreateDataTableTab()
    {
        var grid = CreateDemoGrid(GridlyMode.DataTable);
    ...
    {
        var grid = CreateDemoGrid(GridlyMode.Virtual);
    ...
    {
        var grid = new TreeGridlyView
        {
            Dock = DockStyle.Fill,
            Mode = GridlyMode.Tree,
            FullRowSelect = true,
            ShowGridLines = true,
            EnableModernEmptyState = true,
            EmptyListMessage = "Tree kayıt yok"
        };
        grid.Columns.Add(new GridlyColumn("Ad", "Name", 260));
        grid.Columns.Add(new GridlyColumn("Tür", "State", 130));
        grid.Columns.Add(new GridlyColumn("Yıl", "Year", 80));
        ...
        grid.Mode = GridlyMode.Tree;
        return Wrap("Mode = Tree", "TreeGridlyView ile hiyerarşik veri senaryosu. Aynı Gridly kolon/render altyapısını kullanır.", grid);
    }

    private static TabPage CreateTileTab()
    {
        var grid = CreateDemoGrid(GridlyMode.Tile);
        grid.TileMinWidth = 260;
        grid.TilePreferredHeight = 110;
        grid.SetObjects(CreateRows(150));
        grid.Mode = GridlyMode.Tile;
        return Wrap("Mode = Tile", "Kart/tile görünüm odaklı kullanım. Mode=Tile otomatik Tile ViewMode uygular.", grid);
    }

    private static GridlyView CreateDemoGrid(GridlyMode mode)
    {
        ...
        grid.Columns.Add(new GridlyColumn("Id", nameof(DemoRow.Id), 70));
        grid.Columns.Add(new GridlyColumn("Ad", nameof(DemoRow.Name), 240));
        grid.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 110));
        grid.Columns.Add(new GridlyColumn("Meslek", nameof(DemoRow.Occupation), 150));
        grid.Columns.Add(new GridlyColumn("Yıl", nameof(DemoRow.Year), 80));
        grid.Columns.Add(new GridlyColumn("Progress", nameof(DemoRow.Progress), 110) { Kind = GridlyColumnKind.ProgressBar });
        return grid;
    }
    ...
    private static List<DemoRow> CreateRows(int count)
    {
        ...
        for (int i = 1; i <= count; i++)
        {
            list.Add(new DemoRow
            {
                Id = i,
                RealIndex = i,
                Name = "Gridly Mode satır " + i.ToString("0000"),
                State = i % 7 == 0 ? "Fail" : i % 5 == 0 ? "Review" : "OK",
                Occupation = DemoData.Occupations[i % DemoData.Occupations.Length],
                Year = 2020 + (i % 7),
                Progress = (i * 7) % 101,
    ...
    {
        var t = new DataTable("GridlyModeDataTable");
...
public sealed class FeatureVerificationSuiteForm : Form
{
    public FeatureVerificationSuiteForm()
    {
        ...
        Text = "Gridly Özellik Test Merkezi";
        Width = 920;
        Height = 620;
        MinimumSize = new Size(760, 480);
        BackColor = theme.BackColor;
        ForeColor = theme.ForeColor;
        GridlyWindowChrome.ApplyOnHandleCreated(this, () => theme, true);
        ...
        var title = new Label
        {
            ...
            Text = "Gridly özellik doğrulama örnekleri",
            Font = new Font("Segoe UI", 14F, FontStyle.Bold),
        ...
        AddSuiteCard(grid, theme, "8) GridlyView Mode", "Object/DataTable/Virtual/Tree/Tile modlarını ayrı sekmelerde doğrular.", () => new GridlyModeVerificationForm().Show());
    ...
    private static void AddSuiteCard(TableLayoutPanel grid, GridlyTheme theme, string header, string description, Action open)
    {
...
public sealed class LayoutManagerVerificationForm : GridlySampleFormBase
{
    private readonly string _layoutPath = Path.Combine(Application.StartupPath, "gridly-v2487-layout-test.json");
    ...
    {
        Gridly.ColumnLayoutStorageKey = "GridlyV2487LayoutTest";
        Gridly.AutoSaveColumnLayout = false;
        Tool.Items.Add(new ToolStripButton("1 Kaydet", null, (_,__) => { Gridly.SaveLayout(_layoutPath); Info.Text = "Layout kaydedildi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("2 Kolonları boz", null, (_,__) => { var progressCol = Gridly.Columns[nameof(DemoRow.Progress)]; if (progressCol != null) progressCol.Visible = false; var nameCol = Gridly.Columns[nameof(DemoRow.Name)]; if (nameCol != null) nameCol.Width = 420; Gridly.SetGroupBy(nameof(DemoRow.State)); Gridly.RefreshView(); Info.Text = "Kolon görünürlüğü/genişliği ve grup değiştirildi. Şimdi Yükle ile geri al."; }));
        Tool.Items.Add(new ToolStripButton("3 Yükle", null, (_,__) => { Gridly.LoadLayout(_layoutPath); Info.Text = "Layout yüklendi."; }));
        Tool.Items.Add(new ToolStripButton("4 Reset", null, (_,__) => { Gridly.ResetColumnLayout(); Info.Text = "Layout varsayılana döndü."; }));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Gridly.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Duruma göre grupla", null, (_,__) => Gridly.SetGroupBy(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Grubu kaldır", null, (_,__) => Gridly.ClearGrouping()));
    }
}

public sealed class IncrementalSearchHighlightVerificationForm : GridlySampleFormBase
{
    ...
    {
        Gridly.HighlightSearchText = true;
        Gridly.HighlightGlobalFilterText = true;
        Gridly.EnableIncrementalSearch = true;
        Gridly.DefaultHighlightDurationMs = 4500;
        Gridly.SetObjects(CreateRows(1000));
        Tool.Items.Add(new ToolStripLabel("Ara:"));
        ...
        Tool.Items.Add(new ToolStripButton("Seçiliyi vurgula", null, (_,__) => { if (Gridly.SelectedObject != null) Gridly.HighlightObject(Gridly.SelectedObject, Color.Orange, 6000, "manual"); }));
        Tool.Items.Add(new ToolStripButton("Vurguları temizle", null, (_,__) => Gridly.ClearRowHighlights()));
        _find.TextChanged += (_,__) => { if (!string.IsNullOrWhiteSpace(_find.Text)) Gridly.JumpToFirstMatch(_find.Text); else Gridly.ClearSearchHighlight(); };
    }

    private void DoFindNext()
    {
        bool ok = Gridly.FindNext(_find.Text);
        Info.Text = ok ? "Sonraki eşleşmeye gidildi." : "Eşleşme bulunamadı.";
    }

    private void DoFindPrevious()
    {
        bool ok = Gridly.FindPrevious(_find.Text);
    ...
    {
        int highlighted = 0;
        for (int i = 0; i < Gridly.ViewCount && highlighted < 25; i++)
        {
            if (Gridly.GetModelObject(i) is DemoRow row && row.State == "Fail")
            {
                Gridly.HighlightRow(i, Color.FromArgb(255, 193, 7), 7000, "Fail");
                highlighted++;
            }
        }
...
public sealed class AdvancedCopyExportVerificationForm : GridlySampleFormBase
{
    private readonly string _folder = Path.Combine(Application.StartupPath, "GridlyExportTests");
    ...
    public AdvancedCopyExportVerificationForm() : base("Advanced Copy / Export", "Test: birkaç satır seç; hücre/satır/JSON kopyala. CSV, Excel XML ve JSON dosyaları test klasörüne yazılır.")
    {
        ...
        Gridly.MultiSelect = true;
        Gridly.SelectObject(Gridly.GetModelObject(4));
        Gridly.SelectObject(Gridly.GetModelObject(9), addToSelection: true);
        Tool.Items.Add(new ToolStripButton("Seçimi kopyala", null, (_,__) => { Gridly.CopySelectionToClipboard(); Info.Text = "Seçim TSV olarak panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("Hücreyi kopyala", null, (_,__) => { Gridly.CopySelectedCellToClipboard(); Info.Text = "Aktif hücre panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("JSON kopyala", null, (_,__) => { Gridly.CopySelectionAsJsonToClipboard(); Info.Text = "Seçim JSON olarak panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("CSV export", null, (_,__) => Export(() => Gridly.ExportVisibleCsv(Path.Combine(_folder, "visible.csv")))));
        Tool.Items.Add(new ToolStripButton("Excel export", null, (_,__) => Export(() => Gridly.ExportVisibleExcel(Path.Combine(_folder, "visible.xlsx"), "GridlyTest"))));
        Tool.Items.Add(new ToolStripButton("JSON export", null, (_,__) => Export(() => Gridly.ExportVisibleJson(Path.Combine(_folder, "visible.json")))));
        Tool.Items.Add(new ToolStripButton("Seçili JSON export", null, (_,__) => Export(() => Gridly.ExportSelectedJson(Path.Combine(_folder, "selected.json")))));
    }
...
public sealed class GroupingVerificationForm : GridlySampleFormBase
{
    ...
    {
        Gridly.EnableGrouping = true;
        Gridly.SetObjects(CreateRows(800));
        Tool.Items.Add(new ToolStripButton("Durum", null, (_,__) => { Gridly.SetGroupBy(nameof(DemoRow.State)); Info.Text = "Duruma göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Meslek", null, (_,__) => { Gridly.SetGroupBy(nameof(DemoRow.Occupation)); Info.Text = "Mesleğe göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Yıl", null, (_,__) => { Gridly.SetGroupBy(nameof(DemoRow.Year)); Info.Text = "Yıla göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Daralt", null, (_,__) => { Gridly.CollapseAllGroups(); Info.Text = "Tüm gruplar daraltıldı."; }));
        Tool.Items.Add(new ToolStripButton("Aç", null, (_,__) => { Gridly.ExpandAllGroups(); Info.Text = "Tüm gruplar açıldı."; }));
        Tool.Items.Add(new ToolStripButton("Renk ver", null, (_,__) => { Gridly.CustomGroupBackColor = Color.FromArgb(235, 229, 250); Gridly.Invalidate(); Info.Text = "Grup başlığı rengi değiştirildi."; }));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => { Gridly.ClearGrouping(); Info.Text = "Gruplama kaldırıldı."; }));
        Gridly.SetGroupBy(nameof(DemoRow.State));
    }
}

public sealed class MiniAnalyticsVerificationForm : GridlySampleFormBase
{
    ...
    public MiniAnalyticsVerificationForm() : base("Mini Analytics", "Test: kolon bazlı Row/Blank/Distinct/Top Values raporunu üret. Filtre uygula ve raporu tekrar yenileyerek görünür veri analitiğini kontrol et.")
    {
        ...
        Gridly.SetObjects(CreateRows(1200));
        ...
        Tool.Items.Add(new ToolStripButton("Panoya kopyala", null, (_,__) => { Gridly.CopyMiniAnalyticsToClipboard(); Info.Text = "Mini analytics panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("Fail filtresi", null, (_,__) => { Gridly.SetGlobalFilter("Fail"); RefreshAnalytics(); }));
        Tool.Items.Add(new ToolStripButton("Filtre temizle", null, (_,__) => { Gridly.ClearFilters(); RefreshAnalytics(); }));
        RefreshAnalytics();
    }
    ...
    private void RefreshAnalytics()
    {
        ...
        foreach (var item in Gridly.GetVisibleAnalytics(maxDistinctPerColumn: 5, maxRows: 50000))
        {
    ...
    {
        var item = Gridly.GetColumnAnalytics(aspectName, maxDistinct: 10, maxRows: 50000);
        if (item == null) return;
...
public sealed class DesignerPreviewSampleForm : GridlySampleFormBase
{
    private readonly ToolStripComboBox _preview = new() { DropDownStyle = ComboBoxStyle.DropDownList, Width = 130 };

    public DesignerPreviewSampleForm() : base("Gridly Designer Önizleme", "DesignTimeThemePreview, AutoThemeFromParent ve örnek veri davranışını runtime'da da test eder.")
    {
        foreach (GridlyDesignTimeThemePreview value in Enum.GetValues(typeof(GridlyDesignTimeThemePreview)))
            _preview.Items.Add(value);
        _preview.SelectedItem = GridlyDesignTimeThemePreview.Auto;
        ...
        Tool.Items.Add(new ToolStripButton("Örnek veri yenile", null, (_,__) => { Gridly.DesignTimeSampleData = true; Gridly.RefreshDesignTimePreview(); }));

        Gridly.AutoThemeFromParent = true;
        Gridly.DesignTimeSampleData = true;
        ApplyPreviewMode();
    }

    private void ApplyPreviewMode()
    {
        if (_preview.SelectedItem is not GridlyDesignTimeThemePreview mode) return;
        Gridly.DesignTimeThemePreview = mode;
        ApplyParentPreview();
    }

    private void ApplyParentPreview()
    {
        var mode = Gridly.DesignTimeThemePreview;
        var theme = mode switch
        {
            ...
            GridlyDesignTimeThemePreview.Fluent => BackColor.GetBrightness() < 0.45f ? GridlyTheme.FluentDarkTheme() : GridlyTheme.FluentLightTheme(),
            _ => GridlyTheme.FromParentColor(BackColor, ForeColor)
        };
        Gridly.ApplyTheme(theme);
        Info.BackColor = theme.PanelBackColor;
        Info.ForeColor = theme.ForeColor;
        Tool.BackColor = theme.HeaderBackColor;
        Tool.ForeColor = theme.HeaderForeColor;
        Info.Text = $"Designer önizleme modu: {mode} | AutoThemeFromParent={Gridly.AutoThemeFromParent} | Form rengi={ColorTranslator.ToHtml(BackColor)}";
    }
}
...
public sealed class DatabaseCrudEditorsSampleForm : Form
{
    ...
    private void ConfigureGeneratedColumns()
    {
        ...
        if (barcode != null) { barcode.EditorType = Gridly.Editing.GridlyCellEditorKind.TextBox; barcode.Required = true; barcode.MaxLength = 30; barcode.Width = 160; }

        var active = _grid.GetColumn("IsActive");
        if (active != null) { active.Header = "Aktif"; active.Kind = GridlyColumnKind.CheckBox; active.EditorType = Gridly.Editing.GridlyCellEditorKind.CheckBox; active.Width = 80; }

        var result = _grid.GetColumn("Result");
        if (result != null) { result.Header = "Sonuç"; result.Kind = GridlyColumnKind.ComboBox; result.EditorType = Gridly.Editing.GridlyCellEditorKind.ComboBox; result.ComboBoxItems = new List<string> { "PASS", "FAIL", "WAIT", "REPAIR" }; result.Width = 110; }

        var qty = _grid.GetColumn("Quantity");
        if (qty != null) { qty.Header = "Adet"; qty.Kind = GridlyColumnKind.Numeric; qty.EditorType = Gridly.Editing.GridlyCellEditorKind.Numeric; qty.NumericMinimum = 0; qty.NumericMaximum = 999999; qty.Width = 90; }

        var score = _grid.GetColumn("Score");
        if (score != null) { score.Header = "Puan"; score.Kind = GridlyColumnKind.Numeric; score.EditorType = Gridly.Editing.GridlyCellEditorKind.Numeric; score.NumericMinimum = 0; score.NumericMaximum = 100; score.Width = 90; }

        var date = _grid.GetColumn("TestDate");
        if (date != null) { date.Header = "Test Tarihi"; date.Kind = GridlyColumnKind.Date; date.EditorType = Gridly.Editing.GridlyCellEditorKind.DateTime; date.DateTimeFormat = Gridly.Editing.GridlyDateTimeEditorFormat.Custom; date.DateTimeCustomFormat = "dd.MM.yyyy HH:mm"; date.Width = 150; }
    ...
    private void DeleteRows()
    {
        ...
        MessageBox.Show(this, deleted + " satır silinmek üzere işaretlendi.", "Gridly CRUD");
        RefreshStatus();
    }
    ...
    private void SaveChangesDemo()
    {
        ...
        MessageBox.Show(this, "Demo DataTable modunda SaveChanges AcceptChanges yapar. SQL adapter bağlıysa INSERT/UPDATE/DELETE komutları çalışır. Affected: " + affected, "Gridly CRUD");
        RefreshStatus();
    }
...
{
    private readonly Gridly.Core.GridlyView _grid = new()
    {
        Dock = DockStyle.Fill,
        Mode = GridlyMode.Object,
        ViewMode = GridlyViewMode.Details,
        DesignTimeThemePreview = GridlyDesignTimeThemePreview.Auto,
    ...
    private readonly Label _info = new()
    {
        ...
        TextAlign = ContentAlignment.MiddleLeft,
        Text = "Gridly üzerinde sağ tıkla: kullanıcı ContextMenuStrip menüsünün altında 'Liste özellikleri' olarak Gridly'nin kendi hızlı menüsü birleşir."
    };
    ...
    {
        Text = "Gridly Designer + ContextMenu Merge Örneği";
        Width = 980;
        Height = 640;
        StartPosition = FormStartPosition.CenterScreen;
        ...
        _grid.Columns.Add(new GridlyColumn("Id", nameof(Row.Id), 70));
        _grid.Columns.Add(new GridlyColumn("Ad", nameof(Row.Name), 180) { FillFreeSpace = true });
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(Row.State), 110));
        _grid.Columns.Add(new GridlyColumn("Oran", nameof(Row.Rate), 90) { TextAlign = ContentAlignment.MiddleRight });
        ...
        _top.Items.Add(new ToolStripButton("Mode=Object", null, (_,__) => _grid.Mode = GridlyMode.Object));
        _top.Items.Add(new ToolStripButton("Mode=Virtual", null, (_,__) => _grid.Mode = GridlyMode.Virtual));
        _top.Items.Add(new ToolStripButton("Mode=Tree", null, (_,__) => _grid.Mode = GridlyMode.Tree));
        _top.Items.Add(new ToolStripButton("Mode=Tile", null, (_,__) => _grid.Mode = GridlyMode.Tile));
...
// not only as detached code snippets under samples/Gridly.FeatureSamples.
public sealed class PdfExportSuiteSampleForm : GridlySampleFormBase
{
    public PdfExportSuiteSampleForm() : base("v28.7 PDF Export Suite", "Details/Table ve Card/Dashboard PDF export örneği. Masaüstüne Gridly_Table_Export.pdf ve Gridly_Card_Export.pdf üretir.")
    {
        Gridly.Columns.Clear();
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DemoRow.State), 120) { CardRole = "Title", CardOrder = 0 });
        Gridly.Columns.Add(new GridlyColumn("Makine", nameof(DemoRow.Machine), 150) { CardOrder = 1 });
        Gridly.Columns.Add(new GridlyColumn("Meslek", nameof(DemoRow.Occupation), 140) { CardOrder = 2 });
        Gridly.Columns.Add(new GridlyColumn("Açıklama", nameof(DemoRow.Description), 260) { CardOrder = 3, CardShowCaption = true });
        Gridly.Columns.Add(new GridlyColumn("Puan", nameof(DemoRow.Rating), 80) { CardOrder = 4 });
        Gridly.SetObjects(CreateRows(60));
        Gridly.CardVisualInfoGetter = row =>
        {
            ...
            return new GridlyCardVisualInfo
            {
                AccentColor = statusColor,
                DotColor = statusColor,
                Badges =
                {
                    new GridlyCardBadge { Text = r.State.Contains("Fail", StringComparison.OrdinalIgnoreCase) ? "FAIL" : "OK", BackColor = statusColor, ForeColor = Color.White }
                }
            };
        };
        Tool.Items.Add(new ToolStripSeparator());
        ...
        Tool.Items.Add(new ToolStripButton("Card görünümü", null, (_,__) => Gridly.SetViewMode(GridlyViewMode.DashboardCard)));
    }

    private void ExportPdf(bool card)
    {
        string file = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), card ? "Gridly_Card_Export.pdf" : "Gridly_Table_Export.pdf");
        string actual = Gridly.ExportVisiblePdf(file, new global::Gridly.Exporting.GridlyPdfExportOptions
        {
            Title = card ? "Gridly Card PDF Export" : "Gridly Table PDF Export",
            Subtitle = "v28.7 export smoke sample",
            Mode = card ? global::Gridly.Exporting.GridlyPdfExportMode.Card : global::Gridly.Exporting.GridlyPdfExportMode.Table,
            Orientation = card ? global::Gridly.Exporting.GridlyPdfPageOrientation.Portrait : global::Gridly.Exporting.GridlyPdfPageOrientation.Landscape,
            FitToPageWidth = true,
            ShowGridLines = true,
            ZebraRows = true,
            CardColumns = 2,
        ...
        MessageBox.Show(this, "PDF oluşturuldu:\n" + actual, "Gridly PDF Export", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
}

public sealed class CellOverflowScrollSampleForm : GridlySampleFormBase
{
    public CellOverflowScrollSampleForm() : base("v28.8 Hücre İçi Scroll", "Uzun açıklama/not kolonlarında satır yüksekliği büyümeden hücre içinde mouse wheel ile kaydırma ve çift tık okuyucu popup test edilir.")
    {
        Gridly.Columns.Clear();
        Gridly.RowHeight = 62;
        Gridly.DetailsRowHeight = 62;
        Gridly.AllowMultilineCells = true;
        Gridly.AutoRowHeightForMultilineCells = false;
        Gridly.EnableCellOverflowScroll = true;
        Gridly.ShowCellOverflowScrollBars = true;
        Gridly.EnableCellOverflowDetailsPopup = true;
        Gridly.Columns.Add(new GridlyColumn("Id", nameof(DemoRow.Id), 70));
        Gridly.Columns.Add(new GridlyColumn("Makine", nameof(DemoRow.Machine), 150));
        Gridly.Columns.Add(new GridlyColumn("Kısa Durum", nameof(DemoRow.State), 120));
        Gridly.Columns.Add(new GridlyColumn("Mesaj / Açıklama", nameof(DemoRow.Description), 300)
        {
            WordWrap = true,
        ...
        Gridly.Columns.Add(new GridlyColumn("Teknisyen Notu", nameof(DemoRow.Notes), 280)
        {
            WordWrap = true,
            AllowCellScroll = true,
            CellScrollMaxVisibleLines = 3,
        ...
        Gridly.Columns.Add(new GridlyColumn("Zaman", nameof(DemoRow.Date), 140));
        var rows = CreateRows(80);
