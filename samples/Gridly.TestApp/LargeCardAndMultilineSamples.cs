...
public sealed class LargeCardSampleForm : Form
{
    private readonly GridlyView _grid = new()
    {
        ...
        ViewMode = GridlyViewMode.LargeCard,
    ...
    {
        Text = "Gridly Geniş Kart / Ticket Görünümü Örneği";
        Width = 1180;
        Height = 720;
        StartPosition = FormStartPosition.CenterScreen;

        _grid.Columns.Add(new GridlyColumn("Ticket", nameof(TicketCard.TicketNo), 120));
        _grid.Columns.Add(new GridlyColumn("Başlık", nameof(TicketCard.Title), 260) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new GridlyColumn("Makine", nameof(TicketCard.Machine), 160));
        _grid.Columns.Add(new GridlyColumn("Oto Kod", nameof(TicketCard.OtoCode), 100));
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(TicketCard.Status), 95));
        _grid.Columns.Add(new GridlyColumn("Mesaj", nameof(TicketCard.LastMessage), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 5 });

        _tool.Items.Add(new ToolStripButton("Geniş Kart", null, (_,__) => SetLargeCard()));
        _tool.Items.Add(new ToolStripButton("Details", null, (_,__) => _grid.SetViewMode(GridlyViewMode.Details)));
        _tool.Items.Add(new ToolStripButton("Tile", null, (_,__) => _grid.SetViewMode(GridlyViewMode.Tile)));
    ...
    private void SetLargeCard()
    {
        ...
        _grid.SetViewMode(GridlyViewMode.LargeCard);
    }
...
public sealed class MultilineCellsSampleForm : Form
{
    ...
    {
        Text = "Gridly Details Çok Satırlı Hücre Örneği";
        Width = 1180;
        Height = 720;
        StartPosition = FormStartPosition.CenterScreen;

        _grid.Columns.Add(new GridlyColumn("Id", nameof(MessageRow.Id), 70));
        _grid.Columns.Add(new GridlyColumn("Makine", nameof(MessageRow.Machine), 150));
        _grid.Columns.Add(new GridlyColumn("Kısa Durum", nameof(MessageRow.Status), 120));
        _grid.Columns.Add(new GridlyColumn("Mesaj / Açıklama", nameof(MessageRow.Message), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 5 });
        _grid.Columns.Add(new GridlyColumn("Teknisyen Notu", nameof(MessageRow.TechnicianNote), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
        _grid.Columns.Add(new GridlyColumn("Zaman", nameof(MessageRow.TimeText), 150));
