...
public sealed class GridlyV273RendererShowcaseForm : Form
{
    ...
    public GridlyV273RendererShowcaseForm()
    {
        Text = "Gridly v27.3 Renderer Showcase";
        Width = 1280;
        Height = 760;
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new GridlyColumn("Id", nameof(RendererRow.Id), 64) { TextAlign = ContentAlignment.MiddleRight });
        _grid.Columns.Add(new GridlyColumn("Alan", nameof(RendererRow.Area), 120) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(RendererRow.Status), 128) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Başlık", nameof(RendererRow.Title), 260) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new GridlyColumn("Etiketler", nameof(RendererRow.Tags), 230) { Kind = GridlyColumnKind.Tags, TagSeparator = ";" });
        _grid.Columns.Add(new GridlyColumn("İlerleme", nameof(RendererRow.Progress), 120) { Kind = GridlyColumnKind.ProgressBar });
        _grid.Columns.Add(new GridlyColumn("Aksiyon", nameof(RendererRow.Action), 118) { Kind = GridlyColumnKind.Button, ButtonText = "Aç", ButtonSizing = GridlyColumnButtonSizing.TextBounds });
        _grid.Columns.Add(new GridlyColumn("Link", nameof(RendererRow.Link), 170) { Kind = GridlyColumnKind.Hyperlink });
        _grid.Columns.Add(new GridlyColumn("Açıklama", nameof(RendererRow.Description), 360) { WordWrap = true, MaxTextLines = 3, FillFreeSpace = true });

        _grid.ButtonClick += (_, e) => MessageBox.Show(this, "Aksiyon: " + ((RendererRow)e.RowObject).Title, "Gridly v27.3");
        _grid.HyperlinkClick += (_, e) => MessageBox.Show(this, "Link: " + Convert.ToString(e.Column.GetValue(e.RowObject)), "Gridly v27.3");
    }

    private void LoadRows()
    {
        var rows = new List<RendererRow>();
        string[] areas = { "AOI", "MasterData", "SAP", "Program", "Server", "Gridly" };
        ...
        for (int i = 1; i <= 72; i++)
        {
            ...
            rows.Add(new RendererRow
            {
                ...
                Title = area switch
                {
                    ...
                    _ => "Gridly renderer ve UX davranışı"
                },
    ...
    private void ApplyTheme(bool dark)
    {
        ...
        GridlyTheme theme = GridlyTheme.FromParentColor(back, fore);
