...
public sealed class GridlyV502HardeningSampleForm : Form
{
    private readonly GridlyView _grid = new()
    {
        Dock = DockStyle.Fill,
        Mode = GridlyMode.Object,
        ViewMode = GridlyViewMode.RowPreview,
        RowHeight = 96,
    ...
    public GridlyV502HardeningSampleForm()
    {
        Text = "Gridly v50.2 Build & Runtime Hardening";
        Width = 1220;
        Height = 740;
        MinimumSize = new Size(980, 620);
        StartPosition = FormStartPosition.CenterScreen;

        _grid.ApplyThemeStudioPreset(GridlyThemeStudioPreset.FactoryOsDark);
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new GridlyColumn("Alan", nameof(HardeningRow.Area), 140) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Kontrol", nameof(HardeningRow.Check), 220));
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(HardeningRow.Status), 110) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Açıklama", nameof(HardeningRow.Description), 420) { FillFreeSpace = true });
        _grid.Columns.Add(new GridlyColumn("Audix/AOI etkisi", nameof(HardeningRow.ProjectImpact), 300));
    }
    ...
    private void ConfigureToolbar()
    {
        ...
        _tool.Items.Add(new ToolStripButton("Dark", null, (_, __) => _grid.ApplyThemeStudioPreset(GridlyThemeStudioPreset.FactoryOsDark)));
        _tool.Items.Add(new ToolStripButton("Light", null, (_, __) => _grid.ApplyThemeStudioPreset(GridlyThemeStudioPreset.FactoryOsLight)));
        _tool.Items.Add(new ToolStripButton("High Contrast", null, (_, __) => _grid.ApplyThemeStudioPreset(GridlyThemeStudioPreset.HighContrastDark)));
    }
    ...
    private void LoadRows()
    {
        ...
        {
            new("Build", "API Guard", "Aktif", "Duplicate property/event, eksik namespace ve örnek merkez referans risklerini statik tarama ile görünür yapar.", "Yeni faz eklerken GridlyColumnKind / EnableCommandPalette benzeri hataları daha erken yakalar."),
    ...
    {
        var lines = _grid.RunGridly502RuntimeHardeningChecks()
            .Select(c => (c.Passed ? "✓ " : "! ") + c.Area + " / " + c.Check + " - " + c.Message);
        MessageBox.Show(this, string.Join(Environment.NewLine, lines), "Gridly v50.2 Runtime Hardening Check", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
