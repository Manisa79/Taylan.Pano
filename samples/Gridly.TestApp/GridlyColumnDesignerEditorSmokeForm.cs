...
public sealed class GridlyColumnDesignerEditorSmokeForm : GridlySampleFormBase
{
    ...
    private readonly Label _testSteps = new()
    {
        ...
        Text = "1) Audix Kolonu Ekle veya Designer Property Seti uygula.  2) Büyük 'Kolon Editor Aç' düğmesine bas.  3) Açılan Gridly kolon editoründe ekle/sil/değiştir ve Tamam ya da İptal ile dön.  4) Sağdaki raporda propertylerin korunup korunmadığını kontrol et."
    };
    ...
    public GridlyColumnDesignerEditorSmokeForm()
        : base(
            "Gridly Kolon Designer Editor Smoke Test",
            "Gerçek GridlyColumnCollectionEditor penceresini açar. Tamam/İptal, kolon ekle/sil ve property korunmasını burada hızlıca test edebilirsin.")
    {
        Width = 1280;
        Height = 760;
    ...
    private void ResetColumns()
    {
        ...
        Gridly.Columns.Add(new GLVColumn("Status", nameof(DemoRow.State), 120)
        {
            Name = "glvStatus",
        ...
        Gridly.Columns.Add(new GLVColumn("Progress", nameof(DemoRow.Progress), 105)
        {
            Name = "glvProgress",
        ...
        Gridly.Columns.Add(new GLVColumn("Notes", nameof(DemoRow.Notes), 300)
        {
            Name = "glvNotes",
            WordWrap = true,
            FillFreeSpace = true,
    ...
    private void OpenColumnEditor()
    {
        ...
        bool changed = InvokeGridlyColumnEditor();
        Gridly.RebuildColumns();
    ...
    private bool InvokeGridlyColumnEditor()
    {
        var editorType = typeof(GridlyView).Assembly.GetType("Gridly.Design.GridlyColumnCollectionEditor", throwOnError: true);
        ...
        var columnsProperty = TypeDescriptor.GetProperties(Gridly)[nameof(Gridly.Columns)];
    ...
    {
        var notes = Gridly.Columns.ByName("glvNotes") ?? Gridly.Columns.ByAspectName(nameof(DemoRow.Notes));
        if (notes != null)
        {
            notes.PrivateColumn = !notes.PrivateColumn;
            notes.Kind = GridlyColumnKind.Text;
            notes.WordWrap = true;
        ...
        var status = Gridly.Columns.ByName("glvStatus") ?? Gridly.Columns.ByAspectName(nameof(DemoRow.State));
        if (status != null)
        {
            status.Kind = GridlyColumnKind.Badge;
    ...
    private string CreateSnapshotText()
    {
        var lines = new List<string>
        {
            ...
            string.Empty,
            "Count=" + Gridly.Columns.Count.ToString(System.Globalization.CultureInfo.InvariantCulture),
            "Visible=" + string.Join(", ", Gridly.Columns.VisibleColumns.Select(c => c.Name)),
            string.Empty
        };

        for (int i = 0; i < Gridly.Columns.Count; i++)
        {
            var c = Gridly.Columns[i];
            lines.Add($"[{i}] {c.Name}");
    ...
    {
        if (Gridly.Columns.ByName(baseName) == null)
            return baseName;

        int i = 2;
        while (Gridly.Columns.ByName(baseName + i.ToString(System.Globalization.CultureInfo.InvariantCulture)) != null)
            i++;
