...
public sealed class DocumentationMissingScreenshotsForm : GridlySampleFormBase
{
    ...
    public DocumentationMissingScreenshotsForm(DocumentationMissingScreenshotScenario scenario)
        : base($"Gridly DOCX Missing Screenshot - {scenario.Title}", scenario.Description)
    {
        _scenario = scenario;
        Text = $"Gridly Documentation Capture - {scenario.TargetHeading}";
        Width = 1280;
        Height = 800;
    ...
    private void ApplyScenario()
    {
        ...
        Gridly.ClearSort();
        Gridly.ShowHeader = true;
        Gridly.ShowGridLines = true;
        Gridly.FullRowSelect = true;
        Gridly.MultiSelect = true;
        Gridly.AllowEditAllCells = false;
        Gridly.AllowColumnReorder = false;
        Gridly.AllowRowDragDrop = false;
        Gridly.ShowQuickFilterBar = false;
        Gridly.ShowActiveFilterChips = false;
        Gridly.CardVisualAdornments = false;
        Gridly.AllowMultilineCells = false;
        Gridly.MaxCellTextLines = 1;
        Gridly.DetailsRowHeight = 30;
        Gridly.EnableSearchEverywhere = false;
        Gridly.EnableCommandPalette = false;
    ...
    {
        Gridly.Columns.Add(new GridlyColumn("✓", nameof(DocumentationFeatureRow.Checked), 44) { Kind = GridlyColumnKind.CheckBox, HeaderCheckBox = true, HeaderCheckBoxThreeState = true });
        Gridly.Columns.Add(new GridlyColumn("Id", nameof(DocumentationFeatureRow.Id), 70));
        Gridly.Columns.Add(new GridlyColumn("Ad", nameof(DocumentationFeatureRow.Name), 230) { FillFreeSpace = true, Editable = true });
        Gridly.Columns.Add(new GridlyColumn("Kategori", nameof(DocumentationFeatureRow.Category), 150));
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DocumentationFeatureRow.Status), 110) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DocumentationFeatureRow.Progress), 130) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.Columns.Add(new GridlyColumn("Skor", nameof(DocumentationFeatureRow.Score), 90) { Kind = GridlyColumnKind.Rating, MaxRating = 5 });
        if (includeSparkline)
            Gridly.Columns.Add(new GridlyColumn("Trend", nameof(DocumentationFeatureRow.Spark), 140) { Kind = GridlyColumnKind.Sparkline });
        if (includeAction)
        {
            Gridly.Columns.Add(new GridlyColumn("Aksiyon", nameof(DocumentationFeatureRow.Action), 110) { Kind = GridlyColumnKind.Button, ButtonText = "Aç" });
            Gridly.Columns.Add(new GridlyColumn("Link", nameof(DocumentationFeatureRow.Link), 120) { Kind = GridlyColumnKind.Hyperlink });
        }
        Gridly.Columns.Add(new GridlyColumn("Notlar", nameof(DocumentationFeatureRow.Notes), 340) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
    }

    private void AddCapabilityColumns()
    {
        Gridly.Columns.Add(new GridlyColumn("Sıra", nameof(DocumentationFeatureRow.Id), 70));
        Gridly.Columns.Add(new GridlyColumn("Yetkinlik", nameof(DocumentationFeatureRow.Name), 260) { FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Kategori", nameof(DocumentationFeatureRow.Category), 160));
        Gridly.Columns.Add(new GridlyColumn("Durum", nameof(DocumentationFeatureRow.Status), 120) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Kapsam", nameof(DocumentationFeatureRow.Progress), 120) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.Columns.Add(new GridlyColumn("Çıktı", nameof(DocumentationFeatureRow.Action), 130));
        Gridly.Columns.Add(new GridlyColumn("Açıklama", nameof(DocumentationFeatureRow.Notes), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
    }

    private void ConfigureMixedColumnSystem()
    {
        AddStandardColumns(includeAction: true, includeSparkline: true);
        Gridly.Columns.Add(new GridlyColumn("Etiketler", nameof(DocumentationFeatureRow.Tags), 160) { Kind = GridlyColumnKind.Tags });
        Gridly.SetObjects(CreateRows("Column", 42));
        Gridly.SortBy(Gridly.Columns[nameof(DocumentationFeatureRow.Status)], false);
    }
    ...
    private void ConfigureDataBinding()
    {
        ...
        {
            Row(1, "List<T> binding", "Binding", "Ready", 100, "SetObjects", "POCO nesne listesi doğrudan Gridly içine bağlanır."),
    ...
    {
        AddCapabilityColumns();
        Gridly.ShowQuickFilterBar = true;
        Gridly.ShowActiveFilterChips = true;
        Gridly.SetObjects(new[]
        {
        ...
        Gridly.SetGlobalFilter("status open");
    }
    ...
    private void ConfigureQuickFilter()
    {
        ...
        Gridly.ShowQuickFilterBar = true;
        Gridly.ShowActiveFilterChips = true;
        Gridly.SetObjects(CreateRows("Quick Filter", 80));
        Gridly.SetGlobalFilter("Review");
    ...
    private void ConfigureSorting()
    {
        ...
        Gridly.SetObjects(CreateRows("Sorting", 120));
        Gridly.SortBy(Gridly.Columns[nameof(DocumentationFeatureRow.Progress)], descending: true);
    ...
    {
        AddStandardColumns();
        Gridly.SetObjects(CreateRows("Grouping", 120));
        Gridly.SetGroupBy(nameof(DocumentationFeatureRow.Status));
    ...
    {
        AddStandardColumns();
        Gridly.SetObjects(CreateRows("Quality", 80));
        Gridly.RowBackColorGetter = row => row is DocumentationFeatureRow r && r.Status == "Fail" ? (Color?)Color.FromArgb(255, 235, 238) : null;
        Gridly.RowForeColorGetter = row => row is DocumentationFeatureRow r && r.Status == "Fail" ? (Color?)Color.FromArgb(140, 0, 0) : null;
        Gridly.ConditionalFormats.Clear();
        Gridly.ConditionalFormats.Add(new global::Gridly.Formatting.GridlyConditionalFormat
        {
            Column = Gridly.Columns[nameof(DocumentationFeatureRow.Status)],
    ...
    private void ConfigureCardVisuals()
    {
        ...
        Gridly.CardVisualAdornments = true;
        Gridly.SetViewMode(GridlyViewMode.DashboardCard);
        Gridly.SetObjects(CreateRows("Card Visual", 28));
    }

    private void ConfigureCardActions()
    {
        AddStandardColumns(includeAction: true);
        Gridly.SetViewMode(GridlyViewMode.LargeCard);
        Gridly.SetObjects(CreateRows("Card Action", 30));
        Gridly.ButtonClick += (_, e) => Info.Text = $"Kart aksiyonu: {((DocumentationFeatureRow)e.RowObject).Name}";
        Gridly.HyperlinkClick += (_, e) => Info.Text = $"Kart linki: {((DocumentationFeatureRow)e.RowObject).Link}";
    }

    private void ConfigureCheckboxLayout()
    {
        Gridly.Columns.Add(new GridlyColumn("Person", nameof(DocumentationFeatureRow.Name), 210) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Checked) });
        Gridly.Columns.Add(new GridlyColumn("Operasyon", nameof(DocumentationFeatureRow.Category), 160) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Approved) });
        Gridly.Columns.Add(new GridlyColumn("Onay", nameof(DocumentationFeatureRow.Status), 120) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Locked) });
        Gridly.Columns.Add(new GridlyColumn("İlerleme", nameof(DocumentationFeatureRow.Progress), 130) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.Columns.Add(new GridlyColumn("Notlar", nameof(DocumentationFeatureRow.Notes), 420) { FillFreeSpace = true });
        Gridly.KeyboardSpaceTogglesCheckBoxes = true;
        Gridly.SetObjects(CreateRows("Checkbox", 40));
    }
    ...
    private void ConfigureRowHeight()
    {
        ...
        Gridly.MaxCellTextLines = 3;
        Gridly.DetailsRowHeight = 44;
        Gridly.SetObjects(CreateRows("Row Height", 45, longNotes: true));
    ...
    private void ConfigureVirtualMode(int count, string label)
    {
        ...
        Gridly.SetVirtualProvider(new VirtualDemoRowProvider(count));
    ...
    {
        AddStandardColumns();
        Gridly.DetailsRowHeight = 38;
        Gridly.AllowMultilineCells = true;
        Gridly.MaxCellTextLines = 2;
        Gridly.SetObjects(CreateRows("High DPI", 60));
    ...
    {
        Gridly.Columns.Add(new GridlyColumn("Sabit Id", nameof(DocumentationFeatureRow.Id), 88));
        Gridly.Columns.Add(new GridlyColumn("Sabit Durum", nameof(DocumentationFeatureRow.Status), 120) { Kind = GridlyColumnKind.Badge });
        Gridly.Columns.Add(new GridlyColumn("Serbest Ad", nameof(DocumentationFeatureRow.Name), 220) { FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Serbest Notlar", nameof(DocumentationFeatureRow.Notes), 320) { FillFreeSpace = true });
        Gridly.Columns.Add(new GridlyColumn("Progress", nameof(DocumentationFeatureRow.Progress), 130) { Kind = GridlyColumnKind.ProgressBar });
        Gridly.SetObjects(CreateRows("FixedFree", 55));
    ...
    {
        AddCapabilityColumns();
        Gridly.EnableSearchEverywhere = true;
        Gridly.EnableCommandPalette = true;
        Gridly.ShowQuickFilterBar = true;
        Gridly.SetObjects(new[]
        {
    ...
    private void ConfigureInlineEditing()
    {
        ...
        foreach (var column in Gridly.Columns)
        ...
        Gridly.SetObjects(CreateRows("Inline Edit", 50));
    ...
    private void ConfigureDragAndDrop()
    {
        ...
        Gridly.AllowRowDragDrop = true;
        Gridly.ShowDropIndicator = true;
        Gridly.SetObjects(CreateRows("Drag Drop", 50));
    ...
    {
        AddStandardColumns(includeSparkline: true);
        Gridly.SetViewMode(GridlyViewMode.MasterDetail);
        Gridly.SetObjects(CreateRows("Sync", 42));
    ...
    private static List<DocumentationFeatureRow> CreateRows(string prefix, int count, bool longNotes = false)
    {
        ...
        return Enumerable.Range(1, count).Select(i =>
        {
            ...
            string notes = longNotes
                ...
                : $"{prefix} senaryosu için Gridly dokümantasyon satırı {i}.";
...
public static class DocumentationMissingScreenshotScenarios
{
    ...
    {
        new("kolon-sistemi", "Kolon Sistemi", "5. Kolon Sistemi", "kolon-sistemi", "GridlyColumn türleri, header/cell checkbox, badge, progress, rating, button ve hyperlink kolonlarını gösterir."),
