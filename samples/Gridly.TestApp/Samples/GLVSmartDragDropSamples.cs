...
public static class GLVSmartDragDropSamples
{
    ...
    public static FastGridlyView CreateSmartDragDropGrid()
    {
        var grid = new FastGridlyView
        {
            ...
            UserLayoutKey = "Gridly.TestApp.SmartDragDrop"
        };
        ...
        grid.RowDropValidating += (_, e) =>
        {
            ...
            if (e.TargetViewIndex < 0) e.Position = GridlyDropPosition.After;
        };
        ...
        {
            if (e.Kind == GridlyExternalDropKind.Files && e.Payload is string[] files)
                MessageBox.Show("Bırakılan dosya sayısı: " + files.Length);
            else if (e.Kind == GridlyExternalDropKind.Text)
