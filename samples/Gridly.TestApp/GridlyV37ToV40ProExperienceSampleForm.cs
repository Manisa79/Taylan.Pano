...
public sealed class GridlyV37ToV40ProExperienceSampleForm : Form
{
    ...
    public GridlyV37ToV40ProExperienceSampleForm()
    {
        Text = "Gridly v37-v40 Pro Experience - Layout + Performance + Interaction + Analytics";
        Width = 1360;
        Height = 820;
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new GridlyColumn("Tip", nameof(ProExperienceRow.Type), 120) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Kod", nameof(ProExperienceRow.Code), 130));
        _grid.Columns.Add(new GridlyColumn("Başlık", nameof(ProExperienceRow.Title), 260) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new GridlyColumn("Sahip", nameof(ProExperienceRow.Owner), 130) { AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(ProExperienceRow.Status), 120) { Kind = GridlyColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new GridlyColumn("Değer", nameof(ProExperienceRow.Value), 90) { Kind = GridlyColumnKind.ProgressBar });
        _grid.Columns.Add(new GridlyColumn("Açıklama", nameof(ProExperienceRow.Detail), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }
    ...
    private void ApplyPhase(int phase)
    {
        switch (phase)
        {
            case 37:
                ...
                break;
            case 38:
                _grid.ApplyV38PerformanceProfile(GridlyV38PerformancePreset.MediaLibrary);
            ...
            case 39:
                ...
                break;
            case 40:
                _grid.ApplyV40AnalyticsProfile(GridlyV40AnalyticsPreset.FactoryOverview);
    ...
    private void LoadRows()
    {
        ...
        for (int i = 1; i <= 80; i++)
        {
            rows.Add(new ProExperienceRow
            {
                ...
                Title = "Gridly Pro Experience faz örneği " + i,
