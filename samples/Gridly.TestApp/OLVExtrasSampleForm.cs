...
public sealed class OLVExtrasSampleForm : Form
{
    ...
    {
        Text = "Gridly OLV Ekstra Uyumluluk ve Faydalı API Örneği";
        Width = 1200;
        Height = 720;
        StartPosition = FormStartPosition.CenterScreen;

        _grid.Columns.Add(new GridlyColumn("Person", nameof(OlvExtraRow.Person), 180));
        _grid.Columns.Add(new GridlyColumn("Occupation", nameof(OlvExtraRow.Occupation), 140));
        _grid.Columns.Add(new GridlyColumn("Status", nameof(OlvExtraRow.Status), 120));
        _grid.Columns.Add(new GridlyColumn("Key", nameof(OlvExtraRow.Key), 90));
        _grid.Columns.Add(new GridlyColumn("Notes", nameof(OlvExtraRow.Notes), 460) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
