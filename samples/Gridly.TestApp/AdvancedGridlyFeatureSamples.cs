...
using Gridly.Presets;

namespace Gridly.TestApp;

public sealed class ExcelFilterRowSampleForm : Form
{
    private readonly GridlyView _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, FastFilterMenuForHugeLists = true, AsyncLoadFullFilterValues = true };
    private GridlyExcelFilterRowPanel? _filterRow;
    ...
    public ExcelFilterRowSampleForm()
    {
        ...
        _grid.Columns.Add(new GridlyColumn("Barkod", "Barcode", 160) { FillFreeSpace = true });
        _grid.Columns.Add(new GridlyColumn("Makine", "Machine", 120));
        _grid.Columns.Add(new GridlyColumn("Sonuç", "Result", 90) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Aktif", "Active", 70) { Kind = GridlyColumnKind.CheckBox });
        _grid.Columns.Add(new GridlyColumn("Puan", "Score", 80) { Kind = GridlyColumnKind.Numeric, TextAlign = ContentAlignment.MiddleRight });
        _grid.Columns.Add(new GridlyColumn("Tarih", "TestDate", 150) { Kind = GridlyColumnKind.Date });
        _grid.SetObjects(CreateRows(2000));

        _filterRow = new GridlyExcelFilterRowPanel(_grid);
...
{
    private readonly GridlyView _master = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
    private readonly GridlyView _detail = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, EmptyListMessage = "Üstten kayıt seçin" };
    private readonly List<TestResultRow> _masters;
    private readonly List<FailDetailRow> _details;

    public MasterDetailSampleForm()
    {
        Text = "Master-detail Gridly";
        Width = 1280;
        Height = 760;
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
        ...
        GridlyPresets.ApplyAoiFailList(_master);
        _master.Columns.Clear();
        _master.Columns.Add(new GridlyColumn("Id", "Id", 70));
        _master.Columns.Add(new GridlyColumn("Barkod", "Barcode", 150) { FillFreeSpace = true });
        _master.Columns.Add(new GridlyColumn("Makine", "Machine", 120));
        _master.Columns.Add(new GridlyColumn("Program", "AssemblyName", 220) { FillFreeSpace = true });
        _master.Columns.Add(new GridlyColumn("Sonuç", "Result", 90) { Kind = GridlyColumnKind.Badge });
        _master.Columns.Add(new GridlyColumn("Tarih", "TestDate", 150));
        _master.FrozenColumnCount = 2;

        _detail.Columns.Add(new GridlyColumn("FailId", "Id", 70));
        _detail.Columns.Add(new GridlyColumn("Layer", "Layer", 80));
        _detail.Columns.Add(new GridlyColumn("Board", "Board", 80));
        _detail.Columns.Add(new GridlyColumn("RefDes", "RefDes", 120));
        _detail.Columns.Add(new GridlyColumn("Feature", "Feature", 160) { FillFreeSpace = true });
        _detail.Columns.Add(new GridlyColumn("PassRate", "PassRate", 100) { Kind = GridlyColumnKind.ProgressBar });
        _detail.Columns.Add(new GridlyColumn("Decision", "Decision", 120) { Kind = GridlyColumnKind.ComboBox, Editable = true, ComboBoxItems = new List<string>{"Bekliyor", "FalseCall", "Gerçek Hata"} });
...
{
    private readonly GridlyView _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, EnableRowDetails = true };
    ...
    public FrozenCommandDetailsSampleForm()
    {
        ...
        _grid.Columns.Add(new GridlyColumn("Aç", "Open", 70) { Kind = GridlyColumnKind.Button, AspectGetter = _ => "Aç" });
        _grid.Columns.Add(new GridlyColumn("Barkod", "Barcode", 160) { FillFreeSpace = true });
        _grid.Columns.Add(new GridlyColumn("Makine", "Machine", 120));
        _grid.Columns.Add(new GridlyColumn("Sonuç", "Result", 90) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Skor", "Score", 100) { Kind = GridlyColumnKind.ProgressBar });
        _grid.Columns.Add(new GridlyColumn("Not", "Note", 240) { FillFreeSpace = true });
        ...
        _grid.SetRowDetailsProvider(new GridlyRowDetailsProvider
        {
...
{
    private readonly GridlyView _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
    ...
    public ConditionalFormattingSampleForm()
    {
        ...
        _grid.Columns.Add(new GridlyColumn("Sonuç", "Result", 90) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Aktif", "Active", 70) { Kind = GridlyColumnKind.CheckBox });
        _grid.Columns.Add(new GridlyColumn("Skor", "Score", 110) { Kind = GridlyColumnKind.ProgressBar });
        _grid.Columns.Add(new GridlyColumn("Tarih", "TestDate", 150));
        ...
        _grid.ConditionalFormats.Add(new GridlyConditionalFormat
        {
        ...
        var tool = new ToolStrip { Dock = DockStyle.Top, GripStyle = ToolStripGripStyle.Hidden };
        tool.Items.Add(new ToolStripButton("Sadece FAIL", null, (_,__) => _grid.SetColumnFilter(new GridlyColumnFilter { AspectName = "Result", Mode = GridlyFilterMode.Equals, Text = "FAIL" })));
