...
public sealed class GridlyV27ProductSampleForm : Form
{
    ...
    private string StateFile => Path.Combine(Application.StartupPath, "gridly-v27-product-state.json");

    public GridlyV27ProductSampleForm()
    {
        Text = "Gridly v27 - Product Core / MasterData Hazırlığı";
        Width = 1320;
        Height = 820;
        ...
        LoadMasterDataRows();
        _grid.ApplyScenario(GridlyViewScenario.BomPositions);
    ...
    {
        _grid.Columns.Add(new GridlyColumn("Seç", nameof(V27Row.Selected), 52) { Kind = GridlyColumnKind.CheckBox, CellCheckBox = true, HeaderCheckBox = true });
        _grid.Columns.Add(new GridlyColumn("Id", nameof(V27Row.Id), 70));
        _grid.Columns.Add(new GridlyColumn("Tip", nameof(V27Row.Type), 105).ApplyVisualProfile(GridlyCellVisualProfile.Badge));
        _grid.Columns.Add(new GridlyColumn("Kod", nameof(V27Row.Code), 150));
        _grid.Columns.Add(new GridlyColumn("Açıklama", nameof(V27Row.Name), 280) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new GridlyColumn("Makine", nameof(V27Row.Machine), 110));
        _grid.Columns.Add(new GridlyColumn("RefDes", nameof(V27Row.RefDes), 130));
        _grid.Columns.Add(new GridlyColumn("İlerleme", nameof(V27Row.Progress), 95).ApplyVisualProfile(GridlyCellVisualProfile.Progress));
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(V27Row.Status), 115).ApplyVisualProfile(GridlyCellVisualProfile.WarningStatus));
        _grid.Columns.Add(new GridlyColumn("Not", nameof(V27Row.Note), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }

    private void ConfigureToolbar()
    {
        _tool.Items.Add(new ToolStripButton("MasterData BOM", null, (_, __) => ApplyScenario(GridlyViewScenario.BomPositions)));
        _tool.Items.Add(new ToolStripButton("Ticket Dashboard", null, (_, __) => ApplyScenario(GridlyViewScenario.TicketBoard)));
        _tool.Items.Add(new ToolStripButton("Timeline", null, (_, __) => ApplyScenario(GridlyViewScenario.Timeline)));
    ...
    private void ApplyScenario(GridlyViewScenario scenario)
    {
        ...
        if (scenario == GridlyViewScenario.TicketBoard)
            _grid.SetGroupBy(nameof(V27Row.Status));
        else if (scenario == GridlyViewScenario.Timeline)
    ...
    private void LoadMasterDataRows()
    {
        ...
        for (int i = 1; i <= 180; i++)
        {
            ...
            _rows.Add(new V27Row
            {
                ...
                Note = "MasterData için BOM, SAP ürün ağacı, program dosyası ve pozisyon kontrolü aynı Gridly üzerinde denenir."
            });
        }
    }
    ...
    private void LoadRangeVirtualDemo()
    {
        ...
        var provider = new GridlyRangeVirtualProvider(
            () => total,
        ...
        _grid.ApplyScenario(GridlyViewScenario.DenseData);
