...
public sealed class GridlyV51RealUsagePilotSampleForm : Form
{
    ...
    public GridlyV51RealUsagePilotSampleForm()
    {
        Text = "Gridly 5.1 - Audix Pilot + Theme Audit + Example Cleanup";
        Width = 1360;
        Height = 820;
        ...
        LoadRows();
        ApplyTheme(true);
        UpdateDetails("Gridly 5.1 gerçek kullanım pilotu açıldı.");
    }
    ...
    {
        _grid.Columns.Clear();
        var cover = new GridlyColumn("Kapak", nameof(AudixTrackRow.Cover), 96) { Kind = GridlyColumnKind.Image };
        cover.ImageGetter = row => (row as AudixTrackRow)?.Cover;
        _grid.Columns.Add(cover);
        _grid.Columns.Add(new GridlyColumn("Şarkı", nameof(AudixTrackRow.Title), 240) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new GridlyColumn("Sanatçı", nameof(AudixTrackRow.Artist), 170));
        _grid.Columns.Add(new GridlyColumn("Albüm", nameof(AudixTrackRow.Album), 180));
        _grid.Columns.Add(new GridlyColumn("Tür", nameof(AudixTrackRow.MediaKind), 80) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Kalite", nameof(AudixTrackRow.Quality), 80) { Kind = GridlyColumnKind.Badge });
        _grid.Columns.Add(new GridlyColumn("Durum", nameof(AudixTrackRow.PlaybackState), 110) { Kind = GridlyColumnKind.Badge });
    }

    private void ConfigureToolbar()
    {
        _tool.Items.Add(new ToolStripButton("Poster", null, (_, __) => SetMode(GridlyViewMode.Poster)));
        _tool.Items.Add(new ToolStripButton("MediaTile", null, (_, __) => SetMode(GridlyViewMode.MediaTile)));
        _tool.Items.Add(new ToolStripButton("Gallery", null, (_, __) => SetMode(GridlyViewMode.Gallery)));
        _tool.Items.Add(new ToolStripButton("FilmStrip", null, (_, __) => SetMode(GridlyViewMode.FilmStrip)));
    ...
    private void ConfigureMedia()
    {
        ...
        _grid.MediaKindGetter = row => (row as AudixTrackRow)?.MediaKind ?? GridlyMediaKind.Audio;
        _grid.MediaPlaybackStateGetter = row => (row as AudixTrackRow)?.PlaybackState ?? GridlyMediaPlaybackState.None;
    ...
    private void SetMode(GridlyViewMode mode)
    {
        ...
        _grid.TilePosterMode = mode is GridlyViewMode.Poster or GridlyViewMode.Gallery or GridlyViewMode.MediaTile or GridlyViewMode.FilmStrip;
        UpdateDetails($"Görünüm modu değişti: {mode}. Audix'te albüm kapağı için en iyi modlar Poster / MediaTile / Gallery / FilmStrip.");
    }

    private void TogglePlayback(AudixTrackRow? clicked, GridlyMediaPlaybackState currentState)
    {
        if (clicked == null) return;

        foreach (var row in _rows)
        {
            if (!ReferenceEquals(row, clicked) && row.PlaybackState == GridlyMediaPlaybackState.Playing)
                row.PlaybackState = GridlyMediaPlaybackState.None;
        }

        clicked.PlaybackState = currentState == GridlyMediaPlaybackState.Playing
            ? GridlyMediaPlaybackState.Paused
            : GridlyMediaPlaybackState.Playing;

        UpdateDetails(clicked.MediaKind == GridlyMediaKind.Video
    ...
    {
        _grid.ApplyTheme51AuditDefaults(_dark ? GridlyThemeStudioPreset.AudixDark : GridlyThemeStudioPreset.AudixLight);
    ...
    {
        var lines = _grid.RunGridly51UsageChecks()
            .Select(c => (c.Passed ? "✓ " : "! ") + c.Profile + " / " + c.Check + " - " + c.Message);
        MessageBox.Show(this, string.Join(Environment.NewLine, lines), "Gridly 5.1 Usage Check", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }

    private void ApplyTheme(bool dark)
    {
        _dark = dark;
        var preset = dark ? GridlyThemeStudioPreset.AudixDark : GridlyThemeStudioPreset.AudixLight;
        ...
        _header.Text = dark
            ? "Gridly 5.1 Audix Pilot - Koyu tema okunurluk testi"
            : "Gridly 5.1 Audix Pilot - Açık tema okunurluk testi";
    }

    private void LoadRows()
    {
        _rows.Clear();
        _rows.Add(new AudixTrackRow("Me Malone O Pateras Mou", "Antzela Dimitriou", "Greatest Hits", GridlyMediaKind.Audio, "MP3", "03:28", CreateCover(Color.SteelBlue, "A1")));
        _rows.Add(new AudixTrackRow("Mia Litropi Agapi", "Antzela Dimitriou", "Greatest Hits", GridlyMediaKind.Audio, "FLAC", "04:12", CreateCover(Color.MediumPurple, "A2")) { PlaybackState = GridlyMediaPlaybackState.Playing });
        _rows.Add(new AudixTrackRow("Na Giriseis Xana", "Antzela Dimitriou", "This Is Greece", GridlyMediaKind.Audio, "320", "03:57", CreateCover(Color.SeaGreen, "A3")));
        _rows.Add(new AudixTrackRow("Factory Clip 01", "Video Archive", "Factory Videos", GridlyMediaKind.Video, "1080p", "01:14", CreateCover(Color.DarkOrange, "V1")));
        _rows.Add(new AudixTrackRow("AOI Defect Review", "Inspection Video", "AOI Review", GridlyMediaKind.Video, "4K", "00:48", CreateCover(Color.Crimson, "V2")));
        _rows.Add(new AudixTrackRow("Kapak Eksik Örneği", "Unknown Artist", "Missing Covers", GridlyMediaKind.Audio, "MISSING", "02:49", CreatePlaceholderCover()));
    ...
    private void UpdateSelectionDetails()
    {
        ...
        UpdateDetails("Seçilen medya" + Environment.NewLine +
            ...
            "Audix entegrasyon notu: Modelde Cover/Image property, PlaybackState, Quality ve MediaKind alanları olursa Gridly medya kartı state'i kendi çizebilir.");
    }

    private void UpdateDetails(string message)
    {
        _details.Text = message + Environment.NewLine + Environment.NewLine +
            "Gridly 5.1 hedefi:" + Environment.NewLine +
    ...
    private static Bitmap CreateCover(Color baseColor, string text)
    {
        ...
        TextRenderer.DrawText(g, "AUDIX / GRIDLY", small, new Rectangle(0, 394, bmp.Width, 32), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
    ...
    {
        public AudixTrackRow(string title, string artist, string album, GridlyMediaKind mediaKind, string quality, string duration, Image cover)
        {
            Title = title;
            Artist = artist;
            Album = album;
