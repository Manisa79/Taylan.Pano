...
public sealed class GridlyColumnCompatibilitySampleForm : Form
{
    private readonly GridlyView _grid = new()
    {
        Dock = DockStyle.Fill,
        EmptyListMessage = "Gridly kolon uyumluluk örneği boş",
    ...
    private readonly Label _info = new()
    {
        Dock = DockStyle.Fill,
        AutoEllipsis = true,
        Text = "Gridly uyumlu checkbox görünümü: CheckBoxes=True yeni kolon eklemez; checkbox ve header checkbox ilk görünür veri kolonunun içinde çalışır. Header checkbox tüm görünen satırları işaretler/kaldırır."
    };
    ...
    public GridlyColumnCompatibilitySampleForm()
    {
        Text = "Gridly Uyumlu Kolon / Checkbox Örneği";
        Width = 1180;
        Height = 720;
    ...
    private void BuildToolbar()
    {
        ...
        _tool.Items.Add(new ToolStripButton("Light", null, (_, __) => ApplyTheme(Gridly.Theming.GridlyTheme.LightTheme())));
        _tool.Items.Add(new ToolStripButton("Dark", null, (_, __) => ApplyTheme(Gridly.Theming.GridlyTheme.DarkTheme())));
    }
    ...
    private void ConfigureColumns()
    {
        ...
        _grid.Columns.Add(new GridlyColumn("Person", nameof(CompatRow.Person), 220)
        {
            HeaderCheckBox = true,
        ...
        _grid.Columns.Add(new GridlyColumn("Occupation", nameof(CompatRow.Occupation), 150)
        {
        ...
        _grid.Columns.Add(new GridlyColumn("Action", nameof(CompatRow.Action), 110)
        {
            Kind = GridlyColumnKind.Button,
            ButtonSizing = GridlyColumnButtonSizing.TextBounds,
            ButtonMaxWidth = 90,
            ButtonPadding = new Padding(2),
            EnableButtonWhenItemIsDisabled = true
        });

        _grid.Columns.Add(new GridlyColumn("Link", nameof(CompatRow.Url), 170)
        {
            Hyperlink = true,
            HeaderTextAlign = ContentAlignment.MiddleCenter
        });

        _grid.Columns.Add(new GridlyColumn("Notes", nameof(CompatRow.Notes), 260)
        {
            WordWrap = true,
        ...
        _grid.Columns.Add(new GridlyColumn("Runtime Hidden", nameof(CompatRow.HiddenValue), 120)
        {
            Visible = false,
    ...
    private void LoadRows()
    {
        var rows = Enumerable.Range(1, 100).Select(i => new CompatRow
        {
            ...
            Action = "Aç",
            Url = "Detay " + i,
            Notes = "Gridly kolon property uyumluluğu test satırı " + i,
    ...
    private void CycleButtonSizing()
    {
        ...
        {
            GridlyColumnButtonSizing.TextBounds => GridlyColumnButtonSizing.CellBounds,
            GridlyColumnButtonSizing.CellBounds => GridlyColumnButtonSizing.FixedBounds,
            _ => GridlyColumnButtonSizing.TextBounds
        };

        _grid.RefreshView();
    }

    private void ApplyTheme(Gridly.Theming.GridlyTheme theme)
    {
