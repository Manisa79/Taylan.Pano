# Gridly v32.0 Ultimate Experience Roadmap Suite
...
- **Faz 40 Timeline Engine**: ticket, makine ve işlem geçmişi için `GridlyTimelineEvent` ve Timeline örneği.
- **Faz 41 Document Explorer**: PDF/CAD/video/audio/image/folder önizleme sınıflandırması için `GridlyDocumentPreviewKind`, Gallery örneği.
...
- **Faz 46 Dashboard Builder**: `DashboardWidgets`, `GridlyDashboardWidgetDefinition`, KPI/HeatMap/Timeline/Gallery/Kanban widget tipleri.
- **Faz 47 AI Layer**: host uygulama önerileri için `GridlyAiInsight`, `AddAiInsight`.
- **Faz 48 Gridly Ecosystem**: GridlyKanban/GridlyTimeline/GridlyDashboard gibi ortak çekirdek yaklaşımı için birleşik property seti.
...
gridly.EnableMediaLazyLoading = true;
gridly.EnableMediaImageCache = true;
```
