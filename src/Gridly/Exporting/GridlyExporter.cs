...
public static class GridlyExporter
{
    public static string ToCsv(IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, char separator = ';')
    {
    ...
    /// </summary>
    public static string SaveCsv(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, char separator = ';')
    {
    ...
    public static string ToHtmlTable(IEnumerable<GridlyColumn> columns, IEnumerable<object> rows)
    {
        var cols = columns.Where(c => !c.PrivateColumn && c.Visible).ToList();
        var sb = new StringBuilder();
        sb.AppendLine("<table>");
    ...
    public static string ToJson(IEnumerable<GridlyColumn> columns, IEnumerable<object> rows)
    {
        var cols = columns.Where(c => !c.PrivateColumn && c.Visible).ToList();
    ...
    public static string SaveJson(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.json");
    ...
    /// For card/dashboard output use the GridlyPdfExportOptions overload with Mode=Card.
    /// </summary>
    public static string SavePdf(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, string title = "GridlyView")
    {
        return SavePdf(path, columns, rows, new GridlyPdfExportOptions { Title = title, Mode = GridlyPdfExportMode.Table });
    }
    ...
    /// </summary>
    public static string SavePdf(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, GridlyPdfExportOptions options)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.pdf");
    ...
    private static byte[] BuildStyledPdf(IReadOnlyList<GridlyColumn> columns, IReadOnlyList<object> rows, GridlyPdfExportOptions options)
    {
        int pageWidth = options.Orientation == GridlyPdfPageOrientation.Landscape ? 842 : 595;
        int pageHeight = options.Orientation == GridlyPdfPageOrientation.Landscape ? 595 : 842;
        var pages = new List<PdfPage>();
        var mode = options.Mode == GridlyPdfExportMode.Auto ? GridlyPdfExportMode.Table : options.Mode;
        if (mode == GridlyPdfExportMode.Card)
    ...
    private static void RenderPdfTable(List<PdfPage> pages, int pageWidth, int pageHeight, IReadOnlyList<GridlyColumn> columns, IReadOnlyList<object> rows, GridlyPdfExportOptions options)
    {
    ...
    private static void RenderPdfCards(List<PdfPage> pages, int pageWidth, int pageHeight, IReadOnlyList<GridlyColumn> columns, IReadOnlyList<object> rows, GridlyPdfExportOptions options)
    {
        int left = Math.Max(18, options.MarginLeft);
        int right = Math.Max(18, options.MarginRight);
        int top = Math.Max(18, options.MarginTop);
    ...
    private static PdfPage NewPdfPage(List<PdfPage> pages, int pageWidth, int pageHeight, GridlyPdfExportOptions options, int pageNo)
    {
        ...
        {
            DrawPdfText(page, options.Title ?? "Gridly Export", options.MarginLeft, pageHeight - 30, 13, Color.FromArgb(20, 30, 45), true);
    ...
    private static void DrawPdfCard(PdfPage page, int x, int y, int w, int h, IReadOnlyList<GridlyColumn> columns, object row, GridlyPdfExportOptions options)
    {
        GridlyCardVisualInfo? visual = null;
        ...
        IEnumerable<GridlyColumn> cardColumns = columns.Where(c => c.VisibleInCard).OrderBy(c => c.CardOrder);
        ...
        foreach (GridlyColumn column in cardColumns.Take(6))
        {
        ...
        {
            int bx = x + w - 12;
            foreach (GridlyCardBadge badge in visual.Badges.Take(4))
            {
    ...
    private static int[] ComputePdfColumnWidths(IReadOnlyList<GridlyColumn> columns, int usableWidth, bool fitToPage)
    {
    ...
    private static void DrawPdfFooter(PdfPage page, int pageWidth, int pageHeight, GridlyPdfExportOptions options, int pageNo, int rowCount)
    {
    ...
    private static string GlyphText(GridlyCardGlyph glyph)
    {
        return glyph switch
        {
            GridlyCardGlyph.Warning => "!",
            GridlyCardGlyph.Error => "X",
            GridlyCardGlyph.Success => "OK",
            GridlyCardGlyph.Message => "MSG",
            GridlyCardGlyph.Attachment => "ATT",
            GridlyCardGlyph.Pin => "PIN",
            GridlyCardGlyph.Lock => "LOCK",
            GridlyCardGlyph.Star => "*",
            GridlyCardGlyph.Check => "OK",
            GridlyCardGlyph.Clock => "TIME",
            GridlyCardGlyph.Flag => "FLAG",
            GridlyCardGlyph.Bell => "BELL",
            _ => "i"
        };
    }
    ...
    /// </summary>
    public static string SaveExcelWorkbook(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, string worksheetName = "GridlyView")
    {
    ...
    /// </summary>
    public static string SaveExcelXml(string path, IEnumerable<GridlyColumn> columns, IEnumerable<object> rows, string worksheetName = "GridlyView")
        => SaveExcelWorkbook(path, columns, rows, worksheetName);

    private static string NormalizeExcelPath(string path)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Gridly.xlsx");
    ...
    private static string BuildWorksheetXml(IReadOnlyList<GridlyColumn> cols, IReadOnlyList<object> rows)
    {
        var sb = new StringBuilder();
    ...
    private static string GetWritablePath(string path)
    {
        ...
        throw new UnauthorizedAccessException("GridlyView export için yazılabilir bir klasör bulunamadı. Lütfen farklı bir klasör seçin veya Windows Controlled Folder Access/antivirüs izinlerini kontrol edin.");
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(name)) return "GridlyView";
