...
public sealed class GridlyAdvancedFilterBuilderForm : Form
{
    ...
    private readonly List<GridlyColumn> _columns;
    private readonly BindingSource _source = new();
    private readonly List<GridlyColumnFilterPresetItem> _items = new();

    public GridlyFilterPreset Preset { get; private set; }

    public GridlyAdvancedFilterBuilderForm(IEnumerable<GridlyColumn> columns, GridlyFilterPreset initial, GridlyTheme? theme)
    {
        _columns = columns.Where(x => x != null && !string.IsNullOrWhiteSpace(x.AspectName)).ToList();
        Preset = initial ?? new GridlyFilterPreset();
        ...
        GridlyDialogChrome.ConfigureStandardDialog(this, theme ?? WindowsThemeService.CurrentTheme(), new Size(780, 480), sizeable: true, iconKind: GridlyDialogIconKind.Filter);
    ...
    private void BuildLayout()
    {
        ...
        _list.DisplayMember = nameof(GridlyColumnFilterPresetItem.DisplayText);
    ...
    private void LoadInitial()
    {
        ...
        _mode.DataSource = Enum.GetValues(typeof(GridlyFilterMode));
        _logic.DataSource = Enum.GetValues(typeof(GridlyFilterLogic));
        _logic.SelectedItem = Preset.Logic;
        _global.Text = Preset.GlobalText ?? string.Empty;
        _items.Clear();
        if (Preset.Filters != null)
            _items.AddRange(Preset.Filters.Select(x => new GridlyColumnFilterPresetItem
            {
    ...
    private void AddCondition()
    {
        ...
        var mode = _mode.SelectedItem is GridlyFilterMode m ? m : GridlyFilterMode.Contains;
        _items.Add(new GridlyColumnFilterPresetItem
        {
            AspectName = aspect,
            Mode = mode,
            Text = _value1.Text,
    ...
    {
        if (_list.SelectedItem is GridlyColumnFilterPresetItem item)
        {
            _items.Remove(item);
            RefreshList();
        }
    ...
    {
        Preset = new GridlyFilterPreset
        {
            Name = Preset.Name,
            Description = Preset.Description,
            Logic = _logic.SelectedItem is GridlyFilterLogic l ? l : GridlyFilterLogic.And,
            GlobalText = _global.Text ?? string.Empty,
            Filters = _items.Select(x => new GridlyColumnFilterPresetItem
            {
                AspectName = x.AspectName,
                Mode = x.Mode,
                Text = x.Text,
