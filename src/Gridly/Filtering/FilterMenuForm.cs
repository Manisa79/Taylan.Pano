...
public sealed class FilterMenuForm : Form
{
    ...
    private readonly GridlyColumn _column;
    private readonly GridlyTheme _theme;
    private List<ValueItem> _allItems;
    private readonly TextBox _search = new() { PlaceholderText = GridlyText.SearchPlaceholder };
    ...
    private readonly CheckBox _selectAll = new() { Text = GridlyText.SelectAll, AutoSize = true, ThreeState = true, AutoCheck = false };
    private readonly CheckBox _useTextFilter = new() { Text = GridlyText.SearchTypedTextInColumn, AutoSize = true };
    ...
    private readonly Button _apply = new() { Text = GridlyText.Apply, Height = 28 };
    ...
    private readonly Button _clearAll = new() { Text = "✕  " + GridlyText.ClearAllFilters, Height = 26 };
    private readonly Button _sortAsc = new() { Text = "↑  " + GridlyText.SortAscending, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _sortDesc = new() { Text = "↓  " + GridlyText.SortDescending, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _unsort = new() { Text = "↕  " + GridlyText.Unsort, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _groupBy = new() { Text = "▦  " + GridlyText.GroupByColumn(""), Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _clearGrouping = new() { Text = "▢  " + GridlyText.ClearGrouping, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    ...
    public FilterMenuForm(GridlyColumn column, IEnumerable<string> distinctValues, GridlyColumnFilter? currentFilter = null, GridlyTheme? theme = null, Func<string, IReadOnlyList<string>>? searchValueLoader = null, bool isFastPreview = false, int searchDebounceMs = 150, bool resizable = true, Size? defaultSize = null, Size? minimumSize = null, Size? maximumSize = null, bool rememberSize = true, string? sizeMemoryKey = null, bool showValueTooltips = true, bool autoWidthForLongValues = true)
    {
        ...
        _theme = theme ?? GridlyTheme.LightTheme();
        ...
        MaximizeBox = false;
        MinimizeBox = false;
        GridlyDialogChrome.ConfigureStandardDialog(this, _theme, MinimumSize, sizeable: resizable, iconKind: GridlyDialogIconKind.Filter);
        Text = GridlyText.ColumnFilterTitle(column.Header);
        _groupBy.Text = "▦  " + GridlyText.GroupByColumn(column.Header);
    ...
    private void BuildLayout()
    {
        ...
        _apply.Width = 80;
        _limitedInfoLabel.Text = GridlyText.FastPreviewLimitedInfo;
    ...
    {
        GridlyDialogChrome.ConfigureOpenGridlyDialogs(this, theme);

        Color formBack = GridlyDialogThemeApplier.NormalizePanelBack(theme);
        Color controlBack = GridlyDialogThemeApplier.NormalizeControlBack(theme);
        Color fore = GridlyDialogThemeApplier.NormalizeControlFore(theme);
        Color panelFore = GridlyDialogThemeApplier.NormalizePanelFore(theme);
        ...
        _limitedInfoPanel.BackColor = GridlyDialogThemeApplier.Blend(controlBack, theme.AccentColor, theme.IsDark ? 0.36 : 0.18);
    ...
    private static void ApplyThemeRecursive(Control parent, GridlyTheme theme, Color panelBack, Color controlBack, Color panelFore, Color controlFore)
    {
        foreach (Control control in parent.Controls)
        {
            switch (control)
            {
                ...
                case Button button:
                    ...
                    Color hoverBack = GridlyDialogThemeApplier.Blend(controlBack, theme.AccentColor, theme.IsDark ? 0.24 : 0.10);
    ...
    private void RestoreChecks(GridlyColumnFilter? currentFilter)
    {
        bool hasValueFilter = currentFilter?.Mode == GridlyFilterMode.ValueList && currentFilter.SelectedValues != null;
    ...
    private void ApplyAndClose()
    {
        ...
        else if (!_userChangedChecks && !string.IsNullOrWhiteSpace(searchText))
        {
            ...
            if (exact != null)
            {
                ...
                {
                    AspectName = _column.AspectName,
                    Mode = GridlyFilterMode.ValueList,
        ...
        else
        {
            ...
            // the applied filter must be exact-value only. This matches Excel/Gridly user expectation
    ...
    private GridlyColumnFilter? TryCreateExplicitFilter(string modeName, string searchText)
    {
        GridlyFilterMode? mode = modeName switch
        {
            "Contains" => GridlyFilterMode.Contains,
            "StartsWith" => GridlyFilterMode.StartsWith,
            "Equals" => GridlyFilterMode.Equals,
            "Blanks" => GridlyFilterMode.IsEmpty,
            "Non-Blanks" => GridlyFilterMode.IsNotEmpty,
            "GreaterThan" => GridlyFilterMode.GreaterThan,
            "LessThan" => GridlyFilterMode.LessThan,
            "Between" => GridlyFilterMode.Between,
            _ => null
        };
        if (mode == null) return null;
        if (mode.Value == GridlyFilterMode.IsEmpty || mode.Value == GridlyFilterMode.IsNotEmpty)
        {
            return new GridlyColumnFilter { AspectName = _column.AspectName, Mode = mode.Value, Enabled = true };
        }
