...
namespace Gridly.Tree;

public enum TreeGridlySearchBehavior
{
    SelectOnly,
    ExpandAncestors,
...
public partial class TreeGridlyView
{
    ...
    [Category("Gridly Tree - UX v27.8")]
    [DefaultValue(TreeGridlySearchBehavior.ExpandAncestors)]
    public TreeGridlySearchBehavior TreeSearchBehavior { get; set; } = TreeGridlySearchBehavior.ExpandAncestors;
    ...
    public bool ApplyTreeSearch(string text)
    {
        ...
        if (TreeSearchExpandsAncestors || TreeSearchBehavior != TreeGridlySearchBehavior.SelectOnly)
            ExpandAncestors(match);

        if (TreeSearchBehavior == TreeGridlySearchBehavior.ExpandAncestorsAndDescendants)
