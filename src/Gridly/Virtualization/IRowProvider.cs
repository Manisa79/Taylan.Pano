...
/// <summary>
/// Optional provider hook: huge/async providers can notify GridlyView when a cached page is ready.
/// </summary>
...
/// <summary>
/// Optional fast path for huge virtual/query providers. It lets GridlyView update or summarize
/// checkbox state without materializing millions of rows on the UI thread.
/// </summary>
public interface IBulkCheckStateProvider
{
    bool TrySetAllCheckStates(GridlyColumn column, CheckState state);
    bool TryGetCheckStateSummary(GridlyColumn column, out int checkedCount, out int uncheckedCount);
}

/// <summary>
/// Optional fast path for huge/virtual data sources. If implemented, GridlyView can ask the
...
{
    bool TryBuildViewIndexes(GridlyFilterSet filters, GridlyColumn[] columns, GridlyColumn? sortColumn, bool sortDescending, int maxFallbackScanRows, out IReadOnlyList<int> indexes);
}
...
public sealed class ListRowProvider : IIndexedRowProvider
{
    ...
    /// </summary>
    public bool TryBuildViewIndexes(GridlyFilterSet filters, GridlyColumn[] columns, GridlyColumn? sortColumn, bool sortDescending, int maxFallbackScanRows, out IReadOnlyList<int> indexes)
    {
