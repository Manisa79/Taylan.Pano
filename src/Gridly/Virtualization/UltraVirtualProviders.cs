...
namespace Gridly.Virtualization;

/// <summary>
/// Server/query backed provider contract. GridlyView asks the provider to apply the
...
public interface IQueryRowProvider : IRowProvider
{
    ...
    void ApplyView(GridlyFilterSet filters, GridlyColumn[] columns, GridlyColumn? sortColumn, bool sortDescending);
    bool TryGetDistinctValues(GridlyColumn column, GridlyFilterSet filters, GridlyColumn[] columns, int maxValues, string? searchText, out IReadOnlyList<string> values);
}
...
public sealed class DbQueryRowProvider : IQueryRowProvider, IAsyncPreloadRowProvider, IDisposable
{
    ...
    private void AppendWhere(StringBuilder sql, List<(string name, object? value)> parameters, GridlyFilterSet filters, GridlyColumn[] columns, string? ignoredAspectName)
    {
    ...
    private static bool IsKnownColumn(string? name, GridlyColumn[] columns)
