...
public enum GridlyEmptyStateSignatureAlignment
{
    BottomRight,
    BottomLeft,
    TopRight,
...
public enum GridlyTileCheckBoxPosition
{
    TopLeft,
    TopRight,
    BottomLeft,
...
public enum GridlyTileCheckBoxVisibilityMode
{
    Always,
    Hover,
    Selected,
...
public enum GridlyMediaImageScaleMode
{
    Contain,
    Cover,
    Stretch
...
public enum GridlyDetailCardLayout
{
    Standard,
    Compact,
    PropertyGrid,
...
[ToolboxBitmap(typeof(GridlyView), "GridlyView.bmp")]
[Description("Gridly / GridlyView: Gridly List View modern WinForms list/grid control.")]
public partial class GridlyView : Control
{
    ...
    private GridlyColumn? _activeColumn;
    private readonly SortedSet<int> _selectedRows = new();
    private readonly HashSet<object> _checkedRows = new();
    private readonly Dictionary<GridlyColumn, HashSet<object>> _internalCheckRowsByColumn = new();
    private int _selectionAnchorRow = -1;
    private GridlyColumn? _resizingColumn;
    private int _resizeStartX;
    private int _resizeStartWidth;
    private GridlyColumn? _dragColumn;
    ...
    private readonly GridlyFilterSet _filters = new();
    private readonly List<int> _viewIndexes = new();
    private readonly List<GridlyDisplayRow> _displayRows = new();
    private readonly HashSet<string> _collapsedGroups = new(StringComparer.CurrentCultureIgnoreCase);
    private GridlyColumn? _sortColumn;
    private bool _sortDesc;
    private Control? _editor;
    private ICellEditor? _activeCellEditor;
    private (int row, GridlyColumn col)? _editingCell;
    private GridlyTheme _theme = WindowsThemeService.CurrentTheme();
    private bool _applyingTheme;
    private readonly List<GridlyConditionalFormat> _conditionalFormats = new();
    private readonly List<GridlySummaryItem> _summaries = new();
    private readonly GridlyUndoService _undo = new();
    private GridlyRowDetailsProvider? _detailsProvider;
    private int _expandedRow = -1;
    private Control? _detailsControl;
    private bool _viewIsDirect;
    private int _dataVersion;
    private Dictionary<GridlyColumn, string>? _summaryTextCache;
    private readonly GridlyRenderOptions _renderOptions = new();
    ...
    private GridlyMenuDismissMessageFilter? _activeMenuDismissFilter;
    ...
    private GridlyViewMode _viewMode = GridlyViewMode.Details;
    private GridlyMode _mode = GridlyMode.Object;
    ...
    public GridlyView()
    {
        _menuOptions = new GridlyMenuCustomizationOptions(this);
        _menuIcons = new GridlyMenuIconCustomizationOptions(this);
        MenuIconMode = GridlyMenuIconMode.BuiltInThenCustom;
        SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw | ControlStyles.UserPaint, true);
        Columns = new GridlyColumnCollection();
        ...
        WindowsThemeService.ThemeChanged += (_,__) => { if (FollowWindowsTheme || ThemePreset == GridlyThemePreset.System) ApplySelectedTheme(); };
    ...
    [Category("Gridly")]
    [Description("Gridly kolon koleksiyonu. Designer penceresi GLV tarzı GLVColumn düzenleme sağlar.")]
    [Browsable(false)]
    [Editor(typeof(GridlyColumnCollectionEditor), typeof(UITypeEditor))]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
    public GridlyColumnCollection Columns { get; }

    [DefaultValue(true)] public bool FollowWindowsTheme { get; set; } = true;
    private GridlyThemePreset _themePreset = GridlyThemePreset.System;
    [Category("Gridly - Theme"), DefaultValue(GridlyThemePreset.System)]
    public GridlyThemePreset ThemePreset
    {
        get => _themePreset;
        set
        {
            _themePreset = value;
            if (value != GridlyThemePreset.System) FollowWindowsTheme = false;
            ApplySelectedTheme();
        }
    }
    [Category("Gridly - Theme"), DefaultValue(true)]
    ...
    private GridlyDesignTimeThemePreview _designTimeThemePreview = GridlyDesignTimeThemePreview.Auto;
    [Category("Gridly - Design Time"), DefaultValue(GridlyDesignTimeThemePreview.Auto)]
    [Description("Designer önizlemesi için Auto, Light, Dark veya Fluent tema seçimi.")]
    public GridlyDesignTimeThemePreview DesignTimeThemePreview
    {
    ...
    [Category("Gridly - Design Time"), DefaultValue(true)]
    [Description("Visual Studio tasarım zamanında Gridly'nin açık/VS designer uyumlu tema ile çizilmesini sağlar. Runtime temasını etkilemez.")]
    public bool EnableDesignTimeThemeSync { get; set; } = true;

    [Category("Gridly - Design Time"), DefaultValue(false)]
    ...
    [Category("Gridly - Design Time"), DefaultValue(true)]
    [Description("Designer'da Gridly'ye bağlı context menülerin de açık/VS designer uyumlu tema ile çizilmesini sağlar.")]
    ...
    [Category("Gridly - Appearance"), DefaultValue(GridlyRowColorPreset.ThemeDefault)]
    public GridlyRowColorPreset RowColorPreset { get; set; } = GridlyRowColorPreset.ThemeDefault;

    [Category("Gridly - Appearance"), DefaultValue("State")]
    public string RowColorAspectName { get; set; } = "State";

    [Category("Gridly - Appearance"), DefaultValue(0.18)]
    ...
    [Category("Gridly - Filtering"), DefaultValue(true)]
    ...
    [Category("Gridly - Filtering"), DefaultValue(26)] public int ColumnFilterButtonWidth { get; set; } = 26;

    [Category("Gridly - Header Icons"), DefaultValue(GridlyFilterIconStyle.Funnel)]
    [Description("Kolon filtre butonunda kullanılacak default simge: Funnel, Dot veya CustomImage.")]
    public GridlyFilterIconStyle FilterIconStyle { get; set; } = GridlyFilterIconStyle.Funnel;

    [Category("Gridly - Header Icons"), DefaultValue(null)]
    [Description("FilterIconStyle=CustomImage iken kullanılacak global filtre simgesi.")]
    public Image? CustomFilterIcon { get; set; }

    [Category("Gridly - Header Icons"), DefaultValue(true)]
    [Description("Sıralama yapılan kolonda sort simgesini gösterir.")]
    public bool ShowColumnSortGlyphs { get; set; } = true;

    [Category("Gridly - Header Icons"), DefaultValue(GridlySortGlyphStyle.Chevron)]
    [Description("Sort simge stili: Chevron, Triangle veya CustomImage.")]
    public GridlySortGlyphStyle SortGlyphStyle { get; set; } = GridlySortGlyphStyle.Chevron;
    ...
    [Category("Gridly - Compatibility"), DefaultValue(null)]
    [Description("Gridly uyumlu küçük ikon listesi. Kolon ImageGetter string key veya int index döndürürse buradan çözülür.")]
    public ImageList? SmallImageList { get; set; }

    [Category("Gridly - Compatibility"), DefaultValue(null)]
    [Description("Tile/LargeIcons modları için Gridly uyumlu büyük ikon listesi.")]
    public ImageList? LargeImageList { get; set; }

    [Category("Gridly - Compatibility"), DefaultValue(null)]
    [Description("Check/state ikonları için Gridly uyumlu state image listesi.")]
    public ImageList? StateImageList { get; set; }

    [Category("Gridly - Sorting"), DefaultValue(true)]
    ...
    [Category("Gridly - Sorting"), DefaultValue(5000)]
    [Description("Bu kayıt sayısından sonra sort işlemi async yürütülür.")]
    public int AsyncSortThreshold { get; set; } = 5000;

    [Category("Gridly - Sorting"), DefaultValue(true)]
    [Description("Sort sırasında Gridly'nin kısa bekleme göstergisi çizmesine izin verir.")]
    public bool ShowSortBusyIndicator { get; set; } = true;

    [Category("Gridly - Sorting"), DefaultValue(0)]
    ...
    [Category("Gridly - Context Menu"), DefaultValue(true)]
    ...
    [DefaultValue(null)] public string EmptyListMessage { get; set; } = GridlyText.EmptyList;
    [Category("Gridly - Appearance"), DefaultValue(true)]
    ...
    [Category("Gridly - Appearance"), DefaultValue(true)] public bool AutoHideScrollBarsWhenNotNeeded { get; set; } = true;
    [Category("Gridly - Appearance"), DefaultValue(true)] public bool EnableModernEmptyState { get; set; } = true;

    [Category("Gridly - Appearance")]
    [DefaultValue(true)]
    [Description("Boş liste kartında Gridly sürüm imzasını gösterir. Varsayılan olarak sağ altta görünür.")]
    public bool ShowEmptyStateSignature { get; set; } = true;

    [Category("Gridly - Appearance")]
    [DefaultValue(GridlyEmptyStateSignatureAlignment.BottomRight)]
    [Description("Boş liste kartındaki Gridly sürüm imzasının konumunu belirler.")]
    public GridlyEmptyStateSignatureAlignment EmptyStateSignatureAlignment { get; set; } = GridlyEmptyStateSignatureAlignment.BottomRight;
    ...
    [Category("Gridly - Appearance"), DefaultValue(true)] public bool EnableRowHoverGlow { get; set; } = true;
    [Category("Gridly - Localization"), DefaultValue(GridlyLanguage.Auto)] public GridlyLanguage Language { get => GridlyText.Language; set { GridlyText.Language = value; Invalidate(); } }
    public void LoadLocalizationFile(string jsonFilePath) { GridlyText.LoadCustomTranslations(jsonFilePath); Invalidate(); }
    ...
    [Category("Gridly - Multiline Cells")]
    [DefaultValue(false)]
    ...
    [Category("Gridly - DetailCard")]
    [DefaultValue(GridlyDetailCardLayout.Standard)]
    ...
    public GridlyDetailCardLayout DetailCardLayout { get; set; } = GridlyDetailCardLayout.Standard;
    ...
    public bool ShowDetailCardColumnHeaders
    {
        ...
        set
        {
            ...
            if (ViewMode == GridlyViewMode.DetailCard || ViewMode == GridlyViewMode.PropertyCard)
                Invalidate();
        }
    }

    [Category("Gridly - Editing"), DefaultValue(false)] public bool EnableCellEditing { get; set; } = false;
    [Category("Gridly - Editing"), DefaultValue(false)] public bool CellEditActivationOnDoubleClick { get; set; } = false;
    [Category("Gridly - Editing"), DefaultValue(Keys.None)] public Keys CellEditActivationKey { get; set; } = Keys.None;
    [Category("Gridly - Editing"), DefaultValue(true)] public bool AllowEditAllCells { get; set; } = true;

    [Category("Gridly - Context Menu"), DefaultValue(false)]
    ...
    [Category("Gridly - Context Menu"), DefaultValue("")]
    ...
    [Category("Gridly - Filtering"), DefaultValue(true)] public bool FastFilterMenuForHugeLists { get; set; } = true;
    [Category("Gridly - Filtering"), DefaultValue(300)] public int FastFilterMenuInitialScanRows { get; set; } = 300;
    [Category("Gridly - Filtering"), DefaultValue(1000000)] public int FastFilterMenuSearchScanRows { get; set; } = 1000000;

    [Category("Gridly - Header"), DefaultValue(5000)]
    ...
    [Category("Gridly - Filtering"), DefaultValue(true)] public bool TypedFilterSearchesAllRows { get; set; } = true;
    [Category("Gridly - Filtering"), DefaultValue(2000)] public int MaxEmbeddedFilterVisibleValues { get; set; } = 2000;
    [Category("Gridly - Filtering"), DefaultValue(true)] public bool AsyncLoadFullFilterValues { get; set; } = true;
    [Category("Gridly - Filtering"), DefaultValue(1000000)] public int MaxAsyncFilterDistinctScanRows { get; set; } = 1000000;
    [Category("Gridly - Filtering"), DefaultValue(300)] public int FastFilterPopupPreviewRows { get; set; } = 300;
    [Category("Gridly - Filtering"), DefaultValue(75000)] public int FastFilterIndexedProviderThreshold { get; set; } = 75000;
    [Category("Gridly - Smart Filter"), DefaultValue(true)] public bool EnableSmartFilterEngine { get; set; } = true;
    [Category("Gridly - Smart Filter"), DefaultValue(true)] public bool BuildSmartFilterIndexInBackground { get; set; } = true;
    [Category("Gridly - Smart Filter"), DefaultValue(35)] public int SmartFilterSearchDebounceMs { get; set; } = 35;
    [Category("Gridly - Smart Filter"), DefaultValue(500)] public int SmartFilterPopupValueLimit { get; set; } = 500;
    [Category("Gridly - Smart Filter"), DefaultValue(2000000)] public int SmartFilterMaxScanRows { get; set; } = 2000000;
    [Category("Gridly - Smart Filter"), DefaultValue(true)] public bool SmartFilterSearchAllRows { get; set; } = true;
    [Category("Gridly - Smart Filter"), DefaultValue(true)] public bool SmartFilterTopValuesFirst { get; set; } = true;
    [Category("Gridly - Virtualization"), DefaultValue(300)] public int CachePreloadExtraRows { get; set; } = 300;
    ...
    [Category("Gridly - Keyboard"), DefaultValue(900)] public int IncrementalSearchResetMs { get; set; } = 900;
    [Category("Gridly - Highlight"), DefaultValue(true)] public bool EnableHighlightEngine { get; set; } = true;
    [Category("Gridly - Highlight"), DefaultValue(7000)] public int DefaultHighlightDurationMs { get; set; } = 7000;
    ...
    [Category("Gridly - Column Manager"), DefaultValue(true)] public bool EnableColumnAutoResizeOnDoubleClick { get; set; } = true;
    [Category("Gridly - Column Manager"), DefaultValue(600)] public int AutoResizeMaxWidth { get; set; } = 600;
    [Category("Gridly - Column Manager"), DefaultValue(true)] public bool AutoResizeIncludeHeader { get; set; } = true;
    [Category("Gridly - Column Manager"), DefaultValue(2000)] public int AutoResizeSampleRows { get; set; } = 2000;
    [Category("Gridly - Column Manager"), DefaultValue(true)]
    ...
    [Category("Gridly - Column Manager")]
    [DefaultValue(true)]
    ...
    [Category("Gridly - Images"), DefaultValue(true)]
    ...
    [Category("Gridly - Images"), DefaultValue(24)]
    ...
    [Category("Gridly - Column Manager"), DefaultValue(true)] public bool AllowColumnReorder { get; set; } = true;
    [Category("Gridly - Column Manager"), DefaultValue(false)] public bool AutoSaveColumnLayout { get; set; } = false;
    [Category("Gridly - Column Manager"), DefaultValue(true)] public bool ShowColumnReorderPreview { get; set; } = true;
    [Category("Gridly - Column Manager"), DefaultValue(null)] public string? ColumnLayoutStorageKey { get; set; }
    ...
    [Category("Gridly - Filtering"), DefaultValue(GridlyFilterMenuMode.PopupMenu)]
    ...
    public GridlyFilterMenuMode FilterMenuMode { get; set; } = GridlyFilterMenuMode.PopupMenu;
    [Category("Gridly - Filtering"), DefaultValue(true)] public bool ShowFilterStyleSelectorInContextMenu { get; set; } = true;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyHeaderContextMenuBehavior.Full)]
    ...
    public GridlyHeaderContextMenuBehavior HeaderContextMenuBehavior { get; set; } = GridlyHeaderContextMenuBehavior.Full;


    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuProfile.Full)]
    [Description("Gridly'nin hazır menü profilini belirler. None: Gridly kendi sağ tık menüsünü göstermez. Custom: grup ve tekil ayarlar aynen kullanılır.")]
    public GridlyMenuProfile MenuProfile { get; set; } = GridlyMenuProfile.Full;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuGroups.All)]
    [Description("Başlık/kolon menüsünde hangi özellik gruplarının görüneceğini belirler.")]
    public GridlyMenuGroups HeaderMenuGroups { get; set; } = GridlyMenuGroups.All;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuGroups.All)]
    [Description("Liste/satır sağ tık menüsünde hangi özellik gruplarının görüneceğini belirler.")]
    public GridlyMenuGroups BodyMenuGroups { get; set; } = GridlyMenuGroups.All;

    [Category("Gridly - Context Menu"), DefaultValue(true)]
    [Description("Gridly'nin kolon başlığı sağ tık menüsünü kullanır. False ise header tarafında kendi ContextMenuStrip'inizi kullanabilirsiniz.")]
    public bool UseBuiltInHeaderMenu { get; set; } = true;

    [Category("Gridly - Context Menu"), DefaultValue(true)]
    [Description("Gridly'nin liste/satır sağ tık menüsünü kullanır. False ise body tarafında kendi ContextMenuStrip'inizi kullanabilirsiniz.")]
    public bool UseBuiltInBodyMenu { get; set; } = true;

    [Category("Gridly - Context Menu"), DefaultValue(null)]
    [Description("Verilirse kolon başlığında Gridly menüsü yerine bu özel ContextMenuStrip açılır.")]
    public ContextMenuStrip? HeaderMenuOverride { get; set; }

    [Category("Gridly - Context Menu"), DefaultValue(null)]
    [Description("Verilirse liste/satır alanında Gridly menüsü yerine bu özel ContextMenuStrip açılır.")]
    public ContextMenuStrip? BodyMenuOverride { get; set; }

    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool CloseHeaderContextMenuBeforeOpeningFilterPopup { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFilterItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuSortItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFreezeItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuAutoSizeItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuLayoutItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuGroupingItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupHeaderContextMenu { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuToggleThisGroupItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuExpandAllItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuCollapseAllItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuClearGroupingItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuOnlyThisGroupItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuColumnChooserItem { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuModeItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuViewModeItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFilterStyleItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuThemeItems { get; set; } = true;
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuStateItems { get => ShowStateMenuItems; set => ShowStateMenuItems = value; }
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuScenarioItems { get => ShowScenarioMenuItems; set => ShowScenarioMenuItems = value; }
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuModeItems { get => ShowHeaderMenuModeItems; set => ShowHeaderMenuModeItems = value; }
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuViewModeItems { get => ShowHeaderMenuViewModeItems; set => ShowHeaderMenuViewModeItems = value; }
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuFilterStyleItems { get => ShowHeaderMenuFilterStyleItems; set => ShowHeaderMenuFilterStyleItems = value; }
    [Category("Gridly - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuThemeItems { get => ShowHeaderMenuThemeItems; set => ShowHeaderMenuThemeItems = value; }
    ...
    [Category("Gridly - Context Menu"), DefaultValue(true)]
    [Description("Kullanıcının verdiği ContextMenuStrip içine Gridly'nin temel liste özellikleri alt menüsünü otomatik ekler.")]
    public bool MergeBuiltInMenuWithUserContextMenu { get; set; } = true;

    [Category("Gridly - Context Menu"), DefaultValue("Liste özellikleri")]
    [Description("ContextMenuStrip ile birleştirilen Gridly alt menüsünün başlığı.")]
    public string BuiltInMenuMergeText { get; set; } = string.Empty;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuMergePlacement.Bottom)]
    [Description("Gridly menüsü kullanıcı ContextMenuStrip içine nereye eklensin: Top, Bottom, BeforeFirstSeparator veya AfterFirstSeparator.")]
    public GridlyMenuMergePlacement BuiltInMenuMergePlacement { get; set; } = GridlyMenuMergePlacement.Bottom;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuMergePresentation.SubMenu)]
    [Description("Gridly menüsü kullanıcı menüsüne nasıl eklensin: tek alt menü, direkt inline veya grup alt menüleri.")]
    public GridlyMenuMergePresentation BuiltInMenuMergePresentation { get; set; } = GridlyMenuMergePresentation.SubMenu;

    [Category("Gridly - Context Menu"), DefaultValue(GridlyMenuGroups.All)]
    [Description("Kullanıcı menüsüyle merge edilirken hangi Gridly grupları eklensin.")]
    public GridlyMenuGroups MergedMenuGroups { get; set; } = GridlyMenuGroups.All;

    [Category("Gridly - Context Menu"), DefaultValue("Mode,ViewMode,Filter,Sort,ColumnChooser,Clipboard,AutoSize,Layout,Grouping,FilterStyle,Theme")]
    [Description("Merge edilen Gridly gruplarının görüntü sırası. Örn: Filter,ColumnChooser,Clipboard,AutoSize,Theme")]
    ...
    [Category("Gridly - Context Menu"), DefaultValue(true)]
    [Description("Kullanıcı menüsüyle Gridly menüsü arasına otomatik ayraç ekler.")]
    public bool BuiltInMenuMergeSeparator { get; set; } = true;

    [Category("Gridly - Keyboard"), DefaultValue(true)] public bool KeyboardSelectFirstRowOnFocus { get; set; } = true;
    [Category("Gridly - Keyboard"), DefaultValue(true)] public bool KeyboardContextMenuKeyOpensMenu { get; set; } = true;





    [Category("Gridly - Filter Popup v27.3.1"), DefaultValue(true)]
    ...
    [Category("Gridly - Filter Popup v27.3.1")]
    ...
    [Category("Gridly - Filter Popup v28.10"), DefaultValue(true)]
    ...
    [Category("Gridly - Filter Popup v28.12.2"), DefaultValue(true)]
    ...
    [Category("Gridly - Rendering v27.3"), DefaultValue(true)]
    ...
    [Category("Gridly - ProgressBar"), DefaultValue(false)]
    ...
    [Category("Gridly - Printing"), DefaultValue(5000)]
    ...
    [Category("Gridly - Printing"), DefaultValue("GridlyView")]
    public string PrintTitle { get; set; } = "GridlyView";
    ...
    [Category("Gridly - Grouping"), DefaultValue(true)] public bool AllowGroupCollapse { get; set; } = true;
    [Category("Gridly - Grouping"), DefaultValue(true)] public bool DrawGroupCollapseGlyph { get; set; } = true;
    [Category("Gridly - Mode"), DefaultValue(GridlyMode.Object)]
    [Description("GridlyView ana çalışma modu: Object, DataTable, Virtual, Tree veya Tile.")]
    public GridlyMode Mode
    {
        get => _mode;
        set
        {
    ...
    [Category("Gridly - View Modes")]
    [DefaultValue(GridlyViewMode.Details)]
    [Description("Gridly'nin gelişmiş görünüm modu. Details/List/Tile/LargeIcons gibi modları yönetir.")]
    public GridlyViewMode ViewMode { get => _viewMode; set => SetViewMode(value); }
    ...
    public System.Windows.Forms.View View
    {
        ...
        {
            GridlyViewMode.Poster => System.Windows.Forms.View.LargeIcon,
            GridlyViewMode.MediaTile => System.Windows.Forms.View.Tile,
            GridlyViewMode.Gallery => System.Windows.Forms.View.LargeIcon,
            GridlyViewMode.FilmStrip => System.Windows.Forms.View.Tile,
            GridlyViewMode.ExtraLargeIcons => System.Windows.Forms.View.LargeIcon,
            GridlyViewMode.LargeIcons => System.Windows.Forms.View.LargeIcon,
            GridlyViewMode.MediumIcons => System.Windows.Forms.View.SmallIcon,
            GridlyViewMode.List => System.Windows.Forms.View.List,
            GridlyViewMode.Tile => System.Windows.Forms.View.Tile,
            GridlyViewMode.LargeCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.DashboardCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.RowCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.DetailCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.IconGrid => System.Windows.Forms.View.LargeIcon,
            GridlyViewMode.GroupedList => System.Windows.Forms.View.Details,
            GridlyViewMode.GroupCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.PropertyCard => System.Windows.Forms.View.Tile,
            GridlyViewMode.KpiDashboard => System.Windows.Forms.View.Tile,
            GridlyViewMode.HeatMap => System.Windows.Forms.View.Tile,
            GridlyViewMode.MiniChart => System.Windows.Forms.View.Tile,
            GridlyViewMode.RowPreview => System.Windows.Forms.View.Tile,
            GridlyViewMode.Kanban => System.Windows.Forms.View.Tile,
            GridlyViewMode.Timeline => System.Windows.Forms.View.Tile,
            GridlyViewMode.MasterDetail => System.Windows.Forms.View.Details,
            GridlyViewMode.DenseList => System.Windows.Forms.View.Details,
            _ => System.Windows.Forms.View.Details
        };
        set => SetViewMode(value switch
        {
            System.Windows.Forms.View.LargeIcon => GridlyViewMode.LargeIcons,
            System.Windows.Forms.View.SmallIcon => GridlyViewMode.MediumIcons,
            System.Windows.Forms.View.List => GridlyViewMode.List,
            System.Windows.Forms.View.Tile => GridlyViewMode.Tile,
            _ => GridlyViewMode.Details
        });
    }
    ...
    private GridlyMediaImageScaleMode _mediaImageScaleMode = GridlyMediaImageScaleMode.Contain;
    private bool _mediaImageRoundedCorners = true;
    private bool _tileCheckBoxes;
    private GridlyTileCheckBoxPosition _tileCheckBoxPosition = GridlyTileCheckBoxPosition.TopRight;
    ...
    private GridlyTileCheckBoxVisibilityMode _tileCheckBoxVisibilityMode = GridlyTileCheckBoxVisibilityMode.Always;
    ...
    [Category("Gridly - Tile / Card CheckBox")]
    [DefaultValue(false)]
    ...
    [Category("Gridly - Tile / Card CheckBox")]
    [DefaultValue(GridlyTileCheckBoxPosition.TopRight)]
    [DisplayName("Kart Checkbox Konumu")]
    [Description("Tile/Kart/Geniş Kart modlarında overlay checkbox konumu.")]
    public GridlyTileCheckBoxPosition TileCheckBoxPosition
    {
    ...
    [Category("Gridly - Tile / Card CheckBox")]
    [DefaultValue(GridlyTileCheckBoxVisibilityMode.Always)]
    ...
    public GridlyTileCheckBoxVisibilityMode TileCheckBoxVisibilityMode
    {
    ...
    public int LargeCardPreferredHeight
    {
        ...
        set
        {
            ...
            if (ViewMode == GridlyViewMode.LargeCard) ResetTileRowHeightToPreferredIfNeeded();
            UpdateScrollbars();
            Invalidate();
        }
    }
    ...
    [Category("Gridly - Poster Mode")]
    [DefaultValue(GridlyMediaImageScaleMode.Contain)]
    ...
    public GridlyMediaImageScaleMode MediaImageScaleMode
    {
    ...
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomRowBackColor { get => _customBackColor; set { _customBackColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomAlternateRowBackColor { get => _customAlternateBackColor; set { _customAlternateBackColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomHotRowBackColor { get => _customHotBackColor; set { _customHotBackColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomSelectionBackColor { get => _customSelectionBackColor; set { _customSelectionBackColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomSelectionForeColor { get => _customSelectionForeColor; set { _customSelectionForeColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomGroupBackColor { get => _customGroupBackColor; set { _customGroupBackColor = value; Invalidate(); } }
    [Category("Gridly - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomGroupForeColor { get => _customGroupForeColor; set { _customGroupForeColor = value; Invalidate(); } }
    [Category("Gridly - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightBackColor { get => _highlightBackColor; set { _highlightBackColor = value; Invalidate(); } }
    [Category("Gridly - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightBorderColor { get => _highlightBorderColor; set { _highlightBorderColor = value; Invalidate(); } }
    [Category("Gridly - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightForeColor { get => _highlightForeColor; set { _highlightForeColor = value; Invalidate(); } }
    ...
    [Category("Gridly - Theme"), DefaultValue(true)]
    [Description("True ise tema/FormatRow/FormatCell/RowForeColorGetter kaynaklı düşük kontrastlı yazı renklerini arka plana göre otomatik okunur hale getirir. FastGridlyView ve log ekranlarında açık temada beyaz yazı gibi sorunları engeller.")]
    ...
    [Category("Gridly - Theme"), DefaultValue(true)]
    [Description("True ise GridlyView/FastGridlyView/DataListView/TreeGridlyView kolon başlığı renkleri aynı tema paletinden çizilir. Eski designer veya Gridly uyumluluğundan gelen koyu/açık HeaderBackColor farklarını normalize eder.")]
    public bool AutoApplyThemeToColumnHeaders { get; set; } = true;

    [Category("Gridly - Theme"), DefaultValue(true)]
    [Description("True ise header, filter/sort glyph, checkbox, grid çizgisi ve satır renkleri tema değiştiğinde tüm Gridly türevlerinde aynı görsel profile çekilir.")]
    public bool UseUnifiedThemeVisuals { get; set; } = true;

    [Category("Gridly - Theme"), DefaultValue(true)]
    [Description("Built-in, Gridly uyumluluk ve kullanıcı tarafından verilen ContextMenuStrip menülerini Gridly teması ile otomatik boyar.")]
    public bool AutoApplyThemeToContextMenus { get; set; } = true;
    [DefaultValue(false)] public bool EnableRowDetails { get; set; }
    [Browsable(false)] public IList<GridlyConditionalFormat> ConditionalFormats => _conditionalFormats;
    [Browsable(false)] public IList<GridlySummaryItem> Summaries => _summaries;
    [Browsable(false)] public GridlyUndoService UndoService => _undo;
    ...
    /// <summary>Gridly migration alias.</summary>
    ...
    [Category("Gridly - Interaction"), DefaultValue(true)]
    ...
    [Category("Gridly - Keyboard"), DefaultValue(true)]
    ...
    [Browsable(false)] public GridlyFilterSet Filters => _filters;
    ...
    /// <summary>Predicate tabanlı Gridly uyumlu filtre alias'ı.</summary>
    [Browsable(false)]
    ...
    public void SetRowDetailsProvider(GridlyRowDetailsProvider? provider) => _detailsProvider = provider;
    public void ClearRowDetailsProvider() => _detailsProvider = null;

    public PrintDocument CreatePrintDocument(string? title = null)
    {
        var state = new GridlyPrintState { Title = string.IsNullOrWhiteSpace(title) ? PrintTitle : title! };
    ...
    public void ShowPrintPreview(string? title = null)
    {
        ...
        using var preview = new PrintPreviewDialog
        {
            ...
            Text = string.IsNullOrWhiteSpace(title) ? PrintTitle + " - " + GridlyText.PrintPreviewSuffix : title + " - " + GridlyText.PrintPreviewSuffix,
            UseAntiAlias = true
        };
        preview.Shown += (_, __) =>
        {
    ...
    private sealed class GridlyPrintState
    {
        public string Title = "GridlyView";
        public int RowIndex;
        public List<object> Rows = new();
        public List<GridlyColumn> Columns = new();
        public int PageNo;
    }

    private void PreparePrintState(GridlyPrintState state)
    {
        state.RowIndex = 0;
        state.PageNo = 0;
    ...
    private void PrintPage(PrintPageEventArgs e, GridlyPrintState state)
    {
        state.PageNo++;
        ...
        while (state.RowIndex < state.Rows.Count && y + rowHeight + footerHeight <= bounds.Bottom)
        {
            ...
            for (int i = 0; i < state.Columns.Count; i++)
            {
                ...
                if (PrintRenderProgressBars && col.Kind == GridlyColumnKind.ProgressBar)
                {
                    DrawPrintProgress(g, r, ToInt(value), fore, back, grid, accent, printFont);
                }
                else
                {
                    string text = col.Kind == GridlyColumnKind.ProgressBar ? ToInt(value) + "%" : Convert.ToString(value) ?? string.Empty;
        ...
        string footer = $"GridlyView {GridlyVersionInfo.Version}  •  Sayfa {state.PageNo}";
    ...
    private static List<Rectangle> CalculatePrintColumnRects(IReadOnlyList<GridlyColumn> columns, int left, int width, bool fitToPageWidth)
    {
        ...
        {
            var c = columns[i];
            bool checkLike = c.Kind == GridlyColumnKind.CheckBox;
            minWidths[i] = checkLike ? 38 : c.Kind == GridlyColumnKind.ProgressBar ? 74 : 46;
    ...
    private GridlyColumn? GetActiveCheckBoxColumn()
    {
        if (_checkBoxes)
            return GetCompatibilityCheckBoxHostColumn();
        return Columns.FirstOrDefault(x => x.Kind == GridlyColumnKind.CheckBox || x.CellCheckBox);
    }

    private static bool UsesInternalCheckStore(GridlyColumn col)
    {
        ...
        if (col.Kind == GridlyColumnKind.CheckBox)
        ...
        // keep the check state in Gridly's internal store instead of trying to convert the
    ...
    private HashSet<object> GetInternalCheckRows(GridlyColumn col)
    {
    ...
    private CheckState GetRowCheckState(object row, GridlyColumn col)
    {
    ...
    private void SetRowCheckState(object row, GridlyColumn col, CheckState state)
    {
    ...
    public void AutoResizeColumnToContent(GridlyColumn col, int maxRows = -1, int padding = 24)
    {
        AutoResizeColumnToContent(col, maxRows, padding, refresh: true);
    }

    private void AutoResizeColumnToContent(GridlyColumn col, int maxRows, int padding, bool refresh)
    {
    ...
    public void MoveColumn(GridlyColumn column, int targetVisibleIndex)
    {
    ...
    public string GetColumnLayoutFilePath()
    {
        ...
        if (string.IsNullOrWhiteSpace(key)) key = GetType().FullName ?? "GridlyView";
        foreach (var ch in Path.GetInvalidFileNameChars()) key = key.Replace(ch, '_');
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GridlyView", "Layouts");
        Directory.CreateDirectory(dir);
        return Path.Combine(dir, key + ".json");
    }

    public GridlyLayoutState CaptureColumnLayout()
    {
        var state = GridlyLayoutState.Capture(Columns, _filters.GlobalText, _sortColumn?.AspectName, _sortDesc);
    ...
    public void ApplyColumnLayout(GridlyLayoutState state)
    {
    ...
    public bool LoadColumnLayout()
    {
        try
        {
            ...
            ApplyColumnLayout(GridlyLayoutState.FromJson(File.ReadAllText(file)));
            return true;
        }
        catch
        {
    ...
    public string GetColumnLayoutProfileFilePath(string profileName)
    {
        ...
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GridlyView", "Layouts", key);
        Directory.CreateDirectory(dir);
    ...
    public event EventHandler<GridlyCellClickEventArgs>? CellClick;
    public event EventHandler<GridlyCellClickEventArgs>? ButtonClick;
    public event EventHandler<GridlyCellClickEventArgs>? HyperlinkClick;
    public event EventHandler<GridlyCellClickEventArgs>? ItemActivate;
    public event EventHandler<GridlyCellEditEventArgs>? CellValueChanged;
    public event EventHandler? SelectionChanged;

    [Browsable(false)]
    [EditorBrowsable(EditorBrowsableState.Never)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public GridlyTheme Theme
    {
        get => _theme;
        set => ApplyTheme(value ?? WindowsThemeService.CurrentTheme());
    }

    public void ApplyTheme(GridlyTheme theme)
    {
        _applyingTheme = true;
        try
        {
            theme = EnforceThemeAccessibility
                ? global::Gridly.Theming.GridlyThemeAccessibility.Normalize(theme)
                : theme;
            _theme = theme;
    ...
    public void ApplyGridlyThemeToMenu(ContextMenuStrip? menu)
    {
        ...
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(menu, _theme);
    }
    ...
    private void ApplyThemeToAllAttachedContextMenus()
    {
        ...
        ApplyGridlyThemeToMenu(ContextMenuStrip);
        ApplyGridlyThemeToMenu(HeaderMenuOverride);
        ApplyGridlyThemeToMenu(BodyMenuOverride);
    }

    public void ApplyThemePreset(GridlyThemePreset preset)
    {
        ThemePreset = preset;
    }
    ...
    private GridlyTheme ResolveRuntimeTheme()
    {
        if (AutoThemeFromParent && ThemePreset == GridlyThemePreset.System)
            return ResolveAutoThemeFromParent(useFluent: false);

        return GridlyTheme.FromPreset(ThemePreset);
    }
    ...
    protected override void OnParentChanged(EventArgs e)
    {
        ...
        else if (AutoThemeFromParent && ThemePreset == GridlyThemePreset.System)
        {
            ApplySelectedTheme();
        }
    }
    ...
    private void ApplyDesignTimePreviewTheme()
    {
        ...
        GridlyTheme theme = ResolveDesignTimeTheme();
        ApplyTheme(theme);
    }

    private GridlyTheme ResolveDesignTimeTheme()
    {
        GridlyTheme theme = DesignTimeThemePreview switch
        {
            GridlyDesignTimeThemePreview.Light => GridlyTheme.LightTheme(),
            GridlyDesignTimeThemePreview.Dark => GridlyTheme.DarkTheme(),
            GridlyDesignTimeThemePreview.Fluent => DesignTimeFollowParentTheme
                ? ResolveAutoThemeFromParent(useFluent: true)
                : GridlyTheme.FluentLightTheme(),
            _ => DesignTimeFollowParentTheme
                ? ResolveAutoThemeFromParent(useFluent: false)
                : GridlyTheme.LightTheme()
        };

        return NormalizeDesignTimeTheme(theme);
    }

    private GridlyTheme NormalizeDesignTimeTheme(GridlyTheme theme)
    {
        if (theme == null) return GridlyTheme.LightTheme();
        ...
        if (DesignTimeThemePreview != GridlyDesignTimeThemePreview.Dark && !theme.IsDark)
        {
    ...
    private GridlyTheme ResolveAutoThemeFromParent(bool useFluent)
    {
        if (AutoThemeFromParent)
        {
            ...
            {
                var fromParent = GridlyTheme.FromParentColor(parent.BackColor, parent.ForeColor);
                if (useFluent)
                    return fromParent.IsDark ? GridlyTheme.FluentDarkTheme() : GridlyTheme.FluentLightTheme();
                return fromParent;
            }
        }

        var systemTheme = GridlyTheme.FromPreset(ThemePreset);
        if (useFluent)
            return systemTheme.IsDark ? GridlyTheme.FluentDarkTheme() : GridlyTheme.FluentLightTheme();
        return systemTheme;
    }
    ...
    protected override void OnBackColorChanged(EventArgs e)
    {
        ...
        if (IsInDesignMode && DesignTimeThemePreview == GridlyDesignTimeThemePreview.Auto)
            ApplyDesignTimePreviewTheme();
    }
    ...
    private void EnsureDesignTimeSampleData()
    {
        ...
        try
        {
            if (Columns.Count == 0)
            {
                Columns.Add(new GridlyColumn("Durum", "State", 90) { Editable = true });
                Columns.Add(new GridlyColumn("Ad", "Name", 170) { Editable = true, FillFreeSpace = true });
                Columns.Add(new GridlyColumn("Oran", "Rate", 80) { Editable = true, TextAlign = ContentAlignment.MiddleRight });
                Columns.Add(new GridlyColumn("Puan", "Rating", 80) { Kind = GridlyColumnKind.Rating, Editable = true });
            }
    ...
    {
        if (_mode != GridlyMode.Tile) _mode = GridlyMode.Object;
    ...
    private void ApplyModeVisualDefaults(GridlyMode mode)
    {
        switch (mode)
        {
            case GridlyMode.Tile:
                if (_viewMode != GridlyViewMode.Tile) SetViewMode(GridlyViewMode.Tile);
                break;
            case GridlyMode.Tree:
                if (_viewMode != GridlyViewMode.Details) SetViewMode(GridlyViewMode.Details);
                break;
            default:
                if (_viewMode == GridlyViewMode.Tile) SetViewMode(GridlyViewMode.Details);
                break;
        }
    }

    public void SetViewMode(GridlyViewMode mode)
    {
        _viewMode = mode;
        RememberCurrentViewModeForActiveScenario();
        if (mode == GridlyViewMode.Poster)
            ApplyPosterModeDefaults(false);
        if (IsCardLikeViewMode(mode))
            _mode = GridlyMode.Tile;
        else if (_mode == GridlyMode.Tile)
            _mode = GridlyMode.Object;

        if (mode == GridlyViewMode.GroupedList && !EnableGrouping)
        {
        ...
        bool detailsLikeMode = mode == GridlyViewMode.Details || mode == GridlyViewMode.DenseList || mode == GridlyViewMode.GroupedList || mode == GridlyViewMode.MasterDetail;
        ...
        if (detailsLikeMode || mode == GridlyViewMode.List)
        {
            int detailsHeight = mode == GridlyViewMode.DenseList ? 22 : (mode == GridlyViewMode.List ? 30 : 28);
        ...
        else
        {
            ...
            {
                GridlyViewMode.Poster => Math.Max(180, PosterPreferredHeight),
                GridlyViewMode.MediaTile => Math.Max(132, TilePreferredHeight),
                GridlyViewMode.Gallery => Math.Max(170, TilePreferredHeight),
                GridlyViewMode.FilmStrip => Math.Max(150, TilePreferredHeight),
                GridlyViewMode.ExtraLargeIcons => 96,
                GridlyViewMode.LargeIcons => 72,
                GridlyViewMode.MediumIcons => 48,
                GridlyViewMode.Tile => TilePosterMode ? Math.Max(180, TilePreferredHeight) : Math.Max(72, TilePreferredHeight),
                GridlyViewMode.IconGrid => Math.Max(92, TilePreferredHeight),
                GridlyViewMode.DashboardCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 12)),
                GridlyViewMode.KpiDashboard => Math.Max(130, TilePreferredHeight),
                GridlyViewMode.HeatMap => Math.Max(118, TilePreferredHeight),
                GridlyViewMode.MiniChart => Math.Max(108, TilePreferredHeight),
                GridlyViewMode.RowPreview => Math.Max(96, TilePreferredHeight),
                GridlyViewMode.RowCard => Math.Max(118, TilePreferredHeight),
                GridlyViewMode.PropertyCard => GetDetailCardPreferredHeight(),
                GridlyViewMode.DetailCard => GetDetailCardPreferredHeight(),
                GridlyViewMode.GroupCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 20)),
                GridlyViewMode.Kanban => Math.Max(168, Math.Max(TilePreferredHeight, LargeCardPreferredHeight)),
                GridlyViewMode.Timeline => Math.Max(132, TilePreferredHeight),
                GridlyViewMode.LargeCard => Math.Max(LargeCardPreferredHeight, Math.Max(132, TilePreferredHeight)),
                _ => 28
            };
        }
        RefreshCardViewFilterUx();
    ...
    private static bool IsCardLikeViewMode(GridlyViewMode mode)
        => mode is GridlyViewMode.Tile
            or GridlyViewMode.Poster
            or GridlyViewMode.MediaTile
            or GridlyViewMode.Gallery
            or GridlyViewMode.FilmStrip
            or GridlyViewMode.LargeCard
            or GridlyViewMode.ExtraLargeIcons
            or GridlyViewMode.LargeIcons
            or GridlyViewMode.MediumIcons
            or GridlyViewMode.DashboardCard
            or GridlyViewMode.RowCard
            or GridlyViewMode.DetailCard
            or GridlyViewMode.IconGrid
            or GridlyViewMode.GroupCard
            or GridlyViewMode.PropertyCard
            or GridlyViewMode.KpiDashboard
            or GridlyViewMode.HeatMap
            or GridlyViewMode.MiniChart
            or GridlyViewMode.RowPreview
            or GridlyViewMode.Kanban
            or GridlyViewMode.Timeline;

    public static string GetViewModeDisplayName(GridlyViewMode mode)
        => mode switch
        {
            GridlyViewMode.Details => "Detay Liste",
            GridlyViewMode.DenseList => "Yoğun Liste",
            GridlyViewMode.List => "Basit Liste",
            GridlyViewMode.Tile => "Kart Görünümü",
            GridlyViewMode.LargeCard => "Geniş Kart",
            GridlyViewMode.DashboardCard => "Dashboard Kart",
            GridlyViewMode.RowCard => "Satır Kart",
            GridlyViewMode.DetailCard => "DetailCard",
            GridlyViewMode.MediaTile => "MediaTile",
            GridlyViewMode.Gallery => "Gallery",
            GridlyViewMode.FilmStrip => "FilmStrip",
            GridlyViewMode.IconGrid => "İkon Grid",
            GridlyViewMode.GroupedList => "Gruplu Liste",
            GridlyViewMode.GroupCard => "GroupCard",
            GridlyViewMode.PropertyCard => "PropertyCard",
            GridlyViewMode.KpiDashboard => "KPI Dashboard",
            GridlyViewMode.HeatMap => "HeatMap",
            GridlyViewMode.MiniChart => "MiniChart",
            GridlyViewMode.RowPreview => "RowPreview",
            GridlyViewMode.Kanban => "Kanban",
            GridlyViewMode.Timeline => "Zaman Akışı",
            GridlyViewMode.MasterDetail => "Master-Detail",
            GridlyViewMode.Poster => "Poster Görünümü",
            GridlyViewMode.ExtraLargeIcons => "Poster Kart",
            GridlyViewMode.LargeIcons => "Büyük İkon",
            GridlyViewMode.MediumIcons => "Kompakt İkon",
            _ => mode.ToString()
        };
    ...
    public void SetColumnFilter(GridlyColumnFilter filter)
    {
        _filters.Set(filter);
        BuildViewIndex();
        QueueAutoSaveUserLayout();
    ...
    public void ShowColumnChooser()
    {
        ...
        {
            MessageBox.Show((IWin32Window?)FindForm() ?? this, ex.ToString(), "Gridly kolon seçici hatası", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    public void LoadLayout(string path)
    {
        ...
        var state = GridlyLayoutState.FromJson(File.ReadAllText(path));
        SanitizeLayoutFiltersIfNeeded(state);
        ApplyLayout(state);
    }

    private void SanitizeLayoutFiltersIfNeeded(GridlyLayoutState state)
    {
        if (state == null || PersistColumnFilters) return;

        state.GlobalFilter = string.Empty;
        state.ColumnFilters = new List<GridlyColumnFilter>();
    }
    public GridlyLayoutState CaptureLayout()
    {
        var state = GridlyLayoutState.Capture(Columns, PersistColumnFilters ? _filters.GlobalText : string.Empty, _sortColumn?.AspectName, _sortDesc);
        state.ColumnFilters = PersistColumnFilters
            ? _filters.Filters.Select(f => new GridlyColumnFilter
            {
    ...
    public void ApplyLayout(GridlyLayoutState state)
    {
        if (state == null) return;
        state.Apply(Columns);
        _filters.Clear();
        ...
        if (PersistVisualPreferences)
        {
            if (!string.IsNullOrWhiteSpace(state.FilterMenuMode) && Enum.TryParse<GridlyFilterMenuMode>(state.FilterMenuMode, out var filterMode)) FilterMenuMode = filterMode;
            if (!string.IsNullOrWhiteSpace(state.FilterIconStyle) && Enum.TryParse<GridlyFilterIconStyle>(state.FilterIconStyle, out var filterIconStyle)) FilterIconStyle = filterIconStyle;
            if (!string.IsNullOrWhiteSpace(state.SortGlyphStyle) && Enum.TryParse<GridlySortGlyphStyle>(state.SortGlyphStyle, out var sortGlyphStyle)) SortGlyphStyle = sortGlyphStyle;
            ...
            if (!string.IsNullOrWhiteSpace(state.ViewMode) && Enum.TryParse<GridlyViewMode>(state.ViewMode, out var viewMode)) ViewMode = viewMode;
            if (!string.IsNullOrWhiteSpace(state.ColumnChooserMenuMode) && Enum.TryParse<GridlyColumnChooserMenuMode>(state.ColumnChooserMenuMode, out var chooserMode)) ColumnChooserMenuMode = chooserMode;
            ...
            if (!string.IsNullOrWhiteSpace(state.MenuIconMode) && Enum.TryParse<GridlyMenuIconMode>(state.MenuIconMode, out var menuIconMode)) MenuIconMode = menuIconMode;
            if (!string.IsNullOrWhiteSpace(state.MenuIconSize) && Enum.TryParse<GridlyMenuIconSize>(state.MenuIconSize, out var menuIconSize)) MenuIconSize = menuIconSize;
            if (!string.IsNullOrWhiteSpace(state.CustomMenuIconFolder)) CustomMenuIconFolder = state.CustomMenuIconFolder;
        }
        BuildViewIndex();
    }

    public string ExportVisibleCsv(string path, char separator = ';') => GridlyExporter.SaveCsv(path, Columns, GetVisibleObjects(), separator);
    public string ExportVisibleExcel(string path, string worksheetName = "GridlyView") => GridlyExporter.SaveExcelXml(path, Columns, GetVisibleObjects(), worksheetName);
    public string ExportVisiblePdf(string path, string title = "GridlyView") => GridlyExporter.SavePdf(path, Columns, GetVisibleObjects(), title);
    public string ExportVisiblePdf(string path, GridlyPdfExportOptions options)
    {
        options ??= new GridlyPdfExportOptions();
        if (options.Mode == GridlyPdfExportMode.Auto)
        {
            options.Mode = ViewMode is GridlyViewMode.DashboardCard or GridlyViewMode.LargeCard or GridlyViewMode.RowCard or GridlyViewMode.DetailCard or GridlyViewMode.Tile or GridlyViewMode.Kanban or GridlyViewMode.Poster or GridlyViewMode.MediaTile or GridlyViewMode.FilmStrip
                ? GridlyPdfExportMode.Card
                : GridlyPdfExportMode.Table;
        }
        ...
        return GridlyExporter.SavePdf(path, Columns, GetVisibleObjects(), options);
    }
    ...
    public void CopySelectionAsJsonToClipboard()
    {
        ...
        var rows = (_selectedRows.Count > 0 ? SelectedObjects : (SelectedObject == null ? Array.Empty<object>() : new[] { SelectedObject })).ToList();
        if (rows.Count == 0) return;
        Clipboard.SetText(GridlyExporter.ToJson(Columns, rows));
    }

    public string ExportSelectedCsv(string path, char separator = ';') => GridlyExporter.SaveCsv(path, Columns, SelectedObjects, separator);
    public string ExportSelectedExcel(string path, string worksheetName = "GridlySelection") => GridlyExporter.SaveExcelXml(path, Columns, SelectedObjects, worksheetName);
    public string ExportVisibleJson(string path) => GridlyExporter.SaveJson(path, Columns, GetVisibleObjects());
    public string ExportSelectedJson(string path) => GridlyExporter.SaveJson(path, Columns, SelectedObjects);

    public IReadOnlyList<GridlyColumnAnalytics> GetVisibleAnalytics(int maxDistinctPerColumn = 8, int maxRows = 50000)
    {
        var rows = EnumerateVisibleObjects(maxRows).ToList();
        var result = new List<GridlyColumnAnalytics>();
        foreach (var col in Columns.VisibleColumns)
            result.Add(GridlyColumnAnalytics.From(col, rows, maxDistinctPerColumn));
        return result;
    }

    public GridlyColumnAnalytics? GetColumnAnalytics(string aspectName, int maxDistinct = 12, int maxRows = 100000)
    {
        ...
        return col == null ? null : GridlyColumnAnalytics.From(col, EnumerateVisibleObjects(maxRows).ToList(), maxDistinct);
    }
    ...
    private Dictionary<GridlyColumn, string> BuildSummaryTextCache()
    {
        var result = new Dictionary<GridlyColumn, string>();
    ...
    private void BuildDisplayRows()
    {
        ...
        foreach (int realIndex in sourceIndexes)
        {
            ...
            if (string.IsNullOrWhiteSpace(key)) key = GridlyText.EmptyValue;
        ...
        foreach (var pair in groups)
        {
            ...
            _displayRows.Add(GridlyDisplayRow.Group(pair.Key, pair.Value.Count, collapsed));
            if (!collapsed)
            {
                foreach (int real in pair.Value) _displayRows.Add(GridlyDisplayRow.Data(real));
            }
        }
        _viewIsDirect = false;
    }
    ...
    private readonly record struct GridlySortEntry(int Index, object? PrimaryKey, object? SecondaryKey);
    ...
    private static int CompareRows(object? a, object? b, GridlyColumn col)
    {
    ...
    private int GetMinimumRowHeightForCurrentTileView()
    {
        return ViewMode switch
        {
            ...
            GridlyViewMode.Gallery => Math.Max(170, TilePreferredHeight),
            GridlyViewMode.FilmStrip => Math.Max(150, TilePreferredHeight),
            GridlyViewMode.ExtraLargeIcons => TilePosterMode ? Math.Max(180, TilePreferredHeight) : 96,
            GridlyViewMode.LargeIcons => TilePosterMode ? Math.Max(180, TilePreferredHeight) : 72,
            GridlyViewMode.MediumIcons => 48,
            GridlyViewMode.Tile => TilePosterMode ? Math.Max(180, TilePreferredHeight) : Math.Max(72, TilePreferredHeight),
            GridlyViewMode.IconGrid => Math.Max(92, TilePreferredHeight),
            GridlyViewMode.DashboardCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 12)),
    ...
    private int GetDetailCardPreferredHeight()
    {
        ...
        if (DetailCardLayout is GridlyDetailCardLayout.Media or GridlyDetailCardLayout.PosterLeft)
    ...
    private int GetTileWidth()
    {
        ...
        {
            GridlyViewMode.Poster => Math.Max(160, PosterPreferredWidth),
            GridlyViewMode.MediaTile => Math.Max(156, TilePreferredWidth),
            GridlyViewMode.Gallery => Math.Max(180, TilePreferredWidth),
            GridlyViewMode.FilmStrip => Math.Max(260, TilePreferredWidth),
            GridlyViewMode.ExtraLargeIcons => TilePosterMode ? 160 : 270,
            GridlyViewMode.LargeIcons => TilePosterMode ? 150 : 240,
            GridlyViewMode.MediumIcons => TilePosterMode ? 140 : 210,
            GridlyViewMode.Tile => TilePreferredWidth,
            GridlyViewMode.IconGrid => Math.Max(160, TilePreferredWidth - 30),
            GridlyViewMode.DashboardCard => Math.Max(320, TilePreferredWidth + 120),
            GridlyViewMode.KpiDashboard => Math.Max(220, TilePreferredWidth),
            GridlyViewMode.HeatMap => Math.Max(160, TilePreferredWidth - 20),
            GridlyViewMode.MiniChart => Math.Max(300, TilePreferredWidth + 80),
            GridlyViewMode.RowPreview => Math.Max(560, LargeCardPreferredWidth),
            GridlyViewMode.RowCard => Math.Max(600, LargeCardPreferredWidth),
            GridlyViewMode.PropertyCard => Math.Max(600, ClientSize.Width - VBarWidth - 12),
            GridlyViewMode.DetailCard => Math.Max(600, ClientSize.Width - VBarWidth - 12),
            GridlyViewMode.GroupCard => Math.Max(300, TilePreferredWidth + 90),
            GridlyViewMode.Kanban => Math.Max(300, TilePreferredWidth + 90),
            GridlyViewMode.Timeline => Math.Max(620, LargeCardPreferredWidth),
            GridlyViewMode.LargeCard => Math.Max(LargeCardPreferredWidth, TilePreferredWidth + 140),
    ...
    private int GetTilesPerRow()
    {
        ...
        if (ViewMode is GridlyViewMode.RowCard or GridlyViewMode.RowPreview or GridlyViewMode.DetailCard or GridlyViewMode.PropertyCard or GridlyViewMode.Timeline or GridlyViewMode.FilmStrip) return 1;
    ...
    {
        // v25.83: Gridly uyumlu CheckBoxes modunda eski/boş selector kolonları
    ...
    private void DrawColumnHeaderContent(Graphics g, Rectangle r, GridlyColumn col)
    {
    ...
    private Rectangle GetHeaderCheckBoxRect(GridlyColumn col)
    {
    ...
    private void ToggleHeaderCheckBox(GridlyColumn col)
    {
        ...
        if ((col.Kind == GridlyColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col)) && col.HeaderCheckBoxUpdatesRowCheckBoxes && next != CheckState.Indeterminate)
        {
    ...
    private GridlyCellEditEventArgs CreateHeaderCheckBoxEditEventArgs(GridlyColumn col, object? newValue)
    {
        int rowIndex = -1;
        ...
        return new GridlyCellEditEventArgs(rowIndex, rowObject!, col, newValue);
    }

    private void UpdateCompatibilityHeaderCheckState(GridlyColumn? col)
    {
        if (col == null || !col.HeaderCheckBox) return;
        if (!(col.Kind == GridlyColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))) return;
    ...
    private Color GetEffectiveHeaderBackColor(GridlyColumn? col = null)
    {
    ...
    private Color GetEffectiveHeaderForeColor(Color headerBack, GridlyColumn? col = null)
    {
    ...
    private bool ShouldShowColumnFilterButton(GridlyColumn col)
        => ShowHeader && ShowFilterMenu && ShowColumnFilterButtons && col != null && col.Filterable && col.Visible;

    private Rectangle GetColumnFilterButtonRect(Rectangle headerBounds, GridlyColumn col)
    {
    ...
    private Rectangle GetColumnFilterButtonRect(GridlyColumn col)
    {
    ...
    private void DrawColumnFilterButton(Graphics g, Rectangle headerBounds, GridlyColumn col)
    {
        ...
        var style = col.FilterIconStyle == GridlyFilterIconStyle.Inherit ? FilterIconStyle : col.FilterIconStyle;
        var customIcon = col.FilterIcon ?? CustomFilterIcon;
        if (style == GridlyFilterIconStyle.CustomImage && customIcon != null)
        {
            DrawHeaderIconImage(g, customIcon, r);
        }
        else if (style == GridlyFilterIconStyle.Dot)
        {
        ...
        if (active && style != GridlyFilterIconStyle.Dot)
        {
    ...
    private void DrawColumnSortGlyph(Graphics g, Rectangle headerBounds, GridlyColumn col)
    {
        ...
        var style = col.SortGlyphStyle == GridlySortGlyphStyle.Inherit ? SortGlyphStyle : col.SortGlyphStyle;
        var customIcon = _sortDesc ? (col.SortDescendingIcon ?? CustomSortDescendingIcon) : (col.SortAscendingIcon ?? CustomSortAscendingIcon);
        if (style == GridlySortGlyphStyle.CustomImage && customIcon != null)
        {
            DrawHeaderIconImage(g, customIcon, r);
            return;
        }
        ...
        if (style == GridlySortGlyphStyle.Triangle)
        {
            var pts = _sortDesc
    ...
    {
        if (RowColorPreset == GridlyRowColorPreset.ThemeDefault)
            return currentBack;
        ...
        {
            GridlyRowColorPreset.SubtleZebra => viewIndex % 2 == 1
                ? Blend(_theme.AlternateBackColor, _theme.AccentColor, _theme.IsDark ? 0.06 : 0.035)
                : currentBack,
            GridlyRowColorPreset.SoftAccent => Blend(currentBack, _theme.AccentColor, _theme.IsDark ? strength * 0.75 : strength * 0.55),
            GridlyRowColorPreset.AOIRisk => string.IsNullOrWhiteSpace(key) ? currentBack : Blend(currentBack, accent, _theme.IsDark ? strength * 1.10 : strength),
            GridlyRowColorPreset.StatusPills => string.IsNullOrWhiteSpace(key) ? currentBack : Blend(currentBack, accent, _theme.IsDark ? strength * 0.85 : strength * 0.70),
            GridlyRowColorPreset.SeverityBands => Blend(currentBack, accent, _theme.IsDark ? strength * 1.20 : strength * 0.95),
            GridlyRowColorPreset.PastelCards => Blend(currentBack, accent, _theme.IsDark ? strength * 0.65 : strength * 0.45),
            GridlyRowColorPreset.FocusGlow => viewIndex == _hotRow
    ...
    private GridlyColumn? GetTileCheckBoxColumn()
    {
        if (!TileCheckBoxes) return null;

        bool IsCheckColumn(GridlyColumn c)
            => c.Kind == GridlyColumnKind.CheckBox || c.CellCheckBox || IsCompatibilityCheckBoxHostColumn(c);
    ...
    private Rectangle GetTileCheckBoxRect(Rectangle cardBounds)
    {
        ...
        {
            case GridlyTileCheckBoxPosition.TopLeft:
                x = left;
                y = top;
                break;

            case GridlyTileCheckBoxPosition.BottomLeft:
                x = left;
                y = bottom;
                break;

            case GridlyTileCheckBoxPosition.BottomRight:
                x = right;
                y = bottom;
                break;

            case GridlyTileCheckBoxPosition.TopRight:
            default:
                x = right;
                y = top;
                break;
    ...
    private bool ShouldDisplayTileCheckBox(int viewIndex, object row, GridlyColumn checkColumn)
    {
        bool isSelected = _selectedRows.Contains(viewIndex) || viewIndex == _selectedRow;
        ...
        {
            GridlyTileCheckBoxVisibilityMode.Hover => isHot,
            GridlyTileCheckBoxVisibilityMode.Selected => isSelected,
            GridlyTileCheckBoxVisibilityMode.HoverOrSelected => isHot || isSelected,
            GridlyTileCheckBoxVisibilityMode.CheckedOrHoverOrSelected => isChecked || isHot || isSelected,
            _ => true
        };
    }

    private void DrawTileCheckBoxAdorner(Graphics g, Rectangle cardBounds, object row, GridlyColumn checkColumn)
    {
    ...
    private bool TryGetTileCheckBoxHit(Point location, int viewIndex, out GridlyColumn? checkColumn)
    {
        checkColumn = null;
    ...
    private void DrawTileCard(Graphics g, Rectangle r, int viewIndex, object row)
    {
        ...
        GridlyCardVisualInfo? cardVisualInfo = ResolveCardVisualInfo(row, cardStatusColor);
        ...
        else if (ViewMode == GridlyViewMode.Timeline)
        {
        ...
        if (ViewMode == GridlyViewMode.KpiDashboard)
        {
        ...
        if (ViewMode == GridlyViewMode.HeatMap)
        {
        ...
        if (ViewMode == GridlyViewMode.MiniChart)
        {
        ...
        var visible = Columns.VisibleColumns.Where(c => c.Kind != GridlyColumnKind.CheckBox && c.Width > 0).ToList();
        var iconCol = visible.FirstOrDefault(c => c.Kind == GridlyColumnKind.Icon || c.Kind == GridlyColumnKind.Image)
        ...
        {
            GridlyViewMode.Poster => 44,
            GridlyViewMode.Gallery => 44,
            GridlyViewMode.MediaTile => 38,
            GridlyViewMode.FilmStrip => 42,
            GridlyViewMode.ExtraLargeIcons => 42,
            GridlyViewMode.IconGrid => 40,
            GridlyViewMode.MediumIcons => 24,
            GridlyViewMode.LargeCard or GridlyViewMode.DashboardCard or GridlyViewMode.GroupCard or GridlyViewMode.RowCard or GridlyViewMode.RowPreview or GridlyViewMode.DetailCard or GridlyViewMode.PropertyCard or GridlyViewMode.Timeline or GridlyViewMode.KpiDashboard or GridlyViewMode.HeatMap or GridlyViewMode.MiniChart => 28,
            _ => 32
        };
        int textX = ViewMode == GridlyViewMode.Timeline ? r.Left + 24 : r.Left + 10;
        ...
        if (showTileCheckBox && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == GridlyTileCheckBoxPosition.TopRight || TileCheckBoxPosition == GridlyTileCheckBoxPosition.BottomRight))
            textW = Math.Max(20, textW - TileCheckBoxSize - 12);
        if (showTileCheckBox && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == GridlyTileCheckBoxPosition.TopLeft || TileCheckBoxPosition == GridlyTileCheckBoxPosition.BottomLeft) && iconCol == null)
        {
            textX += TileCheckBoxSize + 12;
            textW = Math.Max(20, r.Right - textX - 8);
        }

        bool mediaLayout = TilePosterMode || ViewMode is GridlyViewMode.Poster or GridlyViewMode.Gallery or GridlyViewMode.MediaTile or GridlyViewMode.FilmStrip;
        if (mediaLayout && iconCol != null)
        {
            ...
            if (poster != null)
            {
                ...
                {
                    GridlyViewMode.Poster => PosterImageHeight,
                    GridlyViewMode.Gallery => Math.Max(110, TilePosterImageHeight),
                    GridlyViewMode.MediaTile => Math.Max(82, TilePosterImageHeight),
                    GridlyViewMode.FilmStrip => Math.Max(96, r.Height - 52),
                    _ => TilePosterImageHeight
                };
                int posterH = Math.Min(Math.Max(72, defaultImageHeight), Math.Max(72, r.Height - 54));
                var pr = ViewMode == GridlyViewMode.FilmStrip
                ...
                if (ViewMode == GridlyViewMode.FilmStrip)
                {
                    y = r.Top + 16;
                    textX = pr.Right + 14;
        ...
        {
            GridlyViewMode.LargeCard => Math.Max(1, LargeCardMaxTextLines),
            GridlyViewMode.DashboardCard => Math.Max(4, Math.Min(8, LargeCardMaxTextLines)),
            GridlyViewMode.KpiDashboard => Math.Max(2, Math.Min(4, LargeCardMaxTextLines)),
            GridlyViewMode.HeatMap => Math.Max(2, Math.Min(4, LargeCardMaxTextLines)),
            GridlyViewMode.MiniChart => Math.Max(3, Math.Min(5, LargeCardMaxTextLines)),
            GridlyViewMode.RowPreview => Math.Max(1, Math.Min(3, LargeCardMaxTextLines)),
            GridlyViewMode.GroupCard => Math.Max(4, Math.Min(7, LargeCardMaxTextLines)),
            GridlyViewMode.RowCard => Math.Max(4, Math.Min(9, LargeCardMaxTextLines)),
            GridlyViewMode.DetailCard or GridlyViewMode.PropertyCard => Math.Max(1, Columns.VisibleColumns.Count(c => c.Width > 0)),
            GridlyViewMode.Kanban => Math.Max(4, Math.Min(7, LargeCardMaxTextLines)),
            GridlyViewMode.Timeline => Math.Max(5, Math.Min(10, LargeCardMaxTextLines)),
            GridlyViewMode.Tile => Math.Max(1, TileMaxTextLines),
            GridlyViewMode.Poster => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            GridlyViewMode.Gallery => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            GridlyViewMode.MediaTile => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            GridlyViewMode.FilmStrip => Math.Max(4, Math.Min(7, TileMaxTextLines)),
            GridlyViewMode.IconGrid => 2,
            GridlyViewMode.ExtraLargeIcons => 4,
            GridlyViewMode.LargeIcons => 3,
            _ => 2
        };
    ...
    {
        var visible = Columns.VisibleColumns.Where(c => c.Kind != GridlyColumnKind.CheckBox && c.Kind != GridlyColumnKind.Icon && c.Kind != GridlyColumnKind.Image && c.Width > 0).ToList();
    ...
    private void DrawDetailCardContent(Graphics g, Rectangle r, int viewIndex, object row, Color fore, Color back, GridlyCardVisualInfo? cardVisualInfo, Color? cardStatusColor, bool reserveCheckBoxArea)
    {
        ...
        if (detailImageColumn != null &&
            (DetailCardLayout is GridlyDetailCardLayout.Media or GridlyDetailCardLayout.PosterLeft ||
             ViewMode is GridlyViewMode.DetailCard or GridlyViewMode.PropertyCard))
        {
        ...
        var textColumns = visible.Where(c => c.Kind != GridlyColumnKind.Icon && c.Kind != GridlyColumnKind.Image).ToList();
        ...
        int left = r.Left + 16;
        int rightPadding = 16 + (reserveCheckBoxArea && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == GridlyTileCheckBoxPosition.TopRight || TileCheckBoxPosition == GridlyTileCheckBoxPosition.BottomRight) ? TileCheckBoxSize + 12 : 0);
        ...
        bool propertyMode = ViewMode == GridlyViewMode.PropertyCard;
        ...
        foreach (var col in visible)
        {
            ...
            string value;
            if (col.Kind == GridlyColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))
            {
                value = GetRowCheckState(row, col) == CheckState.Checked ? "Evet" : "Hayır";
            }
            else if (col.Kind == GridlyColumnKind.Icon || col.Kind == GridlyColumnKind.Image)
            {
    ...
    private void DrawMediaDetailCardContent(Graphics g, Rectangle r, int viewIndex, object row, Color fore, Color back, GridlyCardVisualInfo? cardVisualInfo, Color? cardStatusColor, bool reserveCheckBoxArea, List<GridlyColumn> visible, GridlyColumn imageColumn)
    {
        var textColumns = visible.Where(c => c != imageColumn && c.Kind != GridlyColumnKind.CheckBox && c.Width > 0).ToList();
        var titleCol = textColumns.FirstOrDefault(c => c.Editable)
        ...
        int imageW = DetailCardLayout == GridlyDetailCardLayout.PosterLeft
            ? Math.Max(132, Math.Min(240, DetailCardMediaImageWidth))
            : Math.Max(96, Math.Min(190, DetailCardMediaImageWidth));
        int imageH = DetailCardLayout == GridlyDetailCardLayout.PosterLeft
    ...
    private string BuildMediaCompactInfo(object row, List<GridlyColumn> columns)
    {
    ...
    private GridlyColumn? FindDetailCardImageColumn(List<GridlyColumn> columns, object row)
    {
    ...
    private Image? ResolveDetailCardImage(GridlyColumn column, object row)
    {
    ...
    private static Image? TryGetColumnValueImage(GridlyColumn column, object row)
    {
        object? value;
        try
        {
    ...
    private void DrawMediaImage(Graphics g, Image image, Rectangle bounds)
    {
        ...
        {
            case GridlyMediaImageScaleMode.Cover:
                DrawImageCover(g, image, bounds);
                break;
            case GridlyMediaImageScaleMode.Stretch:
    ...
    private Color EnsureReadableTextOn(Color back, Color preferredFore)
    {
        ...
        return global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(preferredFore, back, 4.5d);
    }
    ...
    private void DrawCell(Graphics g, Rectangle r, object row, GridlyColumn col, Color fore, Color cellBack, bool isSelected, int viewIndex)
    {
        ...
        bool cellCheckBoxHost = col.CellCheckBox && col.Kind != GridlyColumnKind.CheckBox && !compatibilityCheckHost;
    ...
    private void DrawComboBoxCell(Graphics g, Rectangle r, string text, GridlyColumn col, Color fore, Color cellBack)
    {
    ...
    private void DrawTextWithFilterHighlight(Graphics g, Rectangle r, string text, GridlyColumn col, Color fore, Color cellBack)
    {
        ...
        if (columnFilter?.Mode == GridlyFilterMode.Contains && !string.IsNullOrWhiteSpace(columnFilter.Text))
    ...
    private bool ShouldDrawMultilineCell(GridlyColumn col)
    {
        if (IsTileView) return false;
        if (!AllowMultilineCells && !col.WordWrap) return false;
        if (col.Kind != GridlyColumnKind.Text && col.Kind != GridlyColumnKind.Hyperlink) return false;
        return col.WordWrap || AllowMultilineCells;
    }

    private int ResolveMultilineCellMaxLines(GridlyColumn col)
    {
        if (col.MaxTextLines > 0) return Math.Clamp(col.MaxTextLines, 1, 12);
        return Math.Clamp(MaxCellTextLines, 1, 12);
    }

    private void DrawMultilineCellText(Graphics g, Rectangle r, string text, GridlyColumn col, Color fore)
    {
    ...
    private string BuildCellOverflowKey(int viewIndex, GridlyColumn col)
    {
    ...
    private void DrawCellOverflowAdorners(Graphics g, Rectangle cellRect, Rectangle textRect, GridlyColumn col, Color fore, int offset, int maxOffset)
    {
    ...
    private void ShowCellOverflowReaderPopup(int rowIndex, GridlyColumn col, object row)
    {
    ...
    private static string ResolveButtonCellText(GridlyColumn col, object? value)
    {
    ...
    private void DrawButton(Graphics g, Rectangle r, string text, GridlyColumn? col = null)
    {
        var br = Rectangle.Inflate(r, -1, -3);
        if (col != null)
        {
            ...
            if (col.ButtonSizing == GridlyColumnButtonSizing.TextBounds && !string.IsNullOrEmpty(text))
            {
    ...
    private void DrawRating(Graphics g, Rectangle r, int value, GridlyColumn col, Color fore, Color cellBack)
    {
    ...
    private Image? ResolveHeaderImage(GridlyColumn col)
    {
    ...
    private Image? ResolveColumnImage(GridlyColumn col, object row, bool preferLarge = false)
    {
    ...
    private Image? ResolveColumnStateImage(GridlyColumn col, object row)
    {
    ...
    private bool ColumnMayDrawCellImage(GridlyColumn col)
    {
        if (col == null) return false;
        if (col.Kind == GridlyColumnKind.Image || col.Kind == GridlyColumnKind.Icon) return true;
    ...
    private Rectangle DrawColumnImagesIfNeeded(Graphics g, Rectangle r, object row, GridlyColumn col)
    {
        int x = r.X;
    ...
    private void DrawIconText(Graphics g, Rectangle r, object row, GridlyColumn col, object? val, Color fore)
    {
    ...
    private void DrawTags(Graphics g, Rectangle r, string text, GridlyColumn col, Color cellBack)
    {
        int x = r.X;
    ...
    private void DrawEmpty(Graphics g)
    {
        ...
        string title = string.IsNullOrWhiteSpace(EmptyListMessage) ? GridlyText.EmptyList : EmptyListMessage;
        ...
        Color subFore = global::Gridly.Theming.GridlyDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, _theme.PanelBackColor == Color.Empty ? _theme.BackColor : _theme.PanelBackColor);
        TextRenderer.DrawText(g, GridlyText.SearchPlaceholder, Font, subRect, subFore, TextFormatFlags.Left | TextFormatFlags.Top | TextFormatFlags.EndEllipsis);
    ...
    {
        if (!ShowEmptyStateSignature || EmptyStateSignatureAlignment == GridlyEmptyStateSignatureAlignment.Hidden) return;
        ...
        {
            case GridlyEmptyStateSignatureAlignment.BottomLeft:
                x = card.Left + pad;
                y = card.Bottom - textHeight - 6;
                break;
            case GridlyEmptyStateSignatureAlignment.TopRight:
                x = card.Right - textWidth - pad;
                y = card.Top + 6;
                break;
            case GridlyEmptyStateSignatureAlignment.TopLeft:
                x = card.Left + pad;
                y = card.Top + 6;
                break;
            case GridlyEmptyStateSignatureAlignment.CenterBottom:
                x = card.Left + (card.Width - textWidth) / 2;
                y = card.Bottom - textHeight - 6;
                break;
            case GridlyEmptyStateSignatureAlignment.BottomRight:
            default:
                x = card.Right - textWidth - pad;
                y = card.Bottom - textHeight - 6;
                break;
        ...
        Color baseFore = global::Gridly.Theming.GridlyDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, surface);
    ...
    {
        string versionText = "Gridly";
        try
        {
            var version = typeof(GridlyView).Assembly.GetName().Version;
            if (version != null)
                versionText = $"Gridly v{version.Major}.{version.Minor}.{version.Build}.{version.Revision}";
        }
        catch
        {
        }
    ...
    {
        return global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(preferred, back, 4.5d);
    }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        if (e.Button == MouseButtons.Left && TryGetTileCheckBoxHit(e.Location, rowIndex, out var tileCheckColumn) && tileCheckColumn != null)
        {
            ...
            var tileArgs = new GridlyCellClickEventArgs(rowIndex, row, tileCheckColumn);
        ...
        if (e.Button == MouseButtons.Right)
        {
            ...
            // already selected rows. This mirrors ListView/Gridly behavior and
        ...
        var cellArgs = new GridlyCellClickEventArgs(rowIndex, row, col);
        ...
        else if (leftButton && (col.Kind == GridlyColumnKind.CheckBox || col.Kind == GridlyColumnKind.ToggleSwitch)) ToggleBool(row, col);
        else if (leftButton && col.Kind == GridlyColumnKind.Rating) SetRatingFromClick(e.X, row, col);
        else if (allowMouseAction && col.Kind == GridlyColumnKind.Button) ButtonClick?.Invoke(this, cellArgs);
        else if (leftButton && col.Kind == GridlyColumnKind.Hyperlink) HyperlinkClick?.Invoke(this, cellArgs);
        else if (leftButton && col.Kind == GridlyColumnKind.ComboBox && EnableCellEditing && CanEditColumn(col)) BeginEdit(rowIndex, col);
        Invalidate();
    }
    protected override void OnDoubleClick(EventArgs e)
    {
        ...
        if (activeRow != null && col != null) ItemActivate?.Invoke(this, new GridlyCellClickEventArgs(rowIndex, activeRow, col));
    ...
    private void ShowConfiguredFilterMenuForColumn(GridlyColumn col, Point clientLocation)
    {
        if (FilterMenuMode == GridlyFilterMenuMode.PopupMenu || UseEmbeddedHeaderFilterMenu)
        {
            ShowPopupFilterDropDown(col, clientLocation, includeOpenWindowItem: true);
            return;
        }

        if (FilterMenuMode == GridlyFilterMenuMode.Both)
        {
            ShowPopupFilterDropDown(col, clientLocation, includeOpenWindowItem: true);
            return;
        }
    ...
    private void ShowPopupFilterDropDown(GridlyColumn col, Point clientLocation, bool includeOpenWindowItem)
    {
        ...
        {
            Renderer = new global::Gridly.Theming.SmartMenuRenderer(_theme),
            BackColor = _theme.PanelBackColor,
            ForeColor = _theme.ForeColor,
            AutoClose = true,
        ...
        if (includeOpenWindowItem)
        {
            ...
            menu.Items.Add(GridlyText.SeparateFilterWindow, null, (_,__) =>
            {
    ...
    private string BuildFilterPopupSizeMemoryKey(GridlyColumn col)
    {
    ...
    private Size GetInitialFilterPopupSize(GridlyColumn col, Size minimumSize, Size maximumSize, string sizeMemoryKey, bool allowRememberedSize)
    {
    ...
    private Size GetInitialFilterWindowSize(GridlyColumn col, Size minimumSize, Size maximumSize, string sizeMemoryKey, bool allowRememberedSize)
    {
        if (allowRememberedSize && FilterPopupRememberSize && _floatingFilterPopupSizeMemory.TryGetValue(sizeMemoryKey, out Size rememberedSize))
            return ClampFilterPopupSize(rememberedSize, minimumSize, maximumSize);
    ...
    private void ShowFilterMenuForColumn(GridlyColumn col, Point clientLocation)
    {
    ...
    public void SortBy(GridlyColumn? column, bool descending = false)
    {
    ...
    private void BuildViewIndexSmartSort()
    {
        ...
        Task.Run(() =>
        {
            var entries = new List<GridlySortEntry>(Math.Min(scanCount, 250_000));
            for (int i = 0; i < scanCount; i++)
            {
                ...
                entries.Add(new GridlySortEntry(i, primaryKey, secondaryKey));
            }
    ...
    public void ClearSort(GridlyColumn? column = null)
    {
    ...
    public void BeginEdit(int viewRowIndex, GridlyColumn col)
    {
    ...
    private static ICellEditor CreateBuiltInEditor(GridlyColumn col)
    {
        var type = col.EditorType;
        if (type == GridlyCellEditorKind.Auto)
        {
            type = col.Kind switch
            {
                GridlyColumnKind.CheckBox or GridlyColumnKind.ToggleSwitch => GridlyCellEditorKind.CheckBox,
                GridlyColumnKind.ComboBox => GridlyCellEditorKind.ComboBox,
                GridlyColumnKind.Numeric or GridlyColumnKind.Rating or GridlyColumnKind.ProgressBar => GridlyCellEditorKind.Numeric,
                GridlyColumnKind.Date => GridlyCellEditorKind.DateTime,
                _ => GridlyCellEditorKind.TextBox
            };
        }

        return type switch
        {
            GridlyCellEditorKind.CheckBox => new CheckBoxCellEditor(),
            GridlyCellEditorKind.ComboBox => new ComboBoxCellEditor(col.ComboBoxItems),
            GridlyCellEditorKind.Numeric => new NumericCellEditor(),
            GridlyCellEditorKind.DateTime => new DateTimeCellEditor(),
    ...
    private void SetCellValueWithUndo(int viewRowIndex, object row, GridlyColumn col, object? value)
    {
        var old = col.GetValue(row);
        col.PutValue(row, value);
        if (EnableUndoRedo) _undo.Push(new Gridly.Undo.GridlyCellChange { RowObject = row, Column = col, OldValue = old, NewValue = value });
        CellValueChanged?.Invoke(this, new GridlyCellEditEventArgs(viewRowIndex, row, col, value));
        InvalidateDataCaches(keepVersion: true);
        Invalidate();
    }
    private void ToggleBool(object row, GridlyColumn col)
    {
        if (col.Kind == GridlyColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))
        {
            ...
            CellValueChanged?.Invoke(this, new GridlyCellEditEventArgs(_selectedRow, row, col, newState == CheckState.Checked));
    ...
    private void SetRatingFromClick(int x, object row, GridlyColumn col)
    {
    ...
    private string GetSmartFilterIndexKey(GridlyColumn col) => col.AspectName + "|" + col.Header + "|" + _dataVersion.ToString(System.Globalization.CultureInfo.InvariantCulture);

    private void BeginBuildSmartFilterIndex(GridlyColumn col)
    {
    ...
    private Point GetDefaultFilterPopupClientAnchor(GridlyColumn col)
    {
        if (ShowHeader && (ViewMode == GridlyViewMode.Details || ViewMode == GridlyViewMode.DenseList || ViewMode == GridlyViewMode.GroupedList || ViewMode == GridlyViewMode.MasterDetail))
    ...
    private Point NormalizeFilterPopupClientAnchor(GridlyColumn col, Point requestedClientAnchor)
    {
    ...
    public void ShowHeaderContextMenuForColumn(GridlyColumn col, Point clientLocation)
    {
    ...
    private IReadOnlyList<string> GetDistinctColumnValues(GridlyColumn col, int maxScanRows, string? searchText = null)
    {
    ...
    private List<string> ComputeDistinctColumnValuesSnapshot(GridlyColumn col, int maxScanRows, string? searchText = null, bool useTopValueOrder = false)
    {
        var allColumns = Columns.ToArray();
    ...
    private GridlyColumn? HitColumn(int x) { if (IsTileView) return Columns.VisibleColumns.FirstOrDefault(c => c.Editable) ?? Columns.VisibleColumns.FirstOrDefault(); int xx = -_scrollX; foreach (var c in Columns.VisibleColumns) { if (x >= xx && x < xx + c.Width) return c; xx += c.Width; } return null; }
    private GridlyColumn? HitColumnResizeEdge(int x)
    {
        int xx = -_scrollX;
        foreach (var c in Columns.VisibleColumns)
        {
    ...
    private int GetColumnLeft(GridlyColumn col) { int x=-_scrollX; foreach (var c in Columns.VisibleColumns) { if (c == col) return x; x += c.Width; } return 0; }

    private void SetResizingColumnWidth(GridlyColumn column, int requestedWidth)
    {
    ...
    private int GetColumnMinimumResizeWidth(GridlyColumn column)
    {
        return Math.Max(28, column.MinimumWidth > 0 ? column.MinimumWidth : 28);
    }

    private void AbsorbHorizontalOverflowFromFillColumns(GridlyColumn resizedColumn)
    {
        var visible = Columns.VisibleColumns.ToList();
    ...
    public Rectangle GetCellBounds(int viewRowIndex, GridlyColumn col)
    {
        int top = GetRowsTopOffset();
        if (IsTileView)
        {
    ...
    protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
    {
        ...
        if (HandleGridlyKeyboardAccessibilityShortcut(keyData)) return true;
        ...
        if (keyData == Keys.Delete) { CellClick?.Invoke(this, new GridlyCellClickEventArgs(_selectedRow, SelectedObject ?? this, Columns.VisibleColumns.FirstOrDefault() ?? new GridlyColumn("", "", 0))); return true; }
    ...
    private bool TryActivateSelectedButtonCell()
    {
        ...
        var row = GetViewRow(_selectedRow);
        var col = _activeColumn != null && _activeColumn.Kind == GridlyColumnKind.Button
            ? _activeColumn
            : Columns.VisibleColumns.FirstOrDefault(c => c.Kind == GridlyColumnKind.Button);
        if (row == null || col == null) return false;
        ButtonClick?.Invoke(this, new GridlyCellClickEventArgs(_selectedRow, row, col));
        return true;
    }
    ...
    private bool TryToggleSelectedCheckBoxesFromKeyboard()
    {
        ...
        foreach (var row in targetRows)
        {
            ...
            CellValueChanged?.Invoke(this, new GridlyCellEditEventArgs(IndexOfObject(row), row, col, nextState == CheckState.Checked));
    ...
    private bool CanEditColumn(GridlyColumn? col)
    {
        if (col == null) return false;
    ...
    private ToolStripMenuItem CreateEmbeddedFilterMenuItem(GridlyColumn col)
    {
        var root = new ToolStripMenuItem(GridlyText.Filtering);
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        ...
        root.DropDownItems.Add(GridlyText.SortAscending, null, (_,__) => { SortBy(col, false); CloseActiveMenusSafe(); }).Enabled = col.Sortable;
        root.DropDownItems.Add(GridlyText.SortDescending, null, (_,__) => { SortBy(col, true); CloseActiveMenusSafe(); }).Enabled = col.Sortable;
        root.DropDownItems.Add(GridlyText.Unsort, null, (_,__) => { ClearSort(col); CloseActiveMenusSafe(); }).Enabled = _sortColumn == col;
        ...
        var caption = new Label { Text = GridlyText.ColumnFilterTitle(col.Header), Left = 8, Top = 6, Width = 272, Height = 20, ForeColor = filterPanelFore, BackColor = filterPanelBack, Font = new Font(Font, FontStyle.Bold) };
        var search = new TextBox { PlaceholderText = GridlyText.SearchPlaceholder, Left = 8, Top = 30, Width = 272, BackColor = filterControlBack, ForeColor = filterControlFore, BorderStyle = BorderStyle.FixedSingle };
        var useTextFilter = new CheckBox { Text = GridlyText.ApplyTypedTextAsFilter, Left = 8, Top = 58, Width = 272, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat };
        if (TypedFilterSearchesAllRows && FastFilterMenuForHugeLists && _provider.Count > 20_000) useTextFilter.Checked = true;
        var selectAll = new CheckBox { Text = GridlyText.SelectAll, Left = 8, Top = 84, Width = 272, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat, ThreeState = true, AutoCheck = false };
        ...
        var previewInfo = new Label { Text = IsHugeFastFilterPreview() ? GridlyText.FastPreviewInfo : string.Empty, Left = 8, Top = 304, Width = 272, Height = 16, ForeColor = filterMutedFore, BackColor = filterPanelBack };
        ...
        var limitedLabel = new Label { Text = GridlyText.FastPreviewLimitedInfo, Left = 28, Top = 4, Width = 212, Height = 34, BackColor = limitedInfo.BackColor, ForeColor = Color.Black };
        ...
        var clear = new Button { Text = GridlyText.Clear, Left = 112, Top = 374, Width = 78, Height = 28, BackColor = filterControlBack, ForeColor = filterControlFore, FlatStyle = FlatStyle.Flat };
        var apply = new Button { Text = GridlyText.Apply, Left = 202, Top = 374, Width = 78, Height = 28, BackColor = _theme.AccentColor, ForeColor = GetReadableTextColor(_theme.AccentColor), FlatStyle = FlatStyle.Flat };
        ...
        bool IsChecked(string value) => current?.Mode == GridlyFilterMode.ValueList && current.SelectedValues != null ? current.SelectedValues.Contains(value) : true;
        void Refill()
        {
            ...
            if (previewInfo != null)
                previewInfo.Text = fullValuesLoaded ? string.Format("{0:N0}", values.Count) : GridlyText.FastPreviewInfo;
            if (limitedInfo != null)
        ...
        search.TextChanged += (_,__) =>
        {
            ...
            if (!fullValuesLoaded && !string.IsNullOrWhiteSpace(search.Text) && previewInfo != null)
                previewInfo.Text = GridlyText.SearchingPreviewResults;
        ...
        apply.Click += (_,__) =>
        {
            ...
            {
                SetColumnFilter(new GridlyColumnFilter { AspectName = col.AspectName, Mode = GridlyFilterMode.Contains, Text = q, Enabled = true });
                CloseActiveMenusSafe();
                return;
            }
            ...
            {
                SetColumnFilter(new GridlyColumnFilter { AspectName = col.AspectName, Mode = GridlyFilterMode.ValueList, SelectedValues = selected, Enabled = true });
            }
            CloseActiveMenusSafe();
        };
        ...
        root.DropDownItems.Add(new ToolStripSeparator());
        root.DropDownItems.Add(GridlyText.ClearAllFilters, null, (_,__) => { ClearFilters(); CloseActiveMenusSafe(); });
        root.DropDownItems.Add(GridlyText.SeparateFilterWindow, null, (_,__) => { ShowFilterMenuForColumn(col, new Point(GetColumnLeft(col), HeaderHeight)); CloseActiveMenusSafe(); });
        return root;
    }
    ...
    {
        return global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(_theme.ForeColor, backColor, 4.5d);
    }

    private Color GetFilterPopupPanelBack() => global::Gridly.Theming.GridlyDialogThemeApplier.NormalizePanelBack(_theme);
    private Color GetFilterPopupControlBack() => global::Gridly.Theming.GridlyDialogThemeApplier.NormalizeControlBack(_theme);
    private Color GetFilterPopupPanelFore() => global::Gridly.Theming.GridlyDialogThemeApplier.NormalizePanelFore(_theme);
    private Color GetFilterPopupControlFore() => global::Gridly.Theming.GridlyDialogThemeApplier.NormalizeControlFore(_theme);
    private Color GetFilterPopupMutedFore(Color back) => global::Gridly.Theming.GridlyDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, back);
    ...
    private void ApplyFilterPopupTheme(Control root)
    {
        ...
        foreach (Control control in EnumerateFilterPopupControls(root))
        {
            ...
            if (control is Label label)
            {
                ...
                label.ForeColor = global::Gridly.Theming.GridlyDialogThemeApplier.EnsureReadableTextColor(label.ForeColor == Color.Empty ? panelFore : label.ForeColor, back);
                continue;
            }
            ...
            if (control is Panel)
            {
                ...
                control.ForeColor = global::Gridly.Theming.GridlyDialogThemeApplier.EnsureReadableTextColor(control.ForeColor == Color.Empty ? panelFore : control.ForeColor, control.BackColor);
                continue;
            }
    ...
    private ToolStripMenuItem CreateStableFilterLauncherMenuItem(ContextMenuStrip ownerMenu, GridlyColumn col)
    {
        var launcher = new ToolStripMenuItem(GridlyText.Filtering);
        bool opening = false;
    ...
    private Control CreateFloatingFilterActionLabel(string icon, string text, int left, int top, int width, bool enabled, bool active, Action clickAction)
    {
        ...
        Color activeBack = GridlyDialogThemeApplier.Blend(panelBack, _theme.AccentColor, _theme.IsDark ? 0.38 : 0.16);
        Color hoverBack = GridlyDialogThemeApplier.Blend(panelBack, _theme.AccentColor, _theme.IsDark ? 0.25 : 0.10);
    ...
    private void ShowFloatingEmbeddedFilterForm(ContextMenuStrip? ownerMenu, GridlyColumn col, Point screenLocation)
    {
        if (_activeFilterPopupForm != null && !_activeFilterPopupForm.IsDisposed)
        {
            try { _activeFilterPopupForm.Close(); } catch { }
        ...
        var sortAsc = CreateFloatingFilterActionLabel("▲", GridlyText.SortAscending, 12, 10, 310, col.Sortable, _sortColumn == col && !_sortDesc, () => { SortBy(col, false); popup.Close(); });
        var sortDesc = CreateFloatingFilterActionLabel("▼", GridlyText.SortDescending, 12, 34, 310, col.Sortable, _sortColumn == col && _sortDesc, () => { SortBy(col, true); popup.Close(); });
        var unsort = CreateFloatingFilterActionLabel("↕", GridlyText.Unsort, 12, 58, 310, _sortColumn == col, false, () => { ClearSort(col); popup.Close(); });
        var sep1 = new Panel { Left = 10, Top = 84, Width = 326, Height = 1, BackColor = _theme.BorderColor };
        var caption = new Label { Text = GridlyText.ColumnFilterTitle(col.Header), Left = 16, Top = 100, Width = 310, Height = 22, ForeColor = filterPanelFore, BackColor = filterPanelBack, Font = new Font(Font, FontStyle.Bold) };
        var search = new TextBox { PlaceholderText = GridlyText.SearchPlaceholder, Left = 16, Top = 126, Width = 300, Height = 24, BackColor = filterControlBack, ForeColor = filterControlFore, BorderStyle = BorderStyle.FixedSingle };
        var useTextFilter = new CheckBox { Text = GridlyText.ApplyTypedTextAsFilter, Left = 16, Top = 158, Width = 310, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat };
        if (TypedFilterSearchesAllRows && FastFilterMenuForHugeLists && _provider.Count > 20_000) useTextFilter.Checked = true;
        var selectAll = new CheckBox { Text = GridlyText.SelectAll, Left = 16, Top = 184, Width = 310, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat, ThreeState = true, AutoCheck = false };
        ...
        var previewInfo = new Label { Text = IsHugeFastFilterPreview() ? GridlyText.FastPreviewInfo : string.Empty, Left = 16, Top = 394, Width = 300, Height = 16, ForeColor = filterMutedFore, BackColor = filterPanelBack };
        ...
        var limitedLabel = new Label { Text = GridlyText.FastPreviewLimitedInfo, Left = 30, Top = 4, Width = 238, Height = 38, BackColor = limitedInfo.BackColor, ForeColor = Color.Black };
        ...
        limitedClose.Click += (_,__) => limitedInfo.Visible = false;
        limitedInfo.Controls.AddRange(new Control[] { limitedIcon, limitedLabel, limitedClose });
        var clear = new Button { Text = GridlyText.Clear, Left = 130, Top = 470, Width = 78, Height = 30, BackColor = filterControlBack, ForeColor = filterControlFore, FlatStyle = FlatStyle.Flat };
        var apply = new Button { Text = GridlyText.Apply, Left = 220, Top = 470, Width = 78, Height = 30, BackColor = _theme.AccentColor, ForeColor = GetReadableTextColor(_theme.AccentColor), FlatStyle = FlatStyle.Flat };
        var sep2 = new Panel { Left = 10, Top = 510, Width = 326, Height = 1, BackColor = _theme.BorderColor };
        var openWindow = CreateFloatingFilterActionLabel("▽", GridlyText.SeparateFilterWindow, 12, 520, 310, true, false, () => { popup.Close(); BeginInvoke(new Action(() => ShowFilterMenuForColumn(col, PointToClient(popup.Location)))); });
        var clearAll = CreateFloatingFilterActionLabel("×", GridlyText.ClearAllFilters, 12, 542, 310, true, current != null, () => { ClearFilters(); popup.Close(); });
        ...
        apply.Click += (_,__) =>
        {
            ...
            else SetColumnFilter(new GridlyColumnFilter { AspectName = col.AspectName, Mode = GridlyFilterMode.ValueList, SelectedValues = selected, Enabled = true });
            CloseAllMenus();
        };
    ...
    {
        RemoveMenuDismissMessageFilter();
        _activeMenuDismissFilter = new GridlyMenuDismissMessageFilter(
            isInsideAllowedArea: () =>
            {
                var p = Control.MousePosition;
                bool insideOwner = ownerMenu != null && !ownerMenu.IsDisposed && ownerMenu.Bounds.Contains(p);
    ...
    private bool IsMenuGroupVisible(GridlyMenuGroups group, bool header)
    {
        if (MenuProfile == GridlyMenuProfile.None) return false;
        GridlyMenuGroups groups = header ? HeaderMenuGroups : BodyMenuGroups;
        if ((groups & group) != group) return false;

        return MenuProfile switch
        {
            GridlyMenuProfile.Full => true,
            GridlyMenuProfile.Custom => true,
            GridlyMenuProfile.Standard => group is GridlyMenuGroups.Filter or GridlyMenuGroups.Sort or GridlyMenuGroups.AutoSize or GridlyMenuGroups.ColumnChooser or GridlyMenuGroups.ViewMode or GridlyMenuGroups.State or GridlyMenuGroups.Scenario or GridlyMenuGroups.Clipboard or GridlyMenuGroups.Editing or GridlyMenuGroups.RowDetails,
            GridlyMenuProfile.Minimal => group is GridlyMenuGroups.Filter or GridlyMenuGroups.Sort or GridlyMenuGroups.AutoSize or GridlyMenuGroups.Clipboard,
            GridlyMenuProfile.ReadOnly => group is GridlyMenuGroups.Filter or GridlyMenuGroups.Sort or GridlyMenuGroups.AutoSize or GridlyMenuGroups.ColumnChooser or GridlyMenuGroups.ViewMode or GridlyMenuGroups.State or GridlyMenuGroups.Scenario or GridlyMenuGroups.Clipboard or GridlyMenuGroups.RowDetails,
            _ => true
        };
    }

    public void ApplyMenuProfile(GridlyMenuProfile profile)
    {
        MenuProfile = profile;
        switch (profile)
        {
            case GridlyMenuProfile.None:
                UseBuiltInHeaderMenu = false;
                UseBuiltInBodyMenu = false;
                HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.None;
                break;
            case GridlyMenuProfile.Full:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = GridlyMenuGroups.All;
                BodyMenuGroups = GridlyMenuGroups.All;
                break;
            case GridlyMenuProfile.Standard:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = GridlyMenuGroups.Filter | GridlyMenuGroups.Sort | GridlyMenuGroups.AutoSize | GridlyMenuGroups.ColumnChooser | GridlyMenuGroups.ViewMode | GridlyMenuGroups.State | GridlyMenuGroups.Scenario;
                BodyMenuGroups = GridlyMenuGroups.Clipboard | GridlyMenuGroups.Editing | GridlyMenuGroups.RowDetails | GridlyMenuGroups.AutoSize | GridlyMenuGroups.ColumnChooser | GridlyMenuGroups.ViewMode | GridlyMenuGroups.State | GridlyMenuGroups.Scenario;
                break;
            case GridlyMenuProfile.Minimal:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = GridlyMenuGroups.Filter | GridlyMenuGroups.Sort | GridlyMenuGroups.AutoSize;
                BodyMenuGroups = GridlyMenuGroups.Clipboard | GridlyMenuGroups.AutoSize;
                break;
            case GridlyMenuProfile.ReadOnly:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = GridlyHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = GridlyMenuGroups.Filter | GridlyMenuGroups.Sort | GridlyMenuGroups.AutoSize | GridlyMenuGroups.ColumnChooser | GridlyMenuGroups.ViewMode | GridlyMenuGroups.State | GridlyMenuGroups.Scenario;
                BodyMenuGroups = GridlyMenuGroups.Clipboard | GridlyMenuGroups.RowDetails | GridlyMenuGroups.AutoSize | GridlyMenuGroups.ColumnChooser | GridlyMenuGroups.ViewMode | GridlyMenuGroups.State | GridlyMenuGroups.Scenario;
                break;
        }
        Invalidate();
    }

    public void ShowMenuGroup(GridlyMenuGroups group, bool visible, bool header = true, bool body = true)
    {
        ...
        if (MenuProfile != GridlyMenuProfile.None) MenuProfile = GridlyMenuProfile.Custom;
    }

    private void ShowBuiltInContextMenu(Point location)
    {
        bool isHeaderMenu = ShowHeader && location.Y < HeaderHeight;
        if (MenuProfile == GridlyMenuProfile.None) return;
        ...
        if (ShowHeader && location.Y < HeaderHeight)
        {
            ...
            if (HeaderContextMenuBehavior == GridlyHeaderContextMenuBehavior.None) return;
            if (HeaderContextMenuBehavior == GridlyHeaderContextMenuBehavior.FilterOnly)
            {
        ...
        if (ShowHeader && location.Y < HeaderHeight)
        {
            ...
            if (col != null)
            {
                if (ShowHeaderMenuFilterItems && IsMenuGroupVisible(GridlyMenuGroups.Filter, true) && ShowFilterMenu && col.Filterable)
                {
                    if (FilterMenuMode == GridlyFilterMenuMode.PopupMenu || FilterMenuMode == GridlyFilterMenuMode.Both || UseEmbeddedHeaderFilterMenu)
                    {
                    ...
                    if (FilterMenuMode == GridlyFilterMenuMode.ModalWindow || FilterMenuMode == GridlyFilterMenuMode.Both)
                    {
                        menu.Items.Add(GridlyText.FilterWindow(col.Header), null, (_,__) =>
                        {
                    ...
                    if (ShowHeaderMenuFilterStyleItems && IsMenuGroupVisible(GridlyMenuGroups.FilterStyle, true) && ShowFilterStyleSelectorInContextMenu) AddFilterStyleMenu(menu);
                    ...
                    if (ShowQuickClearFilterInHeaderMenu)
                    {
                        ...
                        if (colFilter != null)
                            menu.Items.Add(GridlyText.ClearColumnFilter, null, (_,__) => { _filters.Clear(col.AspectName); BuildViewIndex(); Invalidate(); QueueAutoSaveUserLayout(); });
                        if (HasActiveFilters)
                            menu.Items.Add(GridlyText.ClearAllFilters, null, (_,__) => ClearFilters());
                    }
                    menu.Items.Add(new ToolStripSeparator());
                }
                if (ShowHeaderMenuSortItems && IsMenuGroupVisible(GridlyMenuGroups.Sort, true))
                {
                    AddSortMenu(menu.Items, col);
                    menu.Items.Add(new ToolStripSeparator());
                }
                if (ShowHeaderMenuFreezeItems && IsMenuGroupVisible(GridlyMenuGroups.Freeze, true))
                {
                    AddFreezeMenu(menu.Items, col);
                }
                if (ShowHeaderMenuAutoSizeItems && IsMenuGroupVisible(GridlyMenuGroups.AutoSize, true))
                {
                    AddAutoSizeMenu(menu.Items, col);
                }
                if (ShowHeaderMenuLayoutItems && IsMenuGroupVisible(GridlyMenuGroups.Layout, true))
                {
                    AddLayoutMenu(menu.Items);
                }
                if (ShowHeaderMenuFreezeItems || ShowHeaderMenuAutoSizeItems || ShowHeaderMenuLayoutItems)
                    menu.Items.Add(new ToolStripSeparator());
                if (ShowHeaderMenuGroupingItems && IsMenuGroupVisible(GridlyMenuGroups.Grouping, true))
                {
                    AddGroupingMenu(menu.Items, col);
                }
                if (ShowViewModeMenuItems && IsMenuGroupVisible(GridlyMenuGroups.ViewMode, true)) AddViewModeMenu(menu);
                if ((IsMenuGroupVisible(GridlyMenuGroups.Scenario, true) && ShowScenarioMenuItems) || (IsMenuGroupVisible(GridlyMenuGroups.State, true) && ShowStateMenuItems))
                {
                    menu.Items.Add(new ToolStripSeparator());
                    if (IsMenuGroupVisible(GridlyMenuGroups.Scenario, true) && ShowScenarioMenuItems) AddScenarioMenu(menu.Items);
                    if (IsMenuGroupVisible(GridlyMenuGroups.State, true) && ShowStateMenuItems) AddStateMenu(menu.Items);
                }
                if (ShowHeaderMenuColumnChooserItem && IsMenuGroupVisible(GridlyMenuGroups.ColumnChooser, true))
                {
                    menu.Items.Add(new ToolStripSeparator());
        ...
        else
        {
            ...
            if (IsMenuGroupVisible(GridlyMenuGroups.Clipboard, false))
            {
                menu.Items.Add(GridlyText.Copy, null, (_,__) => CopySelectionToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                menu.Items.Add(GridlyText.CopyCell, null, (_,__) => CopySelectedCellToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                menu.Items.Add(GridlyText.CopySelectionJson, null, (_,__) => CopySelectionAsJsonToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                if (IsMenuGroupVisible(GridlyMenuGroups.Analytics, false))
                    menu.Items.Add(GridlyText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
                menu.Items.Add(GridlyText.Paste, null, (_,__) => PasteTextToSelectedCell(Clipboard.GetText())).Enabled = EnableClipboard && SelectedObject != null && MenuProfile != GridlyMenuProfile.ReadOnly;
            }
            if (IsMenuGroupVisible(GridlyMenuGroups.Editing, false) && MenuProfile != GridlyMenuProfile.ReadOnly)
                AddEditCellMenuItem(menu.Items);
            menu.Items.Add(new ToolStripSeparator());
            if (IsMenuGroupVisible(GridlyMenuGroups.Filter, false))
                menu.Items.Add(GridlyText.ClearFilters, null, (_,__) => ClearFilters()).Enabled = HasActiveFilters;
            if (ShowHeaderMenuFilterStyleItems && ShowFilterStyleSelectorInContextMenu && IsMenuGroupVisible(GridlyMenuGroups.FilterStyle, false))
                AddFilterStyleMenu(menu);
            if (ShowAdvancedFilterMenuItems) AddAdvancedFilterMenu(menu.Items);
                    AddCardLayoutDesignerMenu(menu.Items);
            if (ShowViewModeMenuItems && IsMenuGroupVisible(GridlyMenuGroups.ViewMode, false)) AddViewModeMenu(menu);
            if ((IsMenuGroupVisible(GridlyMenuGroups.Scenario, false) && ShowScenarioMenuItems) || (IsMenuGroupVisible(GridlyMenuGroups.State, false) && ShowStateMenuItems))
            {
                if (IsMenuGroupVisible(GridlyMenuGroups.Scenario, false) && ShowScenarioMenuItems) AddScenarioMenu(menu.Items);
                if (IsMenuGroupVisible(GridlyMenuGroups.State, false) && ShowStateMenuItems) AddStateMenu(menu.Items);
            }
            if (IsMenuGroupVisible(GridlyMenuGroups.AutoSize, false))
                AddAutoSizeMenu(menu.Items, null);
            if (IsMenuGroupVisible(GridlyMenuGroups.Layout, false))
                AddLayoutMenu(menu.Items);
            if (IsMenuGroupVisible(GridlyMenuGroups.Grouping, false))
                AddGroupingMenu(menu.Items, null);
            if (IsMenuGroupVisible(GridlyMenuGroups.ColumnChooser, false)) AddColumnChooserMenu(menu.Items);
            if (ShowHeaderMenuThemeItems && IsMenuGroupVisible(GridlyMenuGroups.Theme, false)) AddThemeMenu(menu.Items);
            if (IsMenuGroupVisible(GridlyMenuGroups.Analytics, false) && !IsMenuGroupVisible(GridlyMenuGroups.Clipboard, false))
                menu.Items.Add(GridlyText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
            menu.Items.Add(new ToolStripSeparator());
            if (EnableRowDetails && IsMenuGroupVisible(GridlyMenuGroups.RowDetails, false))
                menu.Items.Add(GridlyText.RowDetails, null, (_,__) => ToggleRowDetails(_selectedRow)).Enabled = SelectedObject != null;
            }
        }
    ...
    public void AttachBuiltInMenuTo(ContextMenuStrip userMenu, Point? clientLocation = null, string? menuText = null)
    {
        ...
        const string mergeTag = "__GridlyBuiltInMergedMenu__";
        RemoveMergedGridlyMenuItems(userMenu.Items, mergeTag);

        var text = string.IsNullOrWhiteSpace(menuText)
            ? (string.IsNullOrWhiteSpace(BuiltInMenuMergeText) ? GridlyText.BuiltInMenuMergeText : BuiltInMenuMergeText.Trim())
            : menuText.Trim();

        var location = clientLocation ?? PointToClient(Cursor.Position);
        var mergedItems = CreateGridlyMergedMenuItems(text, location, mergeTag);
        ...
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(userMenu, _theme);
    }

    private static void RemoveMergedGridlyMenuItems(ToolStripItemCollection items, string mergeTag)
    {
    ...
    private int GetMergeInsertIndex(ToolStripItemCollection items)
    {
        ...
        {
            case GridlyMenuMergePlacement.Top:
                return 0;
            case GridlyMenuMergePlacement.BeforeFirstSeparator:
                for (int i = 0; i < items.Count; i++)
                    if (items[i] is ToolStripSeparator) return i;
                return items.Count;
            case GridlyMenuMergePlacement.AfterFirstSeparator:
                for (int i = 0; i < items.Count; i++)
                    if (items[i] is ToolStripSeparator) return Math.Min(items.Count, i + 1);
                return items.Count;
            case GridlyMenuMergePlacement.Bottom:
            default:
                return items.Count;
        }
    }

    private List<ToolStripItem> CreateGridlyMergedMenuItems(string text, Point location, string mergeTag)
    {
        var result = new List<ToolStripItem>();

        if (BuiltInMenuMergePresentation == GridlyMenuMergePresentation.SubMenu)
        {
            var root = CreateGridlyMergedMenu(text, location);
        ...
        foreach (var group in GetMergedMenuGroupOrder())
        {
            ...
            if (BuiltInMenuMergePresentation == GridlyMenuMergePresentation.GroupedSubMenus)
            {
                var groupMenu = CreateGridlyMergedGroupMenu(group, location);
    ...
    private IEnumerable<GridlyMenuGroups> GetMergedMenuGroupOrder()
    {
        var used = new HashSet<GridlyMenuGroups>();
        foreach (var token in (MergedMenuGroupOrder ?? string.Empty).Split(new[] { ',', ';', '|', ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
            if (Enum.TryParse(token.Trim(), true, out GridlyMenuGroups group) && group != GridlyMenuGroups.None && group != GridlyMenuGroups.All && used.Add(group))
                yield return group;
        }

        GridlyMenuGroups[] defaults =
        {
            GridlyMenuGroups.Mode,
            GridlyMenuGroups.ViewMode,
            GridlyMenuGroups.Filter,
            GridlyMenuGroups.Sort,
            GridlyMenuGroups.ColumnChooser,
            GridlyMenuGroups.Clipboard,
            GridlyMenuGroups.AutoSize,
            GridlyMenuGroups.Freeze,
            GridlyMenuGroups.Layout,
            GridlyMenuGroups.Grouping,
            GridlyMenuGroups.Editing,
            GridlyMenuGroups.RowDetails,
            GridlyMenuGroups.Analytics,
            GridlyMenuGroups.FilterStyle,
            GridlyMenuGroups.Theme
        };
    ...
    private bool IsMergedMenuGroupVisible(GridlyMenuGroups group, Point location)
    {
    ...
    private ToolStripMenuItem CreateGridlyMergedGroupMenu(GridlyMenuGroups group, Point location)
    {
        ...
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(menu.DropDown, _theme);
        return menu;
    }

    private string GetMergedGroupText(GridlyMenuGroups group)
    {
        return group switch
        {
            GridlyMenuGroups.Filter => GridlyText.Filtering,
            GridlyMenuGroups.Sort => GridlyText.Sorting,
            GridlyMenuGroups.Freeze => GridlyText.Freeze,
            GridlyMenuGroups.AutoSize => GridlyText.ColumnWidth,
            GridlyMenuGroups.Layout => GridlyText.Layout,
            GridlyMenuGroups.Grouping => GridlyText.Grouping,
            GridlyMenuGroups.ColumnChooser => GridlyText.Columns,
            GridlyMenuGroups.Mode => GridlyText.Mode,
            GridlyMenuGroups.ViewMode => GridlyText.View,
            GridlyMenuGroups.FilterStyle => GridlyText.FilterStyle,
            GridlyMenuGroups.Theme => GridlyText.Theme,
            GridlyMenuGroups.Clipboard => GridlyText.Clipboard,
            GridlyMenuGroups.Editing => GridlyText.Editing,
            GridlyMenuGroups.RowDetails => GridlyText.RowDetails,
            GridlyMenuGroups.Analytics => GridlyText.Analytics,
            _ => group.ToString()
        };
    }

    private void AddMergedGroupItems(List<ToolStripItem> result, GridlyMenuGroups group, Point location, object? tag)
    {
    ...
    private void AddMergedGroupItems(ToolStripItemCollection items, GridlyMenuGroups group, Point location)
    {
        bool header = ShowHeader && location.Y < HeaderHeight;
        ...
        switch (group)
        {
            case GridlyMenuGroups.Mode:
                if (ShowHeaderMenuModeItems) AddModeMenu(items);
                break;
            case GridlyMenuGroups.ViewMode:
                if (ShowHeaderMenuViewModeItems) AddViewModeMenuItems(items);
                break;
            case GridlyMenuGroups.Filter:
                if (col != null && ShowHeaderMenuFilterItems && ShowFilterMenu && col.Filterable)
                {
                    if (FilterMenuMode != GridlyFilterMenuMode.ModalWindow || UseEmbeddedHeaderFilterMenu)
                        items.Add(CreateEmbeddedFilterMenuItem(col));
                    items.Add(GridlyText.FilterWindow(col.Header), null, (_,__) => ShowFilterMenuForColumn(col, location)).Enabled = true;
                    if (ShowQuickClearFilterInHeaderMenu)
                    {
                        if (_filters.Get(col.AspectName) != null)
                            items.Add(GridlyText.ClearColumnFilter, null, (_,__) => { _filters.Clear(col.AspectName); BuildViewIndex(); Invalidate(); QueueAutoSaveUserLayout(); });
                        if (HasActiveFilters)
                            items.Add(GridlyText.ClearAllFilters, null, (_,__) => ClearFilters());
                    }
                }
                else
                {
                    items.Add(GridlyText.ClearFilters, null, (_,__) => ClearFilters()).Enabled = HasActiveFilters;
                }
                break;
            case GridlyMenuGroups.FilterStyle:
                ...
                break;
            case GridlyMenuGroups.Sort:
                if (col != null && ShowHeaderMenuSortItems) AddSortMenu(items, col);
                break;
            case GridlyMenuGroups.Freeze:
                if (col != null && ShowHeaderMenuFreezeItems) AddFreezeMenu(items, col);
                break;
            case GridlyMenuGroups.AutoSize:
                if (ShowHeaderMenuAutoSizeItems) AddAutoSizeMenu(items, col);
                break;
            case GridlyMenuGroups.Layout:
                if (ShowHeaderMenuLayoutItems) AddLayoutMenu(items);
                break;
            case GridlyMenuGroups.Grouping:
                if (ShowHeaderMenuGroupingItems) AddGroupingMenu(items, col);
                break;
            case GridlyMenuGroups.ColumnChooser:
                if (ShowHeaderMenuColumnChooserItem) AddColumnChooserMenu(items);
                break;
            case GridlyMenuGroups.Theme:
                if (ShowHeaderMenuThemeItems) AddThemeMenu(items);
                break;
            case GridlyMenuGroups.State:
                if (ShowStateMenuItems) AddStateMenu(items);
                break;
            case GridlyMenuGroups.Scenario:
                if (ShowScenarioMenuItems) AddScenarioMenu(items);
                break;
            case GridlyMenuGroups.Clipboard:
                items.Add(GridlyText.Copy, null, (_,__) => CopySelectionToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(GridlyText.CopyCell, null, (_,__) => CopySelectedCellToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(GridlyText.CopySelectionJson, null, (_,__) => CopySelectionAsJsonToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(GridlyText.Paste, null, (_,__) => PasteTextToSelectedCell(Clipboard.GetText())).Enabled = EnableClipboard && SelectedObject != null && MenuProfile != GridlyMenuProfile.ReadOnly;
                break;
            case GridlyMenuGroups.Editing:
                if (MenuProfile != GridlyMenuProfile.ReadOnly)
                    AddEditCellMenuItem(items);
                break;
            case GridlyMenuGroups.RowDetails:
                if (EnableRowDetails)
                    items.Add(GridlyText.RowDetails, null, (_,__) => ToggleRowDetails(_selectedRow)).Enabled = SelectedObject != null;
                break;
            case GridlyMenuGroups.Analytics:
                items.Add(GridlyText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
                break;
        }
    }
    ...
    {
        return string.IsNullOrWhiteSpace(EditCellMenuText) ? GridlyText.EditCell : EditCellMenuText.Trim();
    }
    ...
    public void AttachBuiltInHeaderMenuTo(ContextMenuStrip userMenu, GridlyColumn? column = null, string? menuText = null)
    {
        if (userMenu == null || userMenu.IsDisposed) return;
        ...
        AttachBuiltInMenuTo(userMenu, location, menuText ?? GridlyText.BuiltInHeaderMenu);
    }
    ...
    private ToolStripMenuItem CreateGridlyMergedMenu(string text, Point location)
    {
    ...
    private void AddViewModeMenuItem(ToolStripItemCollection items, GridlyViewMode mode)
    {
    ...
    {
        var viewMenu = new ToolStripMenuItem(GridlyText.ViewMode)
        {
            ToolTipText = "Gridly görünümünü tablo, kart, medya, dashboard veya workflow senaryosuna göre değiştirir."
        };
        ...
        {
            ApplyV40AnalyticsProfile(GridlyV40AnalyticsPreset.KpiDashboard);
        });
        ...
        {
            ApplyV40AnalyticsProfile(GridlyV40AnalyticsPreset.FactoryOverview);
        });
        AddPresetMenuItem(presets.DropDownItems, "Timeline akışı", "Olay, iş emri veya ticket zaman akışı görünümüne geçer.", () =>
        {
            ApplyV40AnalyticsProfile(GridlyV40AnalyticsPreset.Timeline);
        });
        ...
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Müzik", GridlyMediaSmartPreset.Music, "Albüm kapağı, now playing ve equalizer odaklı MediaTile.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Film", GridlyMediaSmartPreset.Movie, "Afiş/poster, kalite rozeti ve playback overlay odaklı görünüm.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Fotoğraf", GridlyMediaSmartPreset.Photo, "Galeri, cover ölçekleme ve görsel katalog modu.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Doküman", GridlyMediaSmartPreset.Document, "PDF/doküman önizleme için contain ölçekli DetailCard.");
        ...
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "KPI", GridlyDashboardWidgetKind.Kpi, GridlyViewMode.KpiDashboard);
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "HeatMap", GridlyDashboardWidgetKind.HeatMap, GridlyViewMode.HeatMap);
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "MiniChart", GridlyDashboardWidgetKind.Chart, GridlyViewMode.MiniChart);
        dashboard.DropDownItems.Add(new ToolStripSeparator());
        dashboard.DropDownItems.Add("Performans özeti", null, (_, __) =>
        {
            MessageBox.Show(FindForm(), GetPerformanceSummary(), "Gridly Performance", MessageBoxButtons.OK, MessageBoxIcon.Information);
            CloseActiveMenusSafe();
        });
        dashboard.DropDownItems.Add("Diagnostics Center", CreateMenuDotIcon(Color.FromArgb(120, 155, 255)), (_, __) =>
        {
            ShowGridlyDiagnosticsCenter();
            CloseActiveMenusSafe();
        });
        ...
        AddViewModeGroup(viewMenu.DropDownItems, "Tablo ve liste", "Yoğun veri, klasik tablo ve sade liste görünümleri.",
            GridlyViewMode.Details,
            GridlyViewMode.DenseList,
            GridlyViewMode.List,
            GridlyViewMode.GroupedList);

        AddViewModeGroup(viewMenu.DropDownItems, "Kart ve detay", "Ticket, ürün, master/detail ve okunabilir kart görünümleri.",
            GridlyViewMode.Tile,
            GridlyViewMode.LargeCard,
            GridlyViewMode.RowCard,
            GridlyViewMode.RowPreview,
            GridlyViewMode.DetailCard,
            GridlyViewMode.PropertyCard,
            GridlyViewMode.MasterDetail);
        ...
        AddViewModeGroup(viewMenu.DropDownItems, "Medya ve galeri", "Müzik, film, resim, kapak ve katalog senaryoları.",
            ...
            GridlyViewMode.Gallery,
            GridlyViewMode.MediaTile,
            GridlyViewMode.FilmStrip,
            GridlyViewMode.IconGrid,
            GridlyViewMode.ExtraLargeIcons,
            GridlyViewMode.LargeIcons,
            GridlyViewMode.MediumIcons);

        AddViewModeGroup(viewMenu.DropDownItems, "Dashboard ve analiz", "KPI, dashboard, heatmap ve mini trend görünümleri.",
            GridlyViewMode.DashboardCard,
            GridlyViewMode.KpiDashboard,
            GridlyViewMode.HeatMap,
            GridlyViewMode.MiniChart);

        AddViewModeGroup(viewMenu.DropDownItems, "Workflow", "Kanban, timeline ve grup kart akışları.",
            GridlyViewMode.Kanban,
            GridlyViewMode.Timeline,
            GridlyViewMode.GroupCard);

        RemoveRedundantMenuSeparators(viewMenu.DropDownItems);
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(viewMenu.DropDown, _theme);
        return viewMenu;
    }

    private void AddMediaPresetMenuItem(ToolStripItemCollection items, string text, GridlyMediaSmartPreset preset, string tooltip)
    {
        ...
        {
            GridlyMediaSmartPreset.Music => Color.FromArgb(120, 155, 255),
            GridlyMediaSmartPreset.Movie => Color.FromArgb(180, 120, 255),
            GridlyMediaSmartPreset.Photo => Color.FromArgb(90, 190, 150),
            GridlyMediaSmartPreset.Document => Color.FromArgb(220, 170, 80),
            _ => _theme.AccentColor
        });
        item.ToolTipText = tooltip;
    }

    private void AddDashboardWidgetMenuItem(ToolStripItemCollection items, string text, GridlyDashboardWidgetKind kind, GridlyViewMode viewMode)
    {
        ...
        {
            GridlyViewMode.KpiDashboard => Color.FromArgb(92, 200, 128),
            GridlyViewMode.HeatMap => Color.FromArgb(230, 120, 80),
            GridlyViewMode.MiniChart => Color.FromArgb(90, 170, 230),
            _ => _theme.AccentColor
        });
    ...
    private void AddViewModeGroup(ToolStripItemCollection items, string text, string tooltip, params GridlyViewMode[] modes)
    {
        ...
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(group.DropDown, _theme);
        items.Add(group);
    }
    ...
    {
        var modeMenu = new ToolStripMenuItem(GridlyText.Mode);
        foreach (GridlyMode m in Enum.GetValues(typeof(GridlyMode)))
        {
            var local = m;
            AddCheckedMenuItem(modeMenu.DropDownItems, GridlyText.GridlyModeName(local.ToString()), Mode == local, (_,__) => { Mode = local; RefreshView(); CloseActiveMenusSafe(); });
        }
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(modeMenu.DropDown, _theme);
        items.Add(modeMenu);
    }

    private void AddThemeMenu(ToolStripItemCollection items)
    {
        var themeMenu = new ToolStripMenuItem(GridlyText.Theme);
        foreach (GridlyThemePreset p in Enum.GetValues(typeof(GridlyThemePreset)))
        {
            var local = p;
            AddCheckedMenuItem(themeMenu.DropDownItems, GridlyText.ThemePresetName(local.ToString()), ThemePreset == local, (_,__) => { ThemePreset = local; RefreshView(); CloseActiveMenusSafe(); });
        }
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(themeMenu.DropDown, _theme);
        items.Add(themeMenu);
    }
    ...
    private void AddGroupHeaderContextMenuItems(ContextMenuStrip menu, int rowIndex)
    {
        ...
        if (ShowGroupMenuToggleThisGroupItem)
            menu.Items.Add(collapsed ? GridlyText.ExpandThisGroup : GridlyText.CollapseThisGroup, null, (_, __) => ToggleGroup(groupKey));

        if (ShowGroupMenuOnlyThisGroupItem)
            menu.Items.Add(GridlyText.ShowOnlyThisGroup, null, (_, __) => ShowOnlyGroup(groupKey));

        if (ShowGroupMenuToggleThisGroupItem || ShowGroupMenuOnlyThisGroupItem)
            menu.Items.Add(new ToolStripSeparator());

        if (ShowGroupMenuCollapseAllItem)
            menu.Items.Add(GridlyText.CollapseAllGroups, null, (_, __) => CollapseAllGroups()).Enabled = EnableGrouping;

        if (ShowGroupMenuExpandAllItem)
            menu.Items.Add(GridlyText.ExpandAllGroups, null, (_, __) => ExpandAllGroups()).Enabled = EnableGrouping;
        ...
        if (ShowGroupMenuClearGroupingItem)
            menu.Items.Add(GridlyText.ClearGrouping, null, (_, __) => ClearGrouping()).Enabled = EnableGrouping;
    }


    private void AddSortMenu(ToolStripItemCollection items, GridlyColumn? col)
    {
        var root = new ToolStripMenuItem(GridlyText.Sorting);
        if (col != null)
        {
            root.DropDownItems.Add(GridlyText.SortAscending, null, (_,__) => { SortBy(col, false); }).Enabled = col.Sortable;
            root.DropDownItems.Add(GridlyText.SortDescending, null, (_,__) => { SortBy(col, true); }).Enabled = col.Sortable;
            root.DropDownItems.Add(GridlyText.UnsortColumn, null, (_,__) => ClearSort(col)).Enabled = _sortColumn == col;
            root.DropDownItems.Add(new ToolStripSeparator());
        }
        root.DropDownItems.Add(GridlyText.ClearSort, null, (_,__) => ClearSort(null)).Enabled = _sortColumn != null;
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddFreezeMenu(ToolStripItemCollection items, GridlyColumn? col)
    {
        var root = new ToolStripMenuItem(GridlyText.FrozenColumn);
        if (col != null)
            root.DropDownItems.Add(GridlyText.FreezeColumn, null, (_,__) => { FrozenColumnCount = Math.Max(FrozenColumnCount, Columns.VisibleColumns.ToList().IndexOf(col) + 1); RefreshView(); });
        root.DropDownItems.Add(GridlyText.ClearFrozenColumns, null, (_,__) => { FrozenColumnCount = 0; RefreshView(); }).Enabled = FrozenColumnCount > 0;
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddAutoSizeMenu(ToolStripItemCollection items, GridlyColumn? col)
    {
        var root = new ToolStripMenuItem(GridlyText.ColumnWidth);
        if (col != null)
            root.DropDownItems.Add(GridlyText.AutoSizeColumn, null, (_,__) => AutoResizeColumn(col));
        root.DropDownItems.Add(GridlyText.AutoSizeAllColumns, null, (_,__) => AutoResizeAllColumnsToContent());
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }
    ...
    {
        var root = new ToolStripMenuItem(GridlyText.Layout);
        root.DropDownItems.Add(GridlyText.SaveColumnLayout, null, (_,__) => SaveColumnLayout());
        root.DropDownItems.Add(GridlyText.LoadColumnLayout, null, (_,__) => LoadColumnLayout());
        root.DropDownItems.Add(GridlyText.SaveDefaultLayoutProfile, null, (_,__) => SaveColumnLayoutProfile("Default"));
        var profileMenu = new ToolStripMenuItem(GridlyText.LayoutProfiles);
        ...
        root.DropDownItems.Add(GridlyText.ResetColumnLayout, null, (_,__) => ResetColumnLayout());
        root.DropDownItems.Add(new ToolStripSeparator());
        root.DropDownItems.Add(GridlyText.SaveUserLayout, null, (_,__) => SaveUserLayout());
        root.DropDownItems.Add(GridlyText.LoadUserLayout, null, (_,__) => LoadUserLayout());
        root.DropDownItems.Add(GridlyText.ResetUserLayout, null, (_,__) => ResetUserLayout());
        root.DropDownItems.Add(GridlyText.ExportUserLayout, null, (_,__) => ExportUserLayoutWithDialog());
        root.DropDownItems.Add(GridlyText.ImportUserLayout, null, (_,__) => ImportUserLayoutWithDialog());
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddGroupingMenu(ToolStripItemCollection items, GridlyColumn? col)
    {
        var root = new ToolStripMenuItem(GridlyText.Grouping);
        if (col != null)
        {
            root.DropDownItems.Add(GridlyText.GroupByColumn(col.Header), null, (_,__) => SetGroupBy(col.AspectName)).Enabled = col.AllowGroup;
            root.DropDownItems.Add(new ToolStripSeparator());
        }
        root.DropDownItems.Add(GridlyText.CollapseAllGroups, null, (_,__) => CollapseAllGroups()).Enabled = EnableGrouping;
        root.DropDownItems.Add(GridlyText.ExpandAllGroups, null, (_,__) => ExpandAllGroups()).Enabled = EnableGrouping;
        root.DropDownItems.Add(GridlyText.ClearGrouping, null, (_,__) => ClearGrouping()).Enabled = EnableGrouping;
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }
    ...
    {
        var filterStyleMenu = new ToolStripMenuItem(GridlyText.FilterStyle);
        filterStyleMenu.DropDown.Renderer = new global::Gridly.Theming.SmartMenuRenderer(_theme);
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, GridlyText.PopupMenu, FilterMenuMode == GridlyFilterMenuMode.PopupMenu, (_,__) => { FilterMenuMode = GridlyFilterMenuMode.PopupMenu; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, GridlyText.ModalWindow, FilterMenuMode == GridlyFilterMenuMode.ModalWindow, (_,__) => { FilterMenuMode = GridlyFilterMenuMode.ModalWindow; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, GridlyText.Both, FilterMenuMode == GridlyFilterMenuMode.Both, (_,__) => { FilterMenuMode = GridlyFilterMenuMode.Both; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        global::Gridly.Theming.SmartMenuRenderer.ApplyTo(filterStyleMenu.DropDown, _theme);
    ...
    public void AutoResizeColumn(GridlyColumn col) => AutoResizeColumnToContent(col);
...
internal sealed class GridlyMenuDismissMessageFilter : IMessageFilter
{
    ...
    public GridlyMenuDismissMessageFilter(Func<bool> isInsideAllowedArea, Action closeAll)
    {
...
internal readonly struct GridlyDisplayRow
{
    private GridlyDisplayRow(bool isGroup, int realIndex, string groupKey, string caption, int count, bool collapsed)
    {
    ...
    public static GridlyDisplayRow Data(int realIndex) => new(false, realIndex, string.Empty, string.Empty, 0, false);
    public static GridlyDisplayRow Group(string caption, int count, bool collapsed) => new(true, -1, caption, $"{caption} ({count})", count, collapsed);
}

public class GridlyCellClickEventArgs : EventArgs
{
    public GridlyCellClickEventArgs(int rowIndex, object rowObject, GridlyColumn column) { RowIndex = rowIndex; RowObject = rowObject; Column = column; }
    public int RowIndex { get; }
    public object RowObject { get; }
    public GridlyColumn Column { get; }

    /// <summary>True ise tıklanan hücre Gridly checkbox/toggle veya GLV compatibility checkbox hücresidir.</summary>
    public bool IsCheckBoxColumn => Column.Kind == GridlyColumnKind.CheckBox || Column.Kind == GridlyColumnKind.ToggleSwitch || Column.CellCheckBox;
}
public sealed class GridlyCellEditEventArgs : GridlyCellClickEventArgs
{
    public GridlyCellEditEventArgs(int rowIndex, object rowObject, GridlyColumn column, object? newValue) : base(rowIndex,rowObject,column) => NewValue = newValue;
