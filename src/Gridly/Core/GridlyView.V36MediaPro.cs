...
public enum GridlyMediaMissingCoverBehavior
{
    Placeholder,
    Initials,
    Hidden,
...
public partial class GridlyView
{
    ...
    [Category("Gridly - V36 Media Pro")]
    [DefaultValue(GridlyMediaMissingCoverBehavior.Placeholder)]
    [Description("Kapak bulunamadığında placeholder/initials/gizle/rozet-only davranışı.")]
    public GridlyMediaMissingCoverBehavior MissingCoverBehavior { get; set; } = GridlyMediaMissingCoverBehavior.Placeholder;
    ...
    public void ApplyV36MediaProPack()
    {
        ...
        if (MediaPlaceholderImage == null)
            MediaPlaceholderImage = CreateMediaPlaceholderBitmap("GRIDLY", _theme.AccentColor);
    ...
    public Image? ResolveMediaImagePro(object row)
    {
        ...
        if (string.IsNullOrWhiteSpace(path))
            return MissingCoverBehavior == GridlyMediaMissingCoverBehavior.Hidden ? null : MediaPlaceholderImage;
    ...
    public Bitmap CreateMediaPlaceholderBitmap(string label, Color? baseColor = null)
    {
        ...
        using var bg = new System.Drawing.Drawing2D.LinearGradientBrush(new Rectangle(0, 0, bmp.Width, bmp.Height), Gridly.Theming.GridlyThemeAccessibility.Blend(color, Color.White, 0.15d), Gridly.Theming.GridlyThemeAccessibility.Blend(color, Color.Black, 0.45d), 55f);
        ...
        TextRenderer.DrawText(g, "Gridly Media Pro", SystemFonts.CaptionFont, new Rectangle(0, 426, bmp.Width, 30), Color.Gainsboro, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
        return bmp;
    }
