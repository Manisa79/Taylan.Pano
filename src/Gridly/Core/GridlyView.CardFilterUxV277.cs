...
public enum GridlyCardFilterUxPlacement
{
    TopBar,
    FloatingButton,
...
public partial class GridlyView
{
    ...
    [Category("Gridly - Card Filter UX v27.7")]
    [DefaultValue(true)]
    ...
    [Category("Gridly - Card Filter UX v27.7")]
    [DefaultValue(GridlyCardFilterUxPlacement.TopBar)]
    public GridlyCardFilterUxPlacement CardFilterUxPlacement { get; set; } = GridlyCardFilterUxPlacement.TopBar;
    ...
    private void InitializeCardViewFilterUx()
    {
        ...
        _cardFilterColumnBox = new ComboBox
        {
            ...
            Anchor = AnchorStyles.Left | AnchorStyles.Top,
            DisplayMember = nameof(GridlyColumn.Header)
        };
    ...
    private void UpdateCardViewFilterUxLayout()
    {
        ...
        bool showBar = allowedByMode && ShowQuickFilterBar && CardFilterUxPlacement != GridlyCardFilterUxPlacement.FloatingButton;
        bool showFloating = allowedByMode && ShowFloatingFilterButton && !MoveFilterButtonToTopBar && CardFilterUxPlacement != GridlyCardFilterUxPlacement.TopBar;
    ...
    {
        if (_cardFilterBar == null) return;
        var theme = global::Gridly.Theming.GridlyThemeAccessibility.Normalize(_theme);
        Color surface = theme.PanelBackColor;
        Color barBack = global::Gridly.Theming.GridlyThemeAccessibility.EnsureSurfaceContrast(Blend(surface, theme.BackColor, theme.IsDark ? 0.10 : 0.03), theme.BackColor, theme.IsDark);
        Color fore = global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, barBack, 4.5d);
        Color controlBack = global::Gridly.Theming.GridlyThemeAccessibility.EnsureSurfaceContrast(theme.TextBackColor, barBack, theme.IsDark);
        Color controlFore = global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, controlBack, 4.5d);
        Color buttonBack = global::Gridly.Theming.GridlyThemeAccessibility.ButtonBack(theme, primary: false);
        Color buttonFore = global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, buttonBack, 4.5d);
        Color buttonBorder = global::Gridly.Theming.GridlyThemeAccessibility.ButtonBorder(theme, primary: false);
        ...
        if (_cardFilterFloatingButton != null)
        {
            ...
            _cardFilterFloatingButton.ForeColor = global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(Color.White, theme.AccentColor, 4.5d);
            _cardFilterFloatingButton.FlatStyle = FlatStyle.Flat;
            _cardFilterFloatingButton.FlatAppearance.BorderColor = global::Gridly.Theming.GridlyThemeAccessibility.ButtonBorder(theme, primary: true);
        }
    }
    ...
    private void SyncCardFilterColumnList()
    {
        ...
        var current = _cardFilterColumnBox.SelectedItem as GridlyColumn;
        var visibleFilterable = Columns.VisibleColumns.Where(c => c.Filterable).ToList();
        if (_cardFilterColumnBox.Items.Count == visibleFilterable.Count && visibleFilterable.SequenceEqual(_cardFilterColumnBox.Items.Cast<GridlyColumn>()))
            return;
    ...
    private void BuildActiveFilterChips()
    {
        ...
        foreach (var f in _filters.Filters.Where(x => x.Enabled).ToList())
        {
            ...
            string value = f.Mode == GridlyFilterMode.ValueList && f.SelectedValues != null
        ...
        if (_cardFilterChipPanel.Controls.Count == 0)
        {
            var empty = new Label
            {
                ...
                ForeColor = global::Gridly.Theming.GridlyThemeAccessibility.EnsureReadableTextColor(
                    global::Gridly.Theming.GridlyThemeAccessibility.MutedText(_cardFilterChipPanel.BackColor == Color.Empty ? _theme.PanelBackColor : _cardFilterChipPanel.BackColor, _theme.ForeColor),
    ...
    {
        if (_cardFilterColumnBox?.SelectedItem is GridlyColumn col)
        {
    ...
    [Category("Gridly - Card Filter UX v27.8")]
    [DefaultValue(true)]
