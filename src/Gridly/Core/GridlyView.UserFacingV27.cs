...
{
    [Category("Gridly - v27 User Features")]
    [DefaultValue(GridlyViewScenario.DataTable)]
    [Description("Hazır Gridly kullanım senaryosu. Runtime menüden veya designer property penceresinden değiştirilebilir.")]
    public GridlyViewScenario ActiveScenario { get; set; } = GridlyViewScenario.DataTable;
    ...
    [Category("Gridly - v27 User Features")]
    [DefaultValue("")]
    [Description("Kullanıcı presetlerinin saklanacağı klasör. Boşsa LocalAppData/Gridly/States kullanılır.")]
    ...
    /// </summary>
    public void ApplyScenario(GridlyViewScenario scenario)
    ...
    /// </summary>
    public void ApplyScenario(GridlyViewScenario scenario, bool updateActiveScenario)
    {
        ...
        if (ScenarioAppliesViewDefaults)
        {
            ...
            GridlyViewScenarioExtensions.ApplyScenario(this, scenario);
        }
    ...
    public string ResolveStateFilePath(string fallbackName = "gridly-state")
    {
    ...
    private string GetUserStatePresetFolder()
    {
        ...
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), appName, "Gridly", "States");
    }
    ...
    private void AddScenarioMenu(ToolStripItemCollection items)
    {
        ...
        foreach (GridlyViewScenario scenario in Enum.GetValues(typeof(GridlyViewScenario)))
        {
            ...
            item.Click += (_, _) => ApplyScenario((GridlyViewScenario)item.Tag!, updateActiveScenario: true);
    ...
    private void SaveStateWithDialog()
    {
        ...
        {
            Filter = "Gridly state (*.json)|*.json|Tüm dosyalar (*.*)|*.*",
            FileName = MakeSafeFileName(string.IsNullOrWhiteSpace(Name) ? "gridly-state" : Name) + ".json",
            AddExtension = true,
            DefaultExt = "json"
        };
    ...
    {
        string? presetName = GridlyPresetNameDialog.ShowDialog(FindForm(), "Preset adı", string.IsNullOrWhiteSpace(Name) ? "GridlyPreset" : Name);
        if (!string.IsNullOrWhiteSpace(presetName))
            SaveStatePreset(presetName);
    }
}

internal sealed class GridlyPresetNameDialog : Form
{
    ...
    private GridlyPresetNameDialog(string caption, string defaultValue)
    {
        Text = caption;
    ...
    {
        using var form = new GridlyPresetNameDialog(caption, defaultValue);
