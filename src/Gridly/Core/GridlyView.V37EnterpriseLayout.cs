...
public partial class GridlyView
{
    ...
    [Category("Gridly - V37 Enterprise Layout")]
    [DefaultValue(false)]
    ...
    [Category("Gridly - V37 Enterprise Layout")]
    [DefaultValue(GridlyV37LayoutScope.User)]
    [Description("Layout profilinin kullanıcı, proje, makine veya senaryo bazında ele alınacağını belirtir.")]
    public GridlyV37LayoutScope LayoutScope { get; set; } = GridlyV37LayoutScope.User;
    ...
    public GridlyLayoutPresetInfo CreateEnterpriseLayoutSnapshot(string name)
    {
        var preset = new GridlyLayoutPresetInfo
        {
        ...
        if (LayoutIncludeColumns)
        {
            ...
            {
                preset.Columns.Add(new GridlyColumnLayoutInfo
                {
                    Name = column.Name,
    ...
    public bool LoadEnterpriseLayoutFromJson(string json)
    {
        ...
        GridlyLayoutPresetInfo? preset;
        try
        {
            preset = JsonSerializer.Deserialize<GridlyLayoutPresetInfo>(json);
        }
        catch
        {
            return false;
        ...
        try
        {
            if (LayoutIncludeViewMode && Enum.TryParse<GridlyViewMode>(preset.ViewMode, out var mode))
                SetViewMode(mode);
