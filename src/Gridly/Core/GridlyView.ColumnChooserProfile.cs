...
public enum GridlyColumnChooserMenuMode
{
    WindowOnly,
    PopupOnly,
    Both
...
public enum GridlyColumnChooserPopupPlacement
{
    SubMenu,
    Inline
}
...
public partial class GridlyView
{
    ...
    [Category("Gridly - Column Chooser")]
    [DefaultValue(GridlyColumnChooserMenuMode.Both)]
    ...
    public GridlyColumnChooserMenuMode ColumnChooserMenuMode { get; set; } = GridlyColumnChooserMenuMode.Both;
    ...
    [Category("Gridly - Column Chooser")]
    [DefaultValue(GridlyColumnChooserPopupPlacement.SubMenu)]
    [Description("Hızlı kolon görünürlük seçimleri ana menüde direkt mi yoksa Kolonlar alt menüsü altında mı gösterilsin.")]
    public GridlyColumnChooserPopupPlacement ColumnChooserPopupPlacement { get; set; } = GridlyColumnChooserPopupPlacement.SubMenu;
    ...
    private void AddColumnChooserMenu(ToolStripItemCollection items)
    {
        ...
        bool showPopup = ShowColumnChooserInHeaderMenu && ColumnChooserMenuMode != GridlyColumnChooserMenuMode.WindowOnly;
        bool showWindow = ShowColumnChooserWindowInHeaderMenu && ColumnChooserMenuMode != GridlyColumnChooserMenuMode.PopupOnly;

        if (ColumnChooserPopupPlacement == GridlyColumnChooserPopupPlacement.Inline)
        {
            if (showPopup)
            {
            ...
            if (showWindow)
            {
                ...
                items.Add(string.IsNullOrWhiteSpace(ColumnChooserWindowText) ? GridlyText.ColumnChooserWindow : ColumnChooserWindowText.Trim(), null, (_, __) => ShowColumnChooserDeferred());
            }
        ...
        var root = new ToolStripMenuItem(string.IsNullOrWhiteSpace(ColumnChooserPopupText) ? GridlyText.Columns : ColumnChooserPopupText.Trim());
        ...
        if (showWindow)
        {
            ...
            root.DropDownItems.Add(string.IsNullOrWhiteSpace(ColumnChooserWindowText) ? GridlyText.ColumnChooserWindow : ColumnChooserWindowText.Trim(), null, (_, __) => ShowColumnChooserDeferred());
        }
    ...
    {
        items.Add(GridlyText.ShowAll, null, (_, __) => SetAllColumnsVisible(true));
        items.Add(GridlyText.HideAll, null, (_, __) => SetAllColumnsVisible(false));
        items.Add(GridlyText.DefaultColumnLayout, null, (_, __) => ResetColumnLayout());
        ...
        foreach (var col in Columns)
        {
            ...
            {
                if (item.Tag is not GridlyColumn c) return;
        ...
        items.Add(new ToolStripSeparator());
        items.Add(GridlyText.FitVisibleColumns, null, (_, __) => AutoResizeAllColumnsToContent());
    }
