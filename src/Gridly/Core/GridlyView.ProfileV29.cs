...
{
    [Category("Gridly - Profiles")]
    [DefaultValue(true)]
    [Description("v29 profil sistemi. Tek kaynak GridlyLayoutProfile kullanılır; eski kolon profil JSON dosyaları otomatik migrate edilebilir.")]
    ...
    public string GetLayoutProfileFilePath(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GridlyView", "Profiles", key, scope);
        Directory.CreateDirectory(dir);
        return Path.Combine(dir, safeProfile + ".gridlyprofile");
    }
    ...
    public void SaveLayoutProfile(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        GridlyLayoutProfile profile = CaptureLayoutProfile(profileName);
    ...
    public bool LoadLayoutProfile(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        if (!File.Exists(file))
        {
            ...
            GridlyLayoutProfile migrated = GridlyProfileMigrator.FromLegacyColumnProfileJson(File.ReadAllText(legacy), profileName);
            ApplyLayoutProfile(migrated);
            return true;
        }

        GridlyLayoutProfile profile = GridlyLayoutProfile.FromJson(File.ReadAllText(file));
    ...
    public IReadOnlyList<string> GetLayoutProfileNames(string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        return Directory.GetFiles(dir, "*.gridlyprofile")
    ...
    public GridlyLayoutProfile ImportLayoutProfile(string filePath, bool apply = true)
    {
        if (string.IsNullOrWhiteSpace(filePath)) throw new ArgumentException("File path cannot be empty.", nameof(filePath));
        GridlyLayoutProfile profile = GridlyLayoutProfile.FromJson(File.ReadAllText(filePath));
    ...
    public int MigrateLegacyProfiles(bool deleteLegacyAfterMigration = false)
    {
        ...
        foreach (string name in GetColumnLayoutProfileNames())
        {
            ...
            try
            {
                GridlyLayoutProfile profile = GridlyProfileMigrator.FromLegacyColumnProfileJson(File.ReadAllText(legacy), name);
