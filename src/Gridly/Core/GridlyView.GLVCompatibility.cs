...
/// <summary>
/// Gridly migration presets kept for source compatibility with older MasterData/Gridly screens.
/// New code can still use Gridly.Presets.GridlyPresets directly.
/// </summary>
public enum GridlyPreset
{
    Default,
    ClassicGridly,
    LogView,
...
{
    public FormatCellEventArgs(int rowIndex, int columnIndex, object rowObject, GridlyColumn column, object? cellValue, ListViewItem.ListViewSubItem subItem)
    {
        RowIndex = rowIndex;
...
{
    /// <summary>
    /// Gridly compatible row formatting event. Set e.Item.BackColor / ForeColor / Font.
    /// Gridly applies BackColor and ForeColor while painting the visible row.
    /// </summary>
    ...
    [Category("Gridly - Compatibility")]
    [DefaultValue(true)]
    [Description("When true Gridly shows its built-in body context menu if BodyContextMenuStrip is null.")]
    public bool UseBuiltInBodyContextMenu { get; set; } = true;

    [Category("Gridly - Compatibility")]
    [DefaultValue(null)]
    [Description("Gridly style body context menu. Header menus remain Gridly's filter/sort menu.")]
    ...
    [Category("Gridly - Compatibility")]
    [DefaultValue(true)]
    [Description("True ise Text kolonlarında da ImageGetter/StateImageGetter sonucu Gridly gibi metnin solunda çizilir.")]
    ...
    {
        get => false;
        set { /* Gridly zaten owner-draw çalışır; Gridly migration için no-op. */ }
    }
    ...
    [Browsable(false)]
    public GridlyColumnCollection AllColumns => Columns;

    public void ApplyPreset(GridlyPreset preset)
    {
        switch (preset)
        {
            case GridlyPreset.AoiFailList:
                GridlyPresets.ApplyAoiFailList(this);
                break;
            case GridlyPreset.DatabaseEditor:
                GridlyPresets.ApplyDatabaseEditorDefaults(this);
                break;
            case GridlyPreset.LogView:
                ApplyClassicGridlyPreset();
                RowHeight = Math.Max(RowHeight, 24);
                AlternateRows = false;
                ShowGridLines = false;
                EmptyListMessage = "Log kaydı yok";
                break;
            case GridlyPreset.LineOverview:
                ApplyClassicGridlyPreset();
                RowHeight = Math.Max(RowHeight, 34);
                RowColorPreset = Gridly.Theming.GridlyRowColorPreset.StatusPills;
                break;
            case GridlyPreset.CompareList:
                ApplyClassicGridlyPreset();
                ShowGridLines = true;
                AlternateRows = true;
                ...
                break;
            case GridlyPreset.ClassicGridly:
            case GridlyPreset.Default:
            default:
                ApplyClassicGridlyPreset();
                break;
        }
    ...
    {
        if (Enum.TryParse<GridlyPreset>(presetName ?? string.Empty, true, out var preset))
            ApplyPreset(preset);
        else
            ApplyPreset(GridlyPreset.ClassicGridly);
    }

    private void ApplyClassicGridlyPreset()
    {
        ShowHeader = true;
        ShowGridLines = true;
        AlternateRows = true;
    ...
    private void ApplyFormatCellCompatibility(int viewIndex, object row, GridlyColumn col, object? value, ref Color cellBack, ref Color cellFore)
    {
        if (!UseCellFormatEvents) return;
        ...
        {
            // Gridly migration formatting code must never break painting.
        }
    }

    /// <summary>
    /// Gridly compatible helper. Scrolls the row that owns the given model into view.
    /// Usage: grid.EnsureModelVisible(newLog);
    /// </summary>
    public void EnsureModelVisible(object? model) => EnsureObjectVisible(model);

    /// <summary>Gridly compatible alias for EnsureModelVisible.</summary>
    ...
    /// <summary>Gridly style selection check by model object.</summary>
    ...
    // GetItemCount() is implemented in GridlyView.GLVCompat.cs and returns ViewCount.
    ...
    public void AutoResizeColumn(GridlyColumn col, ColumnHeaderAutoResizeStyle headerAutoResize)
    {
    ...
    private void AutoResizeColumnToHeader(GridlyColumn col, bool refresh = true)
    {
    ...
    private void AutoResizeColumnToContentOnly(GridlyColumn col, bool refresh = true)
    {
        if (col == null || !Columns.Contains(col)) return;
    ...
    /// <summary>Gridly compatible alias: replaces current objects and rebuilds visible rows.</summary>
    ...
    // SelectObject, SelectObjects and DeselectObject are implemented in GridlyView.cs.
    ...
    private bool TryShowGLVBodyContextMenu(Point location)
    {
        ...
        {
            ApplyGridlyThemeToMenu(BodyContextMenuStrip);
