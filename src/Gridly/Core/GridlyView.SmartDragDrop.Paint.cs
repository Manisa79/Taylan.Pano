...
public partial class GridlyView
{
    ...
    {
        if (!ShowDropIndicator || _smartDropTargetViewIndex < 0 || _smartDropPosition == GridlyDropPosition.None) return;
        int header = ShowHeader ? HeaderHeight : 0;
        int y = header + (_smartDropTargetViewIndex - _scrollY) * RowHeight;
        if (_smartDropPosition == GridlyDropPosition.After) y += RowHeight;
