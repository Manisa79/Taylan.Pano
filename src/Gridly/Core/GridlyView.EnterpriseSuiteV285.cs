...
{
    private GridlyPluginCollection? _plugins;
    private readonly List<GridlyConditionalRule> _conditionalRules = new();
    private readonly System.Windows.Forms.Timer _liveRefreshTimer = new();
    private GridlyLiveUpdateMode _liveUpdateMode;
    ...
    public event EventHandler<GridlyCardActionClickEventArgs>? CardActionClick;

    [Category("Gridly - Enterprise")]
    [DefaultValue(false)]
    ...
    [Category("Gridly - Live")]
    [DefaultValue(GridlyLiveUpdateMode.Off)]
    public GridlyLiveUpdateMode LiveUpdateMode
    {
        get => _liveUpdateMode;
        set
        {
    ...
    [Category("Gridly - Live")]
    [DefaultValue(GridlyChangedRowAnimation.Flash)]
    public GridlyChangedRowAnimation ChangedRowAnimation { get; set; } = GridlyChangedRowAnimation.Flash;

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public IList<GridlyConditionalRule> ConditionalRules => _conditionalRules;

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public GridlyPluginCollection Plugins => _plugins ??= new GridlyPluginCollection(this);

    private void ConfigureLiveRefreshTimer()
    {
        _liveRefreshTimer.Stop();
        _liveRefreshTimer.Tick -= LiveRefreshTimerOnTick;
        if (_liveUpdateMode != GridlyLiveUpdateMode.TimerRefresh || DesignMode || IsDisposed) return;
    ...
    public void AddConditionalRule(GridlyConditionalRule rule)
    {
    ...
    public void ApplyLayoutProfile(GridlyLayoutProfile profile)
    {
    ...
    public GridlyLayoutProfile CaptureLayoutProfile(string name)
    {
        return new GridlyLayoutProfile
        {
            Name = name,
            DisplayName = name,
    ...
    public IReadOnlyList<GridlySmartSearchToken> ParseSmartSearch(string query)
    {
        List<GridlySmartSearchToken> result = new();
        ...
        foreach (string raw in query.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            result.Add(new GridlySmartSearchToken { ColumnKey = columnKey, Text = token, Exclude = exclude });
        }
        return result;
    }
    ...
    {
        IReadOnlyList<GridlySmartSearchToken> tokens = ParseSmartSearch(query);
        if (tokens.Count == 0) return true;

        foreach (GridlySmartSearchToken token in tokens)
        {
    ...
    private bool SmartSearchTokenMatches(object row, GridlySmartSearchToken token)
    {
        IEnumerable<GridlyColumn> searchColumns = Columns.VisibleColumns.Where(c => c.Searchable);
        ...
        foreach (GridlyColumn column in searchColumns)
        {
    ...
    {
        if (row == null || ChangedRowAnimation == GridlyChangedRowAnimation.None) return;
        int index = IndexOfObject(row);
        if (index < 0) return;
        HighlightRow(index, color ?? _theme.AccentColor, 1200, reason);
    }

    public object? CalculateAggregate(GridlyColumn column)
    {
        if (column == null) throw new ArgumentNullException(nameof(column));
        List<object> rows = GetObjects().ToList();
        if (column.Aggregate == GridlyAggregateMode.Custom)
        ...
        {
            GridlyAggregateMode.Count => rows.Count,
            GridlyAggregateMode.Sum => nums.Sum(),
            GridlyAggregateMode.Average => nums.Count == 0 ? 0 : nums.Average(),
            GridlyAggregateMode.Min => nums.Count == 0 ? 0 : nums.Min(),
            GridlyAggregateMode.Max => nums.Count == 0 ? 0 : nums.Max(),
            _ => null
        };
    }
}
