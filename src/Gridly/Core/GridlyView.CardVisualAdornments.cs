...
public partial class GridlyView
{
    ...
    private GridlyCardAccentMode _cardDefaultAccentMode = GridlyCardAccentMode.Auto;

    [Category("Gridly - Card Visuals")]
    [DefaultValue(true)]
    ...
    [Category("Gridly - Card Visuals")]
    [DefaultValue(GridlyCardAccentMode.Auto)]
    ...
    public GridlyCardAccentMode CardDefaultAccentMode
    {
    ...
    public Func<object, GridlyCardVisualInfo?>? CardVisualInfoGetter { get; set; }

    private GridlyCardVisualInfo? ResolveCardVisualInfo(object row, Color? statusColor)
    {
        if (!CardVisualAdornments || row == null) return null;

        GridlyCardVisualInfo? info = null;
        try
        {
    ...
    private void AddAutoBadgesFromColumns(object row, GridlyCardVisualInfo info)
    {
        if (CardBadgeMaxCount <= 0) return;
        int available = Math.Max(0, CardBadgeMaxCount - info.Badges.Count);
        if (available <= 0) return;

        foreach (GridlyColumn col in Columns.VisibleColumns.Where(c => c.Kind == GridlyColumnKind.Badge).Take(available))
        {
            object? value = null;
    ...
    private bool ShouldDrawCardVisualAccent(GridlyCardVisualInfo? info, Color? statusColor)
    {
    ...
    private Color? ResolveCardDotColor(GridlyCardVisualInfo? info, Color? statusColor)
    {
    ...
    private void DrawCardAccentAdorner(Graphics g, Rectangle r, Color cardBack, bool isSelected, GridlyCardVisualInfo? info, Color? statusColor)
    {
        ...
        GridlyCardAccentMode mode = info != null ? info.AccentMode : GridlyCardAccentMode.Auto;
        if (mode == GridlyCardAccentMode.Auto) mode = CardDefaultAccentMode;
        if (mode == GridlyCardAccentMode.Auto)
        {
            mode = ViewMode switch
            {
                GridlyViewMode.RowCard or GridlyViewMode.Timeline => GridlyCardAccentMode.LeftBar,
                GridlyViewMode.Poster or GridlyViewMode.IconGrid => GridlyCardAccentMode.Outline,
                _ => GridlyCardAccentMode.TopBar
            };
        }
        if (mode == GridlyCardAccentMode.None) return;

        Color paint = isSelected ? color : Blend(color, cardBack, _theme.IsDark ? 0.25 : 0.10);
        switch (mode)
        {
            case GridlyCardAccentMode.LeftBar:
            ...
            case GridlyCardAccentMode.Outline:
                ...
                break;
            case GridlyCardAccentMode.Glow:
                ...
                break;
            case GridlyCardAccentMode.TopBar:
            default:
                using (var b = new SolidBrush(paint))
    ...
    private void DrawCardBadges(Graphics g, Rectangle cardBounds, GridlyCardVisualInfo? info, Color cardBack)
    {
        ...
        int gap = 5;
        int margin = 8;
        Dictionary<GridlyCardBadgePlacement, int> offsets = new();

        foreach (GridlyCardBadge badge in info.Badges.Take(CardBadgeMaxCount))
        {
            GridlyCardBadgePlacement placement = badge.Placement;
    ...
    private static Rectangle GetBadgeRect(Rectangle cardBounds, GridlyCardBadgePlacement placement, int size, int margin, int offset)
    {
        return placement switch
        {
            GridlyCardBadgePlacement.TopLeft => new Rectangle(cardBounds.Left + margin + offset, cardBounds.Top + margin, size, size),
            GridlyCardBadgePlacement.BottomLeft => new Rectangle(cardBounds.Left + margin + offset, cardBounds.Bottom - margin - size, size, size),
            GridlyCardBadgePlacement.BottomRight => new Rectangle(cardBounds.Right - margin - size - offset, cardBounds.Bottom - margin - size, size, size),
            _ => new Rectangle(cardBounds.Right - margin - size - offset, cardBounds.Top + margin, size, size)
        };
    }

    private void DrawCardBadge(Graphics g, Rectangle r, GridlyCardBadge badge, Color cardBack)
    {
        ...
        if (badge.Glyph != GridlyCardGlyph.None)
        {
    ...
    private void DrawCardGlyph(Graphics g, Rectangle r, GridlyCardGlyph glyph, Color fore)
    {
    ...
    private void DrawCardActions(Graphics g, Rectangle cardBounds, GridlyCardVisualInfo? info, Color cardBack)
    {
        ...
        int gap = 5;
        int margin = 8;
        Dictionary<GridlyCardActionPlacement, int> offsets = new();

        foreach (GridlyCardAction action in info.Actions.Where(a => a.Visible).Take(6))
        {
    ...
    private static Rectangle GetActionRect(Rectangle cardBounds, GridlyCardActionPlacement placement, int size, int margin, int offset)
    {
        return placement switch
        {
            GridlyCardActionPlacement.TopLeft => new Rectangle(cardBounds.Left + margin + offset, cardBounds.Top + margin, size, size),
            GridlyCardActionPlacement.BottomLeft => new Rectangle(cardBounds.Left + margin + offset, cardBounds.Bottom - margin - size, size, size),
            GridlyCardActionPlacement.BottomRight => new Rectangle(cardBounds.Right - margin - size - offset, cardBounds.Bottom - margin - size, size, size),
            _ => new Rectangle(cardBounds.Right - margin - size - offset, cardBounds.Top + margin, size, size)
        };
    }

    private void DrawCardAction(Graphics g, Rectangle r, GridlyCardAction action, Color cardBack)
    {
        ...
        if (action.Glyph != GridlyCardGlyph.None)
        {
    ...
    private bool TryProcessCardActionClick(Point location, int rowIndex, object row)
    {
        ...
        GridlyCardVisualInfo? info = ResolveCardVisualInfo(row, cardStatusColor);
        if (info == null || info.Actions.Count == 0) return false;

        GridlyColumn? first = Columns.VisibleColumns.FirstOrDefault();
        ...
        foreach (GridlyCardAction action in info.Actions.Where(a => a.Visible).Take(6))
        {
            ...
            CardActionClick?.Invoke(this, new GridlyCardActionClickEventArgs(row, action));
            return true;
        }
