...
public partial class GridlyView
{
    ...
    [Category("Gridly - User Layout"), DefaultValue(true)]
    ...
    [Category("Gridly - User Layout"), DefaultValue(650)]
    ...
    [Category("Gridly - User Layout"), DefaultValue(null)]
    ...
    [Category("Gridly - User Layout"), DefaultValue(null)]
    [Description("Kullanıcı düzenlerinin saklanacağı klasör. Boşsa %AppData%\\Gridly\\UserLayouts kullanılır.")]
    public string? UserLayoutFolder { get; set; }

    private bool _persistColumnFilters;

    [Category("Gridly - User Layout")]
    [DefaultValue(false)]
    ...
    public string GetUserLayoutFilePath(string? profileName = null)
    {
        var folder = string.IsNullOrWhiteSpace(UserLayoutFolder)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Gridly", "UserLayouts")
            : UserLayoutFolder!;
        ...
        return Path.Combine(folder, SanitizeFileName(key) + ".gridly-layout.json");
    }
    ...
    public void ExportUserLayoutWithDialog()
    {
        ...
        {
            Title = "Gridly kullanıcı düzenini dışa aktar",
            Filter = "Gridly Layout (*.gridly-layout.json)|*.gridly-layout.json|JSON (*.json)|*.json|Tüm dosyalar (*.*)|*.*",
            FileName = SanitizeFileName((string.IsNullOrWhiteSpace(UserLayoutKey) ? GetDefaultUserLayoutKey() : UserLayoutKey!) + ".gridly-layout.json")
        };
    ...
    public void ImportUserLayoutWithDialog()
    {
        ...
        {
            Title = "Gridly kullanıcı düzenini içe aktar",
            Filter = "Gridly Layout (*.gridly-layout.json;*.json)|*.gridly-layout.json;*.json|Tüm dosyalar (*.*)|*.*"
        };
    ...
    {
        value = string.IsNullOrWhiteSpace(value) ? "GridlyLayout" : value.Trim();
