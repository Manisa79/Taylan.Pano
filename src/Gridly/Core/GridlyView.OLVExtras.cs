...
/// These helpers intentionally depend only on Gridly types; there is no BrightIdeasSoftware reference.
/// </summary>
public enum GridlySelectionRestoreMode
{
    None = 0,
    ByReference = 1,
    ByAspectValue = 2
...
public sealed class GridlySelectionSnapshot
{
    internal GridlySelectionSnapshot(List<object> references, List<object?> keys, string keyAspectName, int scrollY)
    {
...
public partial class GridlyView
{
    ...
    [Category("Gridly - Compatibility")]
    [DefaultValue(true)]
    [Description("OLV migration alias. Gridly header zaten tema uyumlu çizilir; false verilirse özel header renkleri korunur.")]
    ...
    public GridlySelectionSnapshot CaptureSelectionSnapshot(string keyAspectName = "")
    {
        ...
        var keys = !string.IsNullOrWhiteSpace(keyAspectName)
            ...
            : new List<object?>();
        return new GridlySelectionSnapshot(selected, keys, keyAspectName ?? string.Empty, _scrollY);
    }

    public void RestoreSelectionSnapshot(GridlySelectionSnapshot? snapshot, bool ensureVisible = true)
    {
