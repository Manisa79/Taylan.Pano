...
{
    [Category("Gridly - State"), DefaultValue("")]
    ...
    [Category("Gridly - State"), DefaultValue(true)]
    ...
    public GridlyState CaptureState(string name = "")
    {
        var state = new GridlyState
        {
    ...
    public void ApplyState(GridlyState state)
    {
        ...
        if (!string.IsNullOrWhiteSpace(state.Scenario) && Enum.TryParse<GridlyViewScenario>(state.Scenario, out var scenario))
            ActiveScenario = scenario;

        if (!string.IsNullOrWhiteSpace(state.ViewMode) && Enum.TryParse<GridlyViewMode>(state.ViewMode, out var viewMode))
            ViewMode = viewMode;
    ...
    public bool LoadState(string path)
    {
        ...
        ApplyState(GridlyState.FromJson(File.ReadAllText(path)));
        return true;
    }
