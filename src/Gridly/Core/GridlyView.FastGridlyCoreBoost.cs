...
{
    private CancellationTokenSource? _fastGridlyAsyncLoadCts;

    /// <summary>
    /// FastGridlyView için önerilen en hızlı ve stabil profil.
    ...
    public void PrebuildSmartFilterIndexes(params string[] aspectNames)
    {
        ...
        IEnumerable<GridlyColumn> targets = aspectNames == null || aspectNames.Length == 0
    ...
    public Task SetObjectsAsyncSafe<T>(Func<CancellationToken, Task<IEnumerable<T>>> loader, bool applyPerformanceProfile = true, CancellationToken token = default)
    {
        ...
        _fastGridlyAsyncLoadCts?.Cancel();
        _fastGridlyAsyncLoadCts?.Dispose();
        _fastGridlyAsyncLoadCts = CancellationTokenSource.CreateLinkedTokenSource(token);
        var linked = _fastGridlyAsyncLoadCts.Token;
    ...
    /// </summary>
    public void RefreshFastGridlyCore(bool prebuildFilterIndexes = true)
    {
