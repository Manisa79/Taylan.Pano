...
public partial class GridlyView
{
    ...
    /// tekrar görünüyordu. Gridly/ListView benzeri davranışta mevcut kolonlar korunur,
    ...
    private void MergeColumnsFromDataTable(DataTable table)
    {
        foreach (DataColumn dc in table.Columns)
        {
            ...
            if (existing != null)
            {
                ...
                existing.Kind = existing.Kind == GridlyColumnKind.Text ? GetKindForDataColumn(dc) : existing.Kind;
                existing.EditorType = existing.EditorType == GridlyCellEditorKind.Auto ? GetEditorForDataColumn(dc) : existing.EditorType;
    ...
    private static GridlyColumn CreateColumnFromDataColumn(DataColumn dc)
    {
        var kind = GetKindForDataColumn(dc);
        var col = new GridlyColumn(dc.Caption == dc.ColumnName ? dc.ColumnName : dc.Caption, dc.ColumnName, GetDefaultWidth(dc.DataType))
        {
            Kind = kind,
        ...
        if (kind == GridlyColumnKind.CheckBox) col.Width = 80;
        return col;
    }

    private static GridlyColumnKind GetKindForDataColumn(DataColumn dc)
    {
        var type = Nullable.GetUnderlyingType(dc.DataType) ?? dc.DataType;
        if (type == typeof(bool)) return GridlyColumnKind.CheckBox;
        if (type == typeof(DateTime)) return GridlyColumnKind.Date;
        if (IsNumericType(type)) return GridlyColumnKind.Numeric;
        return GridlyColumnKind.Text;
    }

    private static GridlyCellEditorKind GetEditorForDataColumn(DataColumn dc)
    {
        var type = Nullable.GetUnderlyingType(dc.DataType) ?? dc.DataType;
        if (type == typeof(bool)) return GridlyCellEditorKind.CheckBox;
        if (type == typeof(DateTime)) return GridlyCellEditorKind.DateTime;
        if (IsNumericType(type)) return GridlyCellEditorKind.Numeric;
        return GridlyCellEditorKind.TextBox;
    }
