...
public enum GridlyColumnPinMode
{
    None = 0,
    Left = 1,
    Right = 2
...
public enum GridlyRowChangeFlashMode
{
    None = 0,
    Cell = 1,
    Row = 2,
...
public sealed class GridlyQuickFilterOptions
{
...
public sealed class GridlyFilterPresetInfo
{
...
public sealed class GridlyRowDetailsTemplate
{
    public string Name { get; set; } = string.Empty;
...
public sealed class GridlyCommandPaletteCommand
{
    public string Id { get; set; } = string.Empty;
    public string Text { get; set; } = string.Empty;
    public string Category { get; set; } = "Gridly";
    public string? ShortcutText { get; set; }
    public Action<GridlyView>? Execute { get; set; }
}

public sealed class GridlyContextMenuRequest
{
    public GridlyContextMenuRequest(GridlyView grid, GridlyColumn? column, object? rowObject, Point clientLocation)
    {
        Grid = grid;
        Column = column;
        RowObject = rowObject;
...
public sealed class GridlyPlatformFeatureOptions
{
...
public enum GridlyCardFilterColumnSelectionMode
{
    AskUser = 0,
    FirstVisible = 1,
    LastUsed = 2,
...
public sealed class GridlyDashboardFilterOptions
{
    public GridlyCardFilterColumnSelectionMode ColumnSelectionMode { get; set; } = GridlyCardFilterColumnSelectionMode.AskUser;
...
{
    private readonly Dictionary<string, GridlyFilterPresetInfo> _v2813FilterPresets = new Dictionary<string, GridlyFilterPresetInfo>(StringComparer.OrdinalIgnoreCase);
    private readonly List<GridlyCommandPaletteCommand> _v2813Commands = new List<GridlyCommandPaletteCommand>();
    private GridlyQuickFilterOptions? _v2813QuickFilterOptions;
    ...
    private GridlyRowDetailsTemplate? _v2813RowDetailsTemplate;
    private readonly Dictionary<object, DateTime> _v2813PinnedRows = new Dictionary<object, DateTime>();

    [Category("Gridly - Platform")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
    public GridlyPlatformFeatureOptions PlatformFeatures { get; } = new GridlyPlatformFeatureOptions();

    [Category("Gridly - Grouping")]
    [DefaultValue(false)]
    ...
    [Category("Gridly - Copy")]
    [DefaultValue(true)]
    public bool EnableCopyPro { get; set; } = true;

    [Category("Gridly - Change Flash")]
    [DefaultValue(GridlyRowChangeFlashMode.Row)]
    public GridlyRowChangeFlashMode ChangeFlashMode { get; set; } = GridlyRowChangeFlashMode.Row;

    [Browsable(false)]
    public IReadOnlyDictionary<string, GridlyFilterPresetInfo> FilterPresets => _v2813FilterPresets;

    [Browsable(false)]
    public IList<GridlyCommandPaletteCommand> CommandPaletteCommands => _v2813Commands;

    [Browsable(false)]
    public Func<GridlyContextMenuRequest, ContextMenuStrip?>? ContextMenuProvider { get; set; }

    [Category("Gridly - Card/Dashboard Filter")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
    public GridlyDashboardFilterOptions DashboardFilterOptions { get; } = new GridlyDashboardFilterOptions();
    ...
    {
        GridlyColumn col = ResolveDashboardFilterColumn(null);
        if (col == null || DashboardFilterOptions.ShowColumnChooserBeforeFilter || DashboardFilterOptions.ColumnSelectionMode == GridlyCardFilterColumnSelectionMode.AskUser)
        {
    ...
    {
        Point screen = anchorControl == null ? Control.MousePosition : anchorControl.PointToScreen(new Point(0, anchorControl.Height));
        GridlyColumn col = ResolveDashboardFilterColumn(typedText);
        if (col == null)
    ...
    private GridlyColumn ResolveDashboardFilterColumn(string typedText)
    {
        List<GridlyColumn> visible = Columns.Where(c => c.Visible && c.AllowFilter).ToList();
        if (visible.Count == 0)
            return null;

        if (DashboardFilterOptions.ColumnSelectionMode == GridlyCardFilterColumnSelectionMode.LastUsed && !string.IsNullOrWhiteSpace(DashboardFilterOptions.LastAspectName))
        {
            GridlyColumn last = visible.FirstOrDefault(c => string.Equals(c.AspectName, DashboardFilterOptions.LastAspectName, StringComparison.OrdinalIgnoreCase));
            if (last != null) return last;
        }

        if (DashboardFilterOptions.ColumnSelectionMode == GridlyCardFilterColumnSelectionMode.AutoBestMatch && !string.IsNullOrWhiteSpace(typedText))
        {
            string q = typedText.Trim();
            GridlyColumn best = null;
            int bestScore = -1;
            foreach (GridlyColumn c in visible)
            {
                int score = 0;
        ...
        if (DashboardFilterOptions.ColumnSelectionMode == GridlyCardFilterColumnSelectionMode.FirstVisible)
            return visible[0];
    ...
    private GridlyColumn ShowDashboardFilterColumnChooser(Point screenAnchor, GridlyColumn suggested)
    {
        List<GridlyColumn> visible = Columns.Where(c => c.Visible && c.AllowFilter).ToList();
        if (visible.Count == 0)
            return null;
    ...
    {
        public DashboardFilterColumnItem(GridlyColumn column) { Column = column; }
        public GridlyColumn Column { get; private set; }
    ...
    public void EnableQuickFilterBar(GridlyQuickFilterOptions? options = null)
    {
        _v2813QuickFilterOptions = options ?? new GridlyQuickFilterOptions { Visible = true };
    ...
    public void SetRowDetailsTemplate(GridlyRowDetailsTemplate template)
    {
    ...
    public void SaveFilterPreset(string name, string? displayName = null)
    {
        ...
        _v2813FilterPresets[name] = new GridlyFilterPresetInfo
        {
            Name = name,
    ...
    private bool TryLoadPlatformFilterPreset(string name)
    {
        ...
        if (!_v2813FilterPresets.TryGetValue(name, out GridlyFilterPresetInfo? preset)) return false;
    ...
    public void AddCommand(string id, string text, Action<GridlyView> execute, string category = "Gridly", string? shortcutText = null)
    {
        ...
        _v2813Commands.Add(new GridlyCommandPaletteCommand
        {
            Id = id,
            Text = text,
            Category = category,
    ...
    public void ShowCommandPalette()
    {
        ...
        void Reload()
        {
            ...
            list.Items.Clear();
            foreach (GridlyCommandPaletteCommand cmd in GetBuiltInAndCustomCommands())
            {
    ...
    public void CopySelection(GridlyCopyFormat format)
    {
        if (!EnableCopyPro) return;
        string text = format switch
        {
            GridlyCopyFormat.Json => BuildCopyJson(),
            GridlyCopyFormat.Markdown => BuildCopyMarkdown(),
            GridlyCopyFormat.Html => BuildCopyHtml(),
    ...
    {
        KeyDown -= GridlyV2813_KeyDown;
        KeyDown += GridlyV2813_KeyDown;
    }

    private void GridlyV2813_KeyDown(object? sender, KeyEventArgs e)
    {
        ...
        {
            CopySelection(GridlyCopyFormat.Json);
            e.Handled = true;
        }
        else if (e.Control && e.Alt && e.KeyCode == Keys.C)
        {
            CopySelection(GridlyCopyFormat.Markdown);
            e.Handled = true;
        }
    }
    ...
    private void BuildQuickFilterBoxes()
    {
        ...
        int x = 6;
        foreach (GridlyColumn col in Columns.Where(c => c.Visible))
        {
    ...
    private void ApplyQuickFilterText()
    {
        ...
        foreach (KeyValuePair<string, TextBox> pair in _v2813QuickFilterBoxes)
        {
            ...
            GridlyColumn col = box.Tag as GridlyColumn;
            ...
            _filters.Set(new GridlyColumnFilter
            {
                ...
                Mode = GridlyFilterMode.Contains,
                Text = text,
                Enabled = true
            });
        }
    ...
    {
        public CommandListItem(GridlyCommandPaletteCommand command, string text)
        {
            Command = command;
            Text = text;
        }
        public GridlyCommandPaletteCommand Command { get; }
        private string Text { get; }
        public override string ToString() => Text;
    }

    private IEnumerable<GridlyCommandPaletteCommand> GetBuiltInAndCustomCommands()
    {
        yield return new GridlyCommandPaletteCommand { Id = "copy", Text = "Seçimi kopyala", Category = "Clipboard", ShortcutText = "Ctrl+C", Execute = g => g.CopySelection(GridlyCopyFormat.PlainText) };
        yield return new GridlyCommandPaletteCommand { Id = "copy-json", Text = "Seçimi JSON kopyala", Category = "Clipboard", ShortcutText = "Ctrl+Shift+C", Execute = g => g.CopySelection(GridlyCopyFormat.Json) };
        yield return new GridlyCommandPaletteCommand { Id = "copy-md", Text = "Seçimi Markdown kopyala", Category = "Clipboard", ShortcutText = "Ctrl+Alt+C", Execute = g => g.CopySelection(GridlyCopyFormat.Markdown) };
        yield return new GridlyCommandPaletteCommand { Id = "clear-filters", Text = "Tüm filtreleri temizle", Category = "Filter", Execute = g => g.ClearFilters() };
        foreach (GridlyCommandPaletteCommand cmd in _v2813Commands) yield return cmd;
    }

    private string BuildCopyPlainText()
    {
        List<GridlyColumn> visible = Columns.Where(c => c.Visible).ToList();
    ...
    private string BuildCopyHtml()
    {
        ...
        foreach (GridlyColumn c in visible) sb.Append("<th>").Append(System.Net.WebUtility.HtmlEncode(c.Header)).Append("</th>");
        sb.AppendLine("</tr></thead>\n<tbody>");
        foreach (object row in EnumerateSelectedOrVisibleRows())
        {
            sb.Append("<tr>");
            foreach (GridlyColumn c in visible) sb.Append("<td>").Append(System.Net.WebUtility.HtmlEncode(Convert.ToString(c.GetValue(row)) ?? string.Empty)).Append("</td>");
            sb.AppendLine("</tr>");
        }
    ...
    {
        // Stable, human-readable snapshot. Internal GridlyFilterSet remains source of truth.
