...
public enum GridlyMediaPlaybackState
{
    None = 0,
    Loading = 1,
    Playing = 2,
...
public sealed class GridlyMediaPlayPauseClickEventArgs : EventArgs
{
    public GridlyMediaPlayPauseClickEventArgs(int rowIndex, object rowObject, GridlyMediaPlaybackState currentState, GridlyMediaKind mediaKind)
    {
        RowIndex = rowIndex;
        RowObject = rowObject;
        CurrentState = currentState;
        MediaKind = mediaKind;
        RequestedState = currentState == GridlyMediaPlaybackState.Playing
    ...
    public GridlyMediaPlaybackState CurrentState { get; }
    public GridlyMediaPlaybackState RequestedState { get; }
...
{
    [Category("Gridly - Media Playback")]
    [DefaultValue(true)]
    ...
    public Func<object, GridlyMediaPlaybackState>? MediaPlaybackStateGetter { get; set; }

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public Func<object, GridlyMediaKind>? MediaKindGetter { get; set; }

    public event EventHandler<GridlyMediaPlayPauseClickEventArgs>? MediaPlayPauseClicked;
    ...
    private GridlyMediaPlaybackState ResolveMediaPlaybackState(object row)
    {
        if (row == null) return GridlyMediaPlaybackState.None;
        ...
        if (value == null) return GridlyMediaPlaybackState.None;
        if (value is GridlyMediaPlaybackState state) return state;
        if (value is bool b) return b ? GridlyMediaPlaybackState.Playing : GridlyMediaPlaybackState.None;
        if (Enum.TryParse<GridlyMediaPlaybackState>(Convert.ToString(value), true, out var parsed)) return parsed;
        return GridlyMediaPlaybackState.None;
    }

    private GridlyMediaKind ResolveMediaKind(object row)
    {
        if (row == null) return GridlyMediaKind.Auto;
        if (MediaKindGetter != null) return MediaKindGetter(row);
        object? value = ResolveMediaNamedValue(row, MediaKindAspectName);
        if (value == null) return GridlyMediaKind.Auto;
        if (value is GridlyMediaKind kind) return kind;
        if (Enum.TryParse<GridlyMediaKind>(Convert.ToString(value), true, out var parsed)) return parsed;
        return GridlyMediaKind.Auto;
    }
    ...
    private void DrawMediaPlaybackChrome(Graphics g, Rectangle imageBounds, object row, Color cardBack, bool isHot, bool isSelected)
    {
        ...
        bool isPlaying = state == GridlyMediaPlaybackState.Playing;
        bool showCenter = ShowMediaOverlayButton && (isHot || isSelected || state != GridlyMediaPlaybackState.None);
    ...
    private void DrawMediaPlaybackButton(Graphics g, Rectangle imageBounds, Color cardBack, GridlyMediaPlaybackState state, GridlyMediaKind kind)
    {
        ...
        {
            GridlyMediaPlaybackState.Playing => "Ⅱ",
            GridlyMediaPlaybackState.Loading => "…",
            GridlyMediaPlaybackState.Error => "!",
            _ => kind == GridlyMediaKind.Video && MediaVideoPreviewMode ? "▶" : (string.IsNullOrWhiteSpace(MediaOverlayButtonText) ? "▶" : MediaOverlayButtonText.Trim())
        };

        Color back = Color.FromArgb(_theme.IsDark ? 220 : 230, _theme.IsDark ? Color.Black : Color.White);
        Color fore = state == GridlyMediaPlaybackState.Error
            ? Color.FromArgb(255, 80, 80)
            : EnsureReadableTextOn(back, _theme.AccentColor);

        using var brush = new SolidBrush(back);
        g.FillEllipse(brush, rect);
        using var pen = new Pen(Color.FromArgb(_theme.IsDark ? 190 : 150, state == GridlyMediaPlaybackState.Error ? Color.Red : _theme.AccentColor), 2f);
    ...
    private bool TryProcessMediaPlaybackClick(Point location, int rowIndex, object row)
    {
        ...
        var args = new GridlyMediaPlayPauseClickEventArgs(rowIndex, row, ResolveMediaPlaybackState(row), ResolveMediaKind(row));
