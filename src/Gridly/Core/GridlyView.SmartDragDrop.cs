...
public enum GridlyDropPosition
{
    None,
    Before,
    After,
...
public enum GridlyExternalDropKind
{
    Unknown,
    Text,
    Files,
...
public sealed class GridlyRowDragStartingEventArgs : CancelEventArgs
{
    public GridlyRowDragStartingEventArgs(IReadOnlyList<object> rows, int sourceViewIndex)
    {
        Rows = rows;
...
public sealed class GridlyRowDropValidatingEventArgs : CancelEventArgs
{
    public GridlyRowDropValidatingEventArgs(IReadOnlyList<object> rows, int targetViewIndex, GridlyDropPosition position)
    {
        Rows = rows;
    ...
    public GridlyDropPosition Position { get; set; }
}

public sealed class GridlyRowDroppedEventArgs : EventArgs
{
    public GridlyRowDroppedEventArgs(IReadOnlyList<object> rows, int targetViewIndex, GridlyDropPosition position)
    {
        Rows = rows;
        TargetViewIndex = targetViewIndex;
        Position = position;
    ...
    public IReadOnlyList<object> Rows { get; }
    public int TargetViewIndex { get; }
    public GridlyDropPosition Position { get; }
}

public sealed class GridlyExternalDropEventArgs : CancelEventArgs
{
    public GridlyExternalDropEventArgs(GridlyExternalDropKind kind, object? payload, int targetViewIndex, GridlyDropPosition position)
    {
        Kind = kind;
        Payload = payload;
        TargetViewIndex = targetViewIndex;
    ...
    public GridlyExternalDropKind Kind { get; }
...
public partial class GridlyView
{
    ...
    private GridlyDropPosition _smartDropPosition = GridlyDropPosition.None;

    [Category("Gridly - Smart Drag Drop")]
    [DefaultValue(false)]
    ...
    public event EventHandler<GridlyRowDragStartingEventArgs>? RowDragStarting;
    public event EventHandler<GridlyRowDropValidatingEventArgs>? RowDropValidating;
    public event EventHandler<GridlyRowDroppedEventArgs>? RowDropped;
    public event EventHandler<GridlyExternalDropEventArgs>? ExternalDropReceived;
    ...
    private void SmartDragDrop_MouseMove(object? sender, MouseEventArgs e)
    {
        ...
        var args = new GridlyRowDragStartingEventArgs(rows, _smartDragStartViewIndex);
    ...
    private void SmartDragDrop_DragLeave(object? sender, EventArgs e)
    {
        ...
        _smartDropPosition = GridlyDropPosition.None;
        Invalidate();
    }
    ...
    private void SmartDragDrop_DragDrop(object? sender, DragEventArgs e)
    {
        ...
        {
            var validate = new GridlyRowDropValidatingEventArgs(payload.Rows, _smartDropTargetViewIndex, _smartDropPosition);
            ...
            RowDropped?.Invoke(this, new GridlyRowDroppedEventArgs(payload.Rows, validate.TargetViewIndex, validate.Position));
        ...
        if (AllowExternalDrop)
        {
            ...
            if (external != null)
            {
                ...
                {
                    var result = MessageBox.Show(FindForm(), "Bu veriler listeye bırakılsın mı?", "Gridly Drag Drop", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
    ...
    private void UpdateSmartDropTarget(Point pt)
    {
        ...
        if (rowIndex >= 0)
        {
            ...
            _smartDropPosition = delta < RowHeight / 2 ? GridlyDropPosition.Before : GridlyDropPosition.After;
        }
        Invalidate();
    }
    ...
    private void ReorderRowsFromSmartDrop(IReadOnlyList<object> rows, int targetViewIndex, GridlyDropPosition position)
    {
        ...
        if (position == GridlyDropPosition.After) insertIndex++;
    ...
    private GridlyExternalDropEventArgs? CreateExternalDropArgs(IDataObject? data, int targetViewIndex, GridlyDropPosition position)
    {
        if (data == null) return null;
        if (data.GetDataPresent(DataFormats.FileDrop))
            return new GridlyExternalDropEventArgs(GridlyExternalDropKind.Files, data.GetData(DataFormats.FileDrop), targetViewIndex, position);
        if (data.GetDataPresent(DataFormats.Text))
            return new GridlyExternalDropEventArgs(GridlyExternalDropKind.Text, data.GetData(DataFormats.Text), targetViewIndex, position);
        return new GridlyExternalDropEventArgs(GridlyExternalDropKind.Unknown, data, targetViewIndex, position);
    }
    ...
    {
        public SmartRowDragPayload(GridlyView source, IReadOnlyList<object> rows)
        {
            Source = source;
            Rows = rows;
        }

        public GridlyView Source { get; }
        public IReadOnlyList<object> Rows { get; }
    }
}
