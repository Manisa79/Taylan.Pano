...
public partial class GridlyView
{
    ...
    private bool ShouldDrawCardStatusTopBar()
    {
        ...
        return ViewMode is GridlyViewMode.Tile
    ...
    private Color? ResolveCardStatusColor(object row)
    {
        ...
        {
            // Consumer delegates should not break Gridly painting.
        }
        ...
        if (statusValue == null)
        {
            GridlyColumn? col = FindCardStatusColumn();
            if (col != null)
            {
    ...
    private GridlyColumn? FindCardStatusColumn()
    {
        if (!string.IsNullOrWhiteSpace(CardStatusAspectName))
        {
            GridlyColumn? exact = Columns.FirstOrDefault(c => string.Equals(c.AspectName, CardStatusAspectName, StringComparison.OrdinalIgnoreCase));
        ...
        {
            GridlyColumn? visibleExact = Columns.VisibleColumns.FirstOrDefault(c => string.Equals(c.AspectName, aspect, StringComparison.OrdinalIgnoreCase));
            if (visibleExact != null) return visibleExact;
        }

        foreach (string aspect in preferredAspects)
        {
            GridlyColumn? anyExact = Columns.FirstOrDefault(c => string.Equals(c.AspectName, aspect, StringComparison.OrdinalIgnoreCase));
