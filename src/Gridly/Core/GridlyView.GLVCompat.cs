...
public sealed class GridlyItemCheckedEventArgs : EventArgs
{
    public GridlyItemCheckedEventArgs(object model, int rowIndex, CheckState checkState)
    {
        Model = model;
        RowObject = model;
        RowIndex = rowIndex;
...
public sealed class GridlyObjectsChangedEventArgs : EventArgs
{
    public GridlyObjectsChangedEventArgs(int objectCount, int visibleCount)
    {
...
public sealed class GridlyModelEventArgs : EventArgs
{
    public GridlyModelEventArgs(object? model, int rowIndex)
    {
        Model = model;
        RowObject = model;
        RowIndex = rowIndex;
...
public partial class GridlyView
{
    ...
    [Category("Gridly - Compatibility")]
    public event EventHandler<GridlyItemCheckedEventArgs>? ItemChecked;

    [Category("Gridly - Compatibility")]
    public event EventHandler<GridlyItemCheckedEventArgs>? ObjectChecked;

    [Category("Gridly - Compatibility")]
    public event EventHandler<GridlyObjectsChangedEventArgs>? ObjectsChanged;

    [Category("Gridly - Compatibility")]
    public event EventHandler<GridlyModelEventArgs>? ObjectAdded;

    [Category("Gridly - Compatibility")]
    public event EventHandler<GridlyModelEventArgs>? ObjectRemoved;
    ...
    [Browsable(false)]
    public GridlyColumn? PrimarySortColumnCompat => _sortColumn;

    [Browsable(false)]
    public GridlyColumn? SortColumn
    {
        get => _sortColumn;
    ...
    private GridlyObjectCollection? _items;

    /// <summary>
    /// GLV/ListView compatibility collection.
    /// Enables common migration code such as gridly.Items.Count, Items.Add, Items.Clear.
    /// For Gridly the collection is a live facade over the current model/tile provider.
    /// </summary>
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public GridlyObjectCollection Items => _items ??= new GridlyObjectCollection(this);
    ...
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public GridlyObjectCollection Item => Items;
    ...
    /// <summary>GLV compatibility: current model objects. Setting this refreshes Gridly.</summary>
    [Browsable(false)]
    ...
    /// <summary>Gridly/GLV compatibility: current visible item count after filtering/sorting/grouping.</summary>
    ...
    /// <summary>
    /// Not: Gridly v25.62 ile params SetObjects(row1, row2, ...) overload'u kaldırıldı.
    ...
    /// Gridly's real scrollbar/layout synchronization lives in RefreshView().
    ...
    public GridlyColumn? GetColumn(string aspectNameOrText)
    {
    ...
    public GLVColumn GetOrCreateGLVColumn(string name, string aspectName, string? text = null, int width = 120)
    {
        ...
        {
            Name = string.IsNullOrWhiteSpace(name) ? GridlyColumnNameHelper.CreateNameFromAspectOrText(aspectName, text) : name
        };
        Columns.Add(column);
        RebuildColumns();
        return column;
    ...
    /// <summary>Live facade that mimics ListView/Gridly Items for migration convenience.</summary>
    public sealed class GridlyObjectCollection : IList<object>
    {
        private readonly GridlyView _owner;
        internal GridlyObjectCollection(GridlyView owner) => _owner = owner;
    ...
    public GridlyColumn? GetColumn(int visibleColumnIndex)
    {
    ...
    public GridlyColumn? GetColumnByName(string name) => string.IsNullOrWhiteSpace(name) ? null : Columns.ByName(name);

    public GridlyColumn? GetColumnByAspect(string aspectName) => string.IsNullOrWhiteSpace(aspectName) ? null : Columns.ByAspectName(aspectName);
    ...
    public GridlyColumn? GetColumnAt(int x) => HitColumn(x);
    ...
    {
        var col = Columns.FirstOrDefault(x => x.Kind == GridlyColumnKind.CheckBox);
    ...
    {
        var args = new GridlyItemCheckedEventArgs(model, rowIndex, state);
    ...
    {
        ObjectsChanged?.Invoke(this, new GridlyObjectsChangedEventArgs(ObjectCount, FilteredCount));
    }

    internal void OnObjectAdded(object model, int rowIndex)
    {
        ObjectAdded?.Invoke(this, new GridlyModelEventArgs(model, rowIndex));
        OnObjectsChanged();
    }

    internal void OnObjectRemoved(object? model, int rowIndex)
    {
        ObjectRemoved?.Invoke(this, new GridlyModelEventArgs(model, rowIndex));
        OnObjectsChanged();
    }
