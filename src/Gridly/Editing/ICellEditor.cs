...
{
    Control CreateEditor(Control owner, Rectangle bounds, GridlyColumn column, object row, object? value);
    object? GetEditedValue(Control editor);
}
public sealed class TextBoxCellEditor : ICellEditor
{
    public Control CreateEditor(Control owner, Rectangle bounds, GridlyColumn column, object row, object? value)
    {
...
public sealed class ComboBoxCellEditor : ICellEditor
{
    ...
    private static void DrawImageComboItem(ComboBox combo, DrawItemEventArgs e, GridlyColumn column)
    {
        e.DrawBackground();
