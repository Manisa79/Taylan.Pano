...
namespace Gridly.Layout;

public sealed class GridlyLayoutState
{
    public List<GridlyColumnLayout> Columns { get; set; } = new();
    ...
    public List<GridlyColumnFilter> ColumnFilters { get; set; } = new();
    ...
    public static GridlyLayoutState Capture(IEnumerable<GridlyColumn> columns, string globalFilter, string? sortAspect, bool sortDescending)
    {
        var state = new GridlyLayoutState { GlobalFilter = globalFilter ?? string.Empty, SortAspectName = sortAspect, SortDescending = sortDescending };
        int order = 0;
        foreach (var c in columns)
            state.Columns.Add(new GridlyColumnLayout { AspectName = c.AspectName, Text = c.Header, Width = c.Width, Visible = c.Visible, DisplayIndex = order++ });
        return state;
    }

    public void Apply(IList<GridlyColumn> columns)
    {
    ...
    public static GridlyLayoutState FromJson(string json) => JsonSerializer.Deserialize<GridlyLayoutState>(json) ?? new GridlyLayoutState();
}

public sealed class GridlyLayoutProfile
{
    ...
    public string GridlyVersion { get; set; } = Gridly.Core.GridlyVersionInfo.Version;
    public DateTime CreatedAt { get; set; } = DateTime.Now;
    public DateTime UpdatedAt { get; set; } = DateTime.Now;
    public GridlyLayoutState State { get; set; } = new();

    public string ToJson() => JsonSerializer.Serialize(this, new JsonSerializerOptions { WriteIndented = true });
    public static GridlyLayoutProfile FromJson(string json) => JsonSerializer.Deserialize<GridlyLayoutProfile>(json) ?? new GridlyLayoutProfile();
}

public static class GridlyProfileMigrator
{
    public static GridlyLayoutProfile FromLegacyColumnProfileJson(string json, string name)
    {
        using JsonDocument doc = JsonDocument.Parse(json);
        GridlyLayoutProfile profile = new GridlyLayoutProfile { Name = name, DisplayName = name, ProfileVersion = 29 };
        ...
        if (doc.RootElement.TryGetProperty("Columns", out JsonElement columns) && columns.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                GridlyColumnLayout layout = new GridlyColumnLayout();
        ...
        {
            profile.State.Columns.Add(new GridlyColumnLayout
            {
                AspectName = key,
                Text = key,
...
public sealed class GridlyColumnLayout
{
