...
{
    private readonly TextBox _search = new() { Dock = DockStyle.Fill, PlaceholderText = GridlyText.ColumnSearchPlaceholder, BorderStyle = BorderStyle.FixedSingle };
    ...
    private readonly Button _all = new() { Text = GridlyText.ShowAll, Width = 112, Height = 30 };
    private readonly Button _none = new() { Text = GridlyText.HideAll, Width = 112, Height = 30 };
    private readonly Button _ok = new() { Text = GridlyText.OK, DialogResult = DialogResult.OK, Width = 92, Height = 30 };
    private readonly Button _cancel = new() { Text = GridlyText.Cancel, DialogResult = DialogResult.Cancel, Width = 92, Height = 30 };
    ...
    private readonly IList<GridlyColumn> _columns;
    private readonly GridlyTheme _theme;
    private readonly Dictionary<GridlyColumn, bool> _pendingStates = new();
    ...
    public ColumnChooserForm(IList<GridlyColumn> columns) : this(columns, WindowsThemeService.CurrentTheme())
    {
    }

    public ColumnChooserForm(IList<GridlyColumn> columns, GridlyTheme? theme)
    {
        _columns = columns;
        _theme = theme ?? WindowsThemeService.CurrentTheme();

        Text = GridlyText.ColumnChooserTitle;
        Width = 450;
        Height = 460;
        MinimumSize = new Size(380, 360);
        KeyPreview = true;
        StartPosition = FormStartPosition.CenterParent;
        GridlyDialogChrome.ConfigureStandardDialog(this, _theme, new Size(380, 360), sizeable: true, iconKind: GridlyDialogIconKind.Column);
        AcceptButton = _ok;
    ...
    {
        GridlyDialogChrome.ConfigureOpenGridlyDialogs(this, _theme);
    ...
    private static string GetColumnText(GridlyColumn c)
    {
    ...
    private static GridlyColumn? GetColumnFromItem(object? item)
    {
        if (item is ColumnChooserItem chooserItem) return chooserItem.Column;
        return item as GridlyColumn;
    }

    private sealed class ColumnChooserItem
    {
        public ColumnChooserItem(GridlyColumn column, string text)
        {
            Column = column;
            Text = text;
        }
