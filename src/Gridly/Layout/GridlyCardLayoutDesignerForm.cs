...
public sealed class GridlyCardLayoutDesignerForm : Form
{
    ...
    private readonly GridlyCardLayoutDefinition _definition;
    private bool _syncing;

    public GridlyCardLayoutDefinition Result => _definition;

    public GridlyCardLayoutDesignerForm(IEnumerable<GridlyColumn> columns, GridlyCardLayoutDefinition? current = null, GridlyTheme? theme = null)
    {
        _definition = current ?? GridlyCardLayoutDefinition.FromColumns(columns);
        Text = "Gridly Card Layout Designer";
        StartPosition = FormStartPosition.CenterParent;
        Size = new Size(760, 520);
        MinimumSize = new Size(620, 420);
        Font = new Font("Segoe UI", 9F);
        GridlyDialogChrome.ConfigureStandardDialog(this, theme ?? WindowsThemeService.CurrentTheme(), new Size(620, 420), sizeable: true, iconKind: GridlyDialogIconKind.Designer);
        AcceptButton = _ok;
        CancelButton = _cancel;

        _role.Items.AddRange(Enum.GetNames(typeof(GridlyCardFieldRole)));
        _density.Items.AddRange(Enum.GetNames(typeof(GridlyCardLayoutDensity)));
        ...
        _density.SelectedIndexChanged += (_, _) => { if (Enum.TryParse<GridlyCardLayoutDensity>(Convert.ToString(_density.SelectedItem), out var d)) _definition.Density = d; };
        _up.Click += (_, _) => MoveSelected(-1);
        _down.Click += (_, _) => MoveSelected(1);

        ApplyTheme(theme ?? GridlyTheme.DarkTheme());
        Refill();
    ...
    private GridlyCardLayoutField? SelectedField => _fields.SelectedItem as GridlyCardLayoutField;
    ...
    private void LoadSelectedField()
    {
        ...
        try
        {
            _role.SelectedItem = field?.Role.ToString() ?? GridlyCardFieldRole.Body.ToString();
    ...
    private void SaveSelectedField()
    {
        ...
        if (Enum.TryParse<GridlyCardFieldRole>(Convert.ToString(_role.SelectedItem), out var role)) field.Role = role;
