...
/// </summary>
public sealed class GridlyColumnCollectionEditor : UITypeEditor
{
    // WinForms designer / GridlyViewDesigner tarafı bazen editor
    ...
    public GridlyColumnCollectionEditor()
    {
    }

    public GridlyColumnCollectionEditor(Type collectionType)
    {
    ...
    {
        if (value is not GridlyColumnCollection columns)
            return value;

        GridlyColumnCollectionEditor.EditColumns(
            columns,
            context?.Instance,
    ...
    internal static bool EditColumns(
        GridlyColumnCollection columns,
        ...
        bool centerScreen = false)
    {
        ...
        using var tx = host?.CreateTransaction("GridlyView kolonları düzenlendi");
        try
        {
            ...
            using var form = new GridlyColumnEditorForm(columns, host);
            if (centerScreen)
            {
    ...
    private static void EnsureDesignerSitedColumns(GridlyColumnCollection columns, IDesignerHost? host)
    {
        ...
        for (int i = 0; i < columns.Count; i++)
        {
            ...
            desiredName = GridlyColumnNameHelper.EnsureUnique(desiredName, columns.Where((_, idx) => idx != i));

            current.Name = desiredName;
        }
    }

    internal static void CopyColumn(GridlyColumn source, GridlyColumn target)
    {
    ...
    internal static GLVColumn CreateDesignerColumn(GridlyColumnCollection columns, IDesignerHost? host)
    {
        var name = CreateUniqueName(columns, host, GridlyColumnNameHelper.GetNextDefaultOrdinal(columns));
    ...
    private static string CreateUniqueName(GridlyColumnCollection columns, IDesignerHost? host, int startIndex)
    {
        ...
        while (true)
        {
            var name = GridlyColumnNameHelper.CreateDefaultName(i);
...
internal sealed class GridlyColumnEditorForm : System.Windows.Forms.Form
{
    private readonly GridlyColumnCollection _columns;
    ...
    private readonly HashSet<GridlyColumn> _createdColumns = new();
    private bool _committed;
    private bool _restored;

    public GridlyColumnEditorForm(GridlyColumnCollection columns, IDesignerHost? host)
    {
        _columns = columns;
        _host = host;

        Text = "Gridly Kolonları";
        StartPosition = System.Windows.Forms.FormStartPosition.CenterParent;
        Width = 760;
        Height = 520;
        GridlyDialogChrome.ConfigureStandardDialog(this, WindowsThemeService.CurrentTheme(), new System.Drawing.Size(640, 420), sizeable: true, iconKind: GridlyDialogIconKind.Column);
        ...
        // DisplayMember kullanmıyoruz; ListBox item text'i doğrudan GridlyColumn.ToString()
    ...
    private bool IsOriginalColumn(GridlyColumn column)
    ...
    private void DestroyDesignerComponent(GridlyColumn column)
    {
    ...
    {
        // v26.64: GridlyColumn artık Component tabanlı olduğu için Visual Studio
        // zaten gerçek Design > (Name) alanını kendisi sağlar. PropertyGrid'i
        // doğrudan kolona bağlamak çift (Name) satırını engeller; GridlyColumn.Name
        // property’si Browsable(false) olduğu için sadece VS'nin Component adı görünür.
        _propertyGrid.SelectedObject = _list.SelectedItem is GridlyColumn column
            ? column
            : null;
        UpdateButtons();
    }
    ...
    {
        var column = GridlyColumnCollectionEditor.CreateDesignerColumn(_columns, _host);
    ...
    {
        if (_list.SelectedItem is not GridlyColumn column) return;
...
internal sealed class GridlyColumnPropertyGridObject : CustomTypeDescriptor
{
    private readonly GridlyColumn _column;

    public GridlyColumnPropertyGridObject(GridlyColumn column)
    ...
    public override PropertyDescriptorCollection GetProperties(Attribute[]? attributes)
    {
        ...
        {
            if (string.Equals(property.Name, nameof(GridlyColumn.Name), StringComparison.Ordinal))
            {
                if (!nameAdded)
                {
                    result.Add(new GridlyColumnNamePropertyDescriptor(property));
                    nameAdded = true;
                }
                continue;
            }
        ...
        if (!nameAdded)
            result.Insert(0, new GridlyColumnNamePropertyDescriptor(null));
...
internal sealed class GridlyColumnNamePropertyDescriptor : PropertyDescriptor
{
    private readonly PropertyDescriptor? _inner;

    public GridlyColumnNamePropertyDescriptor(PropertyDescriptor? inner)
        : base(nameof(GridlyColumn.Name), null)
    {
        _inner = inner;
    }

    public override Type ComponentType => typeof(GridlyColumn);
    ...
    {
        var column = component as GridlyColumn;
    ...
    {
        if (component is GridlyColumn column)
...
{
    public OriginalColumnEntry(GridlyColumn column, ColumnSnapshot snapshot)
    {
        Column = column;
        Snapshot = snapshot;
    }
...
internal sealed class ColumnSnapshot
    {
        ...
        public static ColumnSnapshot From(GridlyColumn column)
        {
        ...
        public void ApplyTo(GridlyColumn column)
        {
