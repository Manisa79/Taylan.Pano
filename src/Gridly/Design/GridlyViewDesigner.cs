...
namespace Gridly.Design;

/// <summary>
/// Visual Studio Designer smart-tag support for GridlyView.
/// It exposes the most frequently changed ListView/Gridly-like settings at design time.
/// </summary>
public sealed class GridlyViewDesigner : Microsoft.DotNet.DesignTools.Designers.ControlDesigner
{
    private DesignerActionListCollection? _actionLists;
    private DesignerVerbCollection? _verbs;
    private GridlyViewActionList? _registeredActionList;
    ...
    public override DesignerVerbCollection Verbs
    {
        get
        {
            ...
            {
                new DesignerVerb("GridlyView: Gridly Kolon Editörünü Aç...", (_, _) => CreateActionList().EditColumns()),
                new DesignerVerb("GridlyView: Varsayılan Kolon Ekle", (_, _) => CreateActionList().AddDefaultColumns()),
                new DesignerVerb("GridlyView: Checkbox Seçimini Aç", (_, _) => CreateActionList().AddCheckBoxColumn()),
                new DesignerVerb("GridlyView: Ana Kapsayıcıda Yerleştir", (_, _) => CreateActionList().DockFill()),
                new DesignerVerb("GridlyView: Aktif Senaryoyu Uygula", (_, _) => CreateActionList().ApplyActiveScenario()),
                new DesignerVerb("GridlyView: Designer Açık Temayı Uygula", (_, _) => CreateActionList().UseLightTheme()),
                new DesignerVerb("GridlyView: Önizlemeyi Yenile", (_, _) => CreateActionList().RefreshPreview())
            };
            return _verbs;
        }
    }
    ...
    public override void Initialize(IComponent component)
    {
        ...
        if (component is not GridlyView)
            return;

        _registeredActionList = new GridlyViewActionList(component);
        _actionLists = null;
    ...
    public override void InitializeNewComponent(IDictionary defaultValues)
    {
        ...
        if (Component is GridlyView gridly)
        {
            gridly.ShowHeader = true;
            gridly.ShowGridLines = true;
            gridly.FullRowSelect = true;
            gridly.UseUnifiedThemeVisuals = true;
            gridly.AutoApplyThemeToColumnHeaders = true;
            gridly.AutoEnsureReadableTextColors = true;
        }
    }
    ...
    private GridlyViewActionList CreateActionList()
    {
        if (_registeredActionList != null)
            return _registeredActionList;

        if (Component == null)
            throw new InvalidOperationException("GridlyView designer component is not initialized.");
...
public sealed class GridlyViewActionList : DesignerActionList
{
    private readonly global::Gridly.Core.GridlyView _control;
    ...
    public GridlyViewActionList(IComponent component) : base(component)
    {
        _control = (global::Gridly.Core.GridlyView)component;
    ...
    public GridlyViewMode ViewMode
    {
    ...
    public GridlyViewScenario ActiveScenario
    {
    ...
    public bool SelectionHeaderCheckBox
    {
        ...
        set
        {
            ...
            using var tx = host?.CreateTransaction(value ? "GridlyView header checkbox açıldı" : "GridlyView header checkbox kapatıldı");
    ...
    public GridlyColumnCollection Columns
    {
        get => _control.Columns;
        set
        {
            // Koleksiyon nesnesi GridlyView içinde sabittir; Visual Studio SmartTag
    ...
    public GridlyHeaderContextMenuBehavior HeaderContextMenuBehavior
    {
    ...
    private void EditColumnsCore()
    {
        ...
        try
        {
            ...
            // GridlyColumnCollectionEditor formunu açar. VS native CollectionEditor
            // fallback'i kaldırıldı; böylece iki farklı editor/sıra/serialization yolu
            // birbirine karışmaz.
            if (GridlyColumnCollectionEditor.EditColumns(_control.Columns, _control, columnsProperty, provider, centerScreen: true))
            {
    ...
    {
        // v25.80: Gridly uyumlu seçim checkbox'ı ayrı kolon oluşturmaz.
        // İlk görünür veri kolonunun içinde çizilir. Bu nedenle SmartTag artık
        // yeni kolon eklemek yerine CheckBoxes özelliğini açar.
        var host = GetDesignerHost();
        using var tx = host?.CreateTransaction("GridlyView seçim checkbox'ı açıldı");
    ...
    {
        var host = GetDesignerHost();
        using var tx = host?.CreateTransaction("GridlyView varsayılan kolonlar eklendi");
        RaiseChanging(nameof(_control.Columns));
    ...
    {
        var baseName = GridlyColumnNameHelper.CreateNameFromAspectOrText(aspectName, text, _control.Columns.Count + 1);
    ...
    {
        using var tx = GetDesignerHost()?.CreateTransaction("GridlyView kolonları temizlendi");
        RaiseChanging(nameof(_control.Columns));
    ...
    public void SetObjectMode() => SetProperty(nameof(_control.Mode), GridlyMode.Object);
    public void SetDataTableMode() => SetProperty(nameof(_control.Mode), GridlyMode.DataTable);
    public void SetVirtualMode() => SetProperty(nameof(_control.Mode), GridlyMode.Virtual);
    public void SetTreeMode() => SetProperty(nameof(_control.Mode), GridlyMode.Tree);
    public void SetTileMode() => SetProperty(nameof(_control.Mode), GridlyMode.Tile);
    public void UseAutoTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), GridlyDesignTimeThemePreview.Auto);
    public void UseLightTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), GridlyDesignTimeThemePreview.Light);
    public void UseDarkTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), GridlyDesignTimeThemePreview.Dark);

    public void MenuFull() => ApplyMenuProfile(GridlyMenuProfile.Full);
    public void MenuStandard() => ApplyMenuProfile(GridlyMenuProfile.Standard);
    public void MenuMinimal() => ApplyMenuProfile(GridlyMenuProfile.Minimal);
    public void MenuReadOnly() => ApplyMenuProfile(GridlyMenuProfile.ReadOnly);
    public void MenuNone() => ApplyMenuProfile(GridlyMenuProfile.None);

    private void ApplyMenuProfile(GridlyMenuProfile profile)
    {
    ...
    public override DesignerActionItemCollection GetSortedActionItems()
    {
        ...
        {
            new DesignerActionMethodItem(this, nameof(EditColumns), "Gridly Kolon Editörünü Aç...", "1. Kolonlar", "Gridly'ye özgü kolon düzenleme penceresini açar.", true),
            ...
            new DesignerActionPropertyItem(nameof(UseBuiltInHeaderMenu), "Başlık Menüsü", "5. Menü / Filtre", "Gridly'nin dahili başlık menüsünü açar/kapatır."),
            new DesignerActionPropertyItem(nameof(UseBuiltInBodyMenu), "Gövde Menüsü", "5. Menü / Filtre", "Gridly'nin dahili gövde menüsünü açar/kapatır."),
            ...
            new DesignerActionPropertyItem(nameof(ThemePreset), "Tema Preseti:", "7. Tema / Designer", "Gridly tema presetini seçer."),
    ...
    private sealed class GridlyTypeDescriptorContext : ITypeDescriptorContext
    {
        private readonly object _instance;
        private readonly PropertyDescriptor? _propertyDescriptor;
        private readonly IServiceProvider? _serviceProvider;

        public GridlyTypeDescriptorContext(object instance, PropertyDescriptor? propertyDescriptor, IServiceProvider? serviceProvider = null)
        {
            _instance = instance;
