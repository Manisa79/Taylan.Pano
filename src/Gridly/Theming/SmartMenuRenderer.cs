...
/// <summary>
/// Theme-aware ToolStrip renderer used by Gridly popup menus, merged user menus and nested drop-down menus.
...
public sealed class SmartMenuRenderer : ToolStripProfessionalRenderer
{
    ...
    public SmartMenuRenderer(GridlyTheme theme) : base(new SmartMenuColorTable(theme))
    {
        _theme = theme;
    }

    public static void ApplyTo(ToolStrip? strip, GridlyTheme theme)
    {
    ...
    public static void ApplyToItem(ToolStripItem item, GridlyTheme theme)
    {
    ...
    private static void OnDropDownOpening(object? sender, EventArgs e)
    {
        ...
        if (item.DropDown.Tag is not GridlyTheme theme) return;
...
{
    private readonly GridlyTheme _theme;
    public SmartMenuColorTable(GridlyTheme theme) => _theme = theme;
