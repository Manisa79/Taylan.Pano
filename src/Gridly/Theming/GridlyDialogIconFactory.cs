...
/// <summary>
/// Creates small theme-aware icons for Gridly owned tool windows. The icons are
/// intentionally generated in code so the library does not need external image
/// resources and stays designer/package friendly.
/// </summary>
public static class GridlyDialogIconFactory
{
    public static Icon Create(GridlyDialogIconKind kind, GridlyTheme theme, int size = 32)
    {
    ...
    private static void DrawGlyph(Graphics g, GridlyDialogIconKind kind, Rectangle r, Pen pen, Pen accentPen, Brush accentBrush, Brush textBrush, GridlyTheme theme)
    {
        int x = r.X;
        int y = r.Y;
        int w = r.Width;
        ...
        switch (kind)
        {
            case GridlyDialogIconKind.Filter:
                Point[] funnel =
                {
                    new(x + w / 4, y + h / 4),
                    new(x + 3 * w / 4, y + h / 4),
            ...
            case GridlyDialogIconKind.Export:
                ...
                break;
            case GridlyDialogIconKind.Designer:
