...
public enum GridlyThemeStudioPreset
{
    Light,
    Dark,
    HighContrastDark,
...
public sealed class GridlyThemeStudioPalette
{
    public string Name { get; set; } = string.Empty;
    public GridlyThemeStudioPreset Preset { get; set; }
    public GridlyTheme Theme { get; set; } = GridlyTheme.LightTheme();
    public string Notes { get; set; } = string.Empty;
}

public static class GridlyThemeStudio
{
    public static GridlyTheme Create(GridlyThemeStudioPreset preset)
    {
        GridlyTheme theme = preset switch
        {
            GridlyThemeStudioPreset.Dark => GridlyTheme.DarkTheme(),
            GridlyThemeStudioPreset.HighContrastDark => HighContrastDark(),
            GridlyThemeStudioPreset.HighContrastLight => HighContrastLight(),
            GridlyThemeStudioPreset.AudixDark => AudixDark(),
            GridlyThemeStudioPreset.AudixLight => AudixLight(),
            GridlyThemeStudioPreset.AoiSupportDark => AoiSupportDark(),
            GridlyThemeStudioPreset.AoiSupportLight => AoiSupportLight(),
            GridlyThemeStudioPreset.FactoryOsDark => FactoryOsDark(),
            GridlyThemeStudioPreset.FactoryOsLight => FactoryOsLight(),
            GridlyThemeStudioPreset.SmokeWhite => SmokeWhite(),
            GridlyThemeStudioPreset.MidnightGlass => MidnightGlass(),
            _ => GridlyTheme.LightTheme()
        };
        return GridlyThemeAccessibility.Normalize(theme);
    }

    public static IReadOnlyList<GridlyThemeStudioPalette> BuiltInPalettes()
    {
        return Enum.GetValues(typeof(GridlyThemeStudioPreset))
            .Cast<GridlyThemeStudioPreset>()
            .Select(p => new GridlyThemeStudioPalette
            {
                Name = p.ToString(),
                Preset = p,
                Theme = Create(p),
    ...
    public static string ExportPaletteText(GridlyTheme theme, string name = "Custom")
    {
        theme = GridlyThemeAccessibility.Normalize(theme);
        return string.Join(Environment.NewLine, new[]
        {
            "Gridly Theme Studio Palette: " + name,
    ...
    private static GridlyTheme HighContrastDark()
    {
        var t = GridlyTheme.DarkTheme();
    ...
    private static GridlyTheme HighContrastLight()
    {
        var t = GridlyTheme.LightTheme();
    ...
    private static GridlyTheme AudixDark()
    {
    ...
    private static GridlyTheme AudixLight()
    {
    ...
    private static GridlyTheme AoiSupportDark()
    {
        var t = HighContrastDark();
    ...
    private static GridlyTheme AoiSupportLight()
    {
        var t = HighContrastLight();
    ...
    private static GridlyTheme FactoryOsDark()
    {
        var t = HighContrastDark();
    ...
    private static GridlyTheme FactoryOsLight()
    {
        var t = HighContrastLight();
    ...
    private static GridlyTheme SmokeWhite()
    {
        var t = HighContrastLight();
    ...
    private static GridlyTheme MidnightGlass()
    {
        var t = HighContrastDark();
    ...
    private static string GetNotes(GridlyThemeStudioPreset preset) => preset switch
    {
        GridlyThemeStudioPreset.AudixDark or GridlyThemeStudioPreset.AudixLight => "Albüm kapağı, poster, media tile ve filmstrip için yüksek okunurluk.",
        GridlyThemeStudioPreset.AoiSupportDark or GridlyThemeStudioPreset.AoiSupportLight => "Ticket kartları, durum rozeti ve operatör/teknisyen mesajları için.",
        GridlyThemeStudioPreset.FactoryOsDark or GridlyThemeStudioPreset.FactoryOsLight => "Line Workspace, Factory Navigator ve üretim dashboardları için.",
        GridlyThemeStudioPreset.SmokeWhite => "NavixBar ve açık panel yüzeyleri ile yumuşak uyum.",
        _ => "Genel Gridly tema paleti."
    };
